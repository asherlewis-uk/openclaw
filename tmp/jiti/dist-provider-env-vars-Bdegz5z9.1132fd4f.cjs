...
function isWorkspacePluginTrustedForProviderEnvVars(plugin, config) {
  return (0, _providerAuthAliases3NFJcokO.i)({
    config,
    isImplicitlyAllowed: (pluginId) => (0, _slotsDLJ2h3Qv.r)(plugin.kind, "context-engine") && (0, _providerAuthAliases3NFJcokO.a)(config?.plugins?.slots?.contextEngine) === pluginId,
    plugin
  });
}
...
function resolveProviderMetadataSnapshot(params) {
  ...
  if ((0, _configStateBgyjpLHd.s)(config.plugins).loadPaths.length === 0) {
    const unscopedCurrent = (0, _currentPluginMetadataSnapshotDHmOxzz.n)({
      env,
      ...(params?.workspaceDir !== void 0 ? { workspaceDir: params.workspaceDir } : {}),
      allowWorkspaceScopedSnapshot: true,
...
function resolveManifestProviderAuthEnvVarCandidates(params) {
  ...
  const aliases = (0, _providerAuthAliases3NFJcokO.n)(params);
...
function resolveManifestProviderAuthEvidence(params) {
  ...
  for (const plugin of snapshot.plugins) {
    if (snapshot.index.plugins.length > 0 && !(0, _installedPluginIndexStoreDetkjvO.d)(snapshot.index, plugin.id, params?.config)) continue;
