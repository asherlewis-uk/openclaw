...
function listPackageRoots() {
  return [(0, _openclawRootDDaGBMF_.n)({ cwd: process.cwd() }), (0, _openclawRootDDaGBMF_.n)({ moduleUrl: "file:///Users/asherlewis/.nvm/versions/node/v22.22.2/lib/node_modules/openclaw/dist/ids-DFu3Ho6n.js" })].filter((entry, index, all) => Boolean(entry) && all.indexOf(entry) === index);
}
function readBundledExtensionCatalogEntriesSync() {
  const pluginsDir = (0, _bundledDirLBl9nCAz.n)();
  ...
  try {
    const entries = _nodeFs.default.readdirSync(pluginsDir, { withFileTypes: true }).filter((entry) => entry.isDirectory()).flatMap((entry) => {
      const parsed = (0, _jsonFilesCahFuwKs.u)(_nodePath.default.join(pluginsDir, entry.name, "package.json"));
...
function readOfficialCatalogFileSync() {
  for (const packageRoot of listPackageRoots()) {
    ...
    const payload = (0, _jsonFilesCahFuwKs.u)(candidate);
    if (payload) {
