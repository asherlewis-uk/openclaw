"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports._ = resolveResponseUsageMode;exports.a = listThinkingLevels;exports.c = resolveThinkingProfile;exports.d = normalizeElevatedLevel;exports.f = normalizeReasoningLevel;exports.g = normalizeVerboseLevel;exports.h = normalizeUsageDisplay;exports.i = listThinkingLevelOptions;exports.l = void 0;exports.m = normalizeTraceLevel;exports.n = isThinkingLevelSupported;exports.o = resolveSupportedThinkingLevel;exports.p = normalizeThinkLevel;exports.r = listThinkingLevelLabels;exports.s = resolveThinkingDefaultForModel;exports.t = formatThinkingLevels;exports.u = isSessionDefaultDirectiveValue;exports.v = resolveBundledProviderPolicySurface;var _stringCoerceLndEvhRk = require("./string-coerce-LndEvhRk.js");
...
var _publicSurfaceLoaderCHXRvo6d = require("./public-surface-loader-CHXRvo6d.js");
...
function tryLoadBundledProviderPolicySurface(pluginId) {
  for (const artifactBasename of PROVIDER_POLICY_ARTIFACT_CANDIDATES) try {
    const mod = (0, _publicSurfaceLoaderCHXRvo6d.t)({
      dirName: pluginId,
      artifactBasename
    });
...
function resolveBundledProviderPolicyPluginId(providerId, options = {}) {
  const normalizedProviderId = (0, _providerIdCz7K6wgK.r)(providerId);
  if (!normalizedProviderId) return null;
  if (!(0, _bundledDirBocaljIz.n)()) return null;
  const registry = options.manifestRegistry ?? (0, _manifestRegistryDbnX5kdP.t)();
  for (const plugin of registry.plugins.toSorted((left, right) => left.id.localeCompare(right.id))) {
    if (plugin.origin !== "bundled") continue;
    if (plugin.providers.some((provider) => (0, _providerIdCz7K6wgK.r)(provider) === normalizedProviderId)) return plugin.id;
  }
  return null;
}
...
const BASE_THINKING_LEVELS = exports.l = [
"off",
"minimal",
"low",
"medium",
...
function normalizeUsageDisplay(raw) {
  if (!raw) return;
  const key = (0, _stringCoerceLndEvhRk.a)(raw);
  if ([
  "off",
  "false",
  "no",
...
function resolveThinkingPolicyContext(params) {
  const providerRaw = (0, _stringCoerceLndEvhRk.c)(params.provider);
  const normalizedProvider = providerRaw ? (0, _providerIdCz7K6wgK.r)(providerRaw) : "";
  const modelId = (0, _stringCoerceLndEvhRk.c)(params.model) ?? "";
  const modelKey = (0, _stringCoerceLndEvhRk.s)(params.model) ?? "";
  const candidate = params.catalog?.find((entry) => (0, _providerIdCz7K6wgK.r)(entry.provider) === normalizedProvider && entry.id === modelId);
  return {
    normalizedProvider,
    modelId,
    modelKey,
...
function normalizeProfileLevel(level) {
  ...
  return {
    id: normalized,
    label: (0, _stringCoerceLndEvhRk.c)(level.label) ?? normalized,
