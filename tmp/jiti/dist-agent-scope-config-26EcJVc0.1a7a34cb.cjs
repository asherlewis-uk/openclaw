...
var _workspaceDefaultCg5lIU6L = require("./workspace-default-Cg5lIU6L.js");
...
function warnMultipleDefaultAgents() {
  Promise.resolve().then(() => jitiImport("./subsystem-Cb901ds8.js").then((m) => _interopRequireWildcard(m))).then(({ createSubsystemLogger }) => {
...
function listAgentIds(cfg) {
  ...
  if (agents.length === 0) return [_sessionKeyDFEyR49L.t];
  const seen = /* @__PURE__ */new Set();
  const ids = [];
  for (const entry of agents) {
    const id = (0, _sessionKeyDFEyR49L.c)(entry?.id);
    if (seen.has(id)) continue;
    seen.add(id);
    ids.push(id);
  }
  return ids.length > 0 ? ids : [_sessionKeyDFEyR49L.t];
}
function resolveDefaultAgentId(cfg) {
  ...
  return (0, _sessionKeyDFEyR49L.c)(chosen || "main");
}
function resolveAgentEntry(cfg, agentId) {
  const id = (0, _sessionKeyDFEyR49L.c)(agentId);
  return listAgentEntries(cfg).find((entry) => (0, _sessionKeyDFEyR49L.c)(entry.id) === id);
}
function resolveAgentConfig(cfg, agentId) {
  const entry = resolveAgentEntry(cfg, (0, _sessionKeyDFEyR49L.c)(agentId));
  if (!entry) return;
...
function resolveAgentWorkspaceDir(cfg, agentId, env = process.env) {
  ...
  if (id === defaultAgentId) {
    ...
    return stripNullBytes((0, _workspaceDefaultCg5lIU6L.n)(env));
  }
