"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.n = withBundledPluginEnablementCompat;exports.r = withBundledPluginVitestCompat;exports.t = withBundledPluginAllowlistCompat;var _configStateBgyjpLHd = require("./config-state-BgyjpLHd.js");
...
function withBundledPluginEnablementCompat(params) {
  ...
  const allow = params.config?.plugins?.allow;
  const allowSet = !useCompatDiscovery && Array.isArray(allow) && allow.length > 0 ? new Set(allow.map((pluginId) => (0, _configStateBgyjpLHd.o)(pluginId)).filter(Boolean)) : void 0;
...
function withBundledPluginVitestCompat(params) {
  ...
  if (!Boolean(env.VITEST) || (0, _manifestRegistryDgt5vVG.n)(params.config?.plugins) || params.pluginIds.length === 0) return params.config;
