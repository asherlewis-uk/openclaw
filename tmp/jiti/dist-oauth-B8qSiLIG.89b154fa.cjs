...
var _resolveBHS7vHp = require("./resolve-BHS7vHp1.js");
...
var _chutesOauthDpVhQFi = require("./chutes-oauth-DpVhQFi0.js");
var _policyCYC_UyL = require("./policy-CYC_UyL9.js");
var _repairCzDiEiN = require("./repair-CzDi-eiN.js");
...
var OAuthManagerRefreshError = class extends Error {
  ...
  constructor(params) {
    ...
    super(`OAuth token refresh failed for ${params.credential.provider}: ${(0, _errorsVfATXfah.i)(params.cause)}`, { cause: delegatedCause });
...
async function loadFreshStoredOAuthCredential(params) {
  const reloaded = (0, _storeA4exFSck.l)(params.agentDir).profiles[params.profileId];
  if (reloaded?.type !== "oauth" || reloaded.provider !== params.provider || !(0, _storeA4exFSck.b)(reloaded)) return null;
...
function resolveEffectiveOAuthCredential(params) {
  ...
  if ((0, _storeA4exFSck.b)(params.credential)) {
    _storeA4exFSck.L.debug("resolved oauth credential from canonical local store", {
      profileId: params.profileId,
  ...
  if (!(0, _storeA4exFSck.x)(params.credential, imported)) {
    _storeA4exFSck.L.warn("refused external oauth bootstrap credential: identity mismatch or missing binding", {
      profileId: params.profileId,
      provider: params.credential.provider
    });
    return params.credential;
  }
  if ((0, _storeA4exFSck.C)({
    existing: params.credential,
    imported
  })) {
    _storeA4exFSck.L.debug("resolved oauth credential from external cli bootstrap", {
      profileId: params.profileId,
...
function createOAuthManager(adapter) {
  function adoptNewerMainOAuthCredential(params) {
    ...
    try {
      const mainCred = (0, _storeA4exFSck.i)(void 0, { allowKeychainPrompt: false }).profiles[params.profileId];
      if (mainCred?.type === "oauth" && mainCred.provider === params.credential.provider && (0, _storeA4exFSck.b)(mainCred) && Number.isFinite(mainCred.expires) && (!Number.isFinite(params.credential.expires) || mainCred.expires > params.credential.expires) && (0, _storeA4exFSck.S)(params.credential, mainCred)) {
        params.store.profiles[params.profileId] = { ...mainCred };
        _storeA4exFSck.L.info("adopted newer OAuth credentials from main agent", {
          profileId: params.profileId,
      ...
      _storeA4exFSck.L.debug("adoptNewerMainOAuthCredential failed", {
        profileId: params.profileId,
  ...
  async function mirrorRefreshedCredentialIntoMainStore(params) {
    try {
      (0, _storeA4exFSck._)((0, _sourceCheckBnSDslnj.l)(void 0));
      await (0, _storeA4exFSck.p)({
        agentDir: void 0,
        updater: (store) => {
          ...
          if (!decision.shouldMirror) {
            if (decision.reason === "identity-mismatch-or-regression") _storeA4exFSck.L.warn("refused to mirror OAuth credential: identity mismatch or regression", { profileId: params.profileId });
            return false;
          }
          store.profiles[params.profileId] = { ...params.refreshed };
          _storeA4exFSck.L.debug("mirrored refreshed OAuth credential to main agent store", {
            profileId: params.profileId,
      ...
      _storeA4exFSck.L.debug("mirrorRefreshedCredentialIntoMainStore failed", {
        profileId: params.profileId,
        error: (0, _errorsVfATXfah.i)(err)
      });
    }
  ...
  async function doRefreshOAuthTokenWithLock(params) {
    const ownerAgentDir = (0, _storeA4exFSck.d)(params);
    const authPath = (0, _sourceCheckBnSDslnj.l)(ownerAgentDir);
    (0, _storeA4exFSck._)(authPath);
    const globalRefreshLockPath = (0, _sourceCheckBnSDslnj.f)(params.provider, params.profileId);
    try {
      return await (0, _fileLockDDZT8E.o)(globalRefreshLockPath, _storeA4exFSck.I, async () => (0, _fileLockDDZT8E.o)(authPath, _storeA4exFSck.M, async () => {
        const store = (0, _storeA4exFSck.l)(ownerAgentDir);
        const cred = store.profiles[params.profileId];
        if (!cred || cred.type !== "oauth") return null;
        let credentialToRefresh = cred;
        if ((0, _storeA4exFSck.b)(cred)) return {
        ...
        if (params.agentDir) try {
          const mainCred = (0, _storeA4exFSck.l)(void 0).profiles[params.profileId];
          if (mainCred?.type === "oauth" && mainCred.provider === cred.provider && (0, _storeA4exFSck.b)(mainCred) && (0, _storeA4exFSck.S)(cred, mainCred)) {
            store.profiles[params.profileId] = { ...mainCred };
            _storeA4exFSck.L.info("adopted fresh OAuth credential from main store (under refresh lock)", {
              profileId: params.profileId,
              agentDir: params.agentDir,
              expires: new Date(mainCred.expires).toISOString()
            });
          ...
          } else if (mainCred?.type === "oauth" && mainCred.provider === cred.provider && (0, _storeA4exFSck.b)(mainCred) && !(0, _storeA4exFSck.S)(cred, mainCred)) _storeA4exFSck.L.warn("refused to adopt fresh main-store OAuth credential: identity mismatch", {
            profileId: params.profileId,
            agentDir: params.agentDir
          });
        } catch (err) {
          _storeA4exFSck.L.debug("inside-lock main-store adoption failed; proceeding to refresh", {
            profileId: params.profileId,
            error: (0, _errorsVfATXfah.i)(err)
          });
        }
        ...
        if (externallyManaged) if (externallyManaged.provider !== cred.provider) _storeA4exFSck.L.warn("refused external oauth bootstrap credential: provider mismatch", {
          profileId: params.profileId,
          provider: cred.provider
        });else
        if (!(0, _storeA4exFSck.x)(cred, externallyManaged)) _storeA4exFSck.L.warn("refused external oauth bootstrap credential: identity mismatch or missing binding", {
          profileId: params.profileId,
          provider: cred.provider
        });else
        {
          if ((0, _storeA4exFSck.w)(cred, externallyManaged) && !(0, _storeA4exFSck.y)(cred, externallyManaged)) {
            store.profiles[params.profileId] = { ...externallyManaged };
            (0, _storeA4exFSck.f)(store, ownerAgentDir);
          }
          credentialToRefresh = externallyManaged;
          if ((0, _storeA4exFSck.b)(externallyManaged)) return {
        ...
        const refreshedCredentials = await withRefreshCallTimeout(`refreshOAuthCredential(${cred.provider})`, _storeA4exFSck.F, async () => {
        ...
        (0, _storeA4exFSck.f)(store, ownerAgentDir);
        if (ownerAgentDir) {
          if ((0, _sourceCheckBnSDslnj.l)(void 0) !== authPath) await mirrorRefreshedCredentialIntoMainStore({
            profileId: params.profileId,
  ...
  async function resolveOAuthAccess(params) {
    ...
    if ((0, _storeA4exFSck.b)(effectiveCredential)) return {
    ...
    try {
      ...
      const refreshedStore = (0, _storeA4exFSck.l)(params.agentDir);
      const refreshed = refreshedStore.profiles[params.profileId];
      if (refreshed?.type === "oauth" && (0, _storeA4exFSck.b)(refreshed)) return {
      ...
      if (params.agentDir) try {
        const mainCred = (0, _storeA4exFSck.i)(void 0, { allowKeychainPrompt: false }).profiles[params.profileId];
        if (mainCred?.type === "oauth" && mainCred.provider === params.credential.provider && (0, _storeA4exFSck.b)(mainCred) && (0, _storeA4exFSck.S)(params.credential, mainCred)) {
          refreshedStore.profiles[params.profileId] = { ...mainCred };
          _storeA4exFSck.L.info("inherited fresh OAuth credentials from main agent", {
            profileId: params.profileId,
            agentDir: params.agentDir,
            expires: new Date(mainCred.expires).toISOString()
          });
...
function extractErrorMessage(error) {
  return (0, _errorsVfATXfah.i)(error);
}
...
async function refreshOAuthCredential(credential) {
  ...
  if (credential.provider === "chutes") return await (0, _chutesOauthDpVhQFi.a)({ credential });
...
const oauthManager = createOAuthManager({
  ...
  readBootstrapCredential: ({ profileId, credential }) => (0, _storeA4exFSck.v)({
    profileId,
    credential
  }),
...
async function resolveProfileSecretString(params) {
  ...
  if (resolvedValue) {
    const inlineRef = (0, _typesSecretsBxqheYvy.o)(resolvedValue, params.refDefaults);
    if (inlineRef) try {
      resolvedValue = await (0, _resolveBHS7vHp.i)(inlineRef, {
        config: params.configForRefResolution,
        env: process.env,
        cache: params.cache
      });
    } catch (err) {
      _storeA4exFSck.L.debug(params.inlineFailureMessage, {
        profileId: params.profileId,
        provider: params.provider,
        error: (0, _errorsVfATXfah.i)(err)
      });
  ...
  const explicitRef = (0, _typesSecretsBxqheYvy.o)(params.valueRef, params.refDefaults);
  if (!resolvedValue && explicitRef) try {
    resolvedValue = await (0, _resolveBHS7vHp.i)(explicitRef, {
      config: params.configForRefResolution,
      env: process.env,
      cache: params.cache
    });
  } catch (err) {
    _storeA4exFSck.L.debug(params.refFailureMessage, {
      profileId: params.profileId,
      provider: params.provider,
      error: (0, _errorsVfATXfah.i)(err)
    });
  }
  return (0, _normalizeSecretInputCrCOUFln.t)(resolvedValue);
}
async function resolveApiKeyForProfile(params) {
  ...
  const configForRefResolution = cfg ?? (0, _io5xE1dPMK.i)();
  ...
  if (cred.type === "token") {
    const expiryState = (0, _storeA4exFSck.O)(cred.expires);
  ...
  try {
    ...
    const refreshedStore = error instanceof OAuthManagerRefreshError ? error.getRefreshedStore() : (0, _storeA4exFSck.c)(params.agentDir);
    ...
    const fallbackProfileId = (0, _repairCzDiEiN.n)({
      cfg,
