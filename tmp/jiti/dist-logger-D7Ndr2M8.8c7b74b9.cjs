"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.C = setYes;exports.S = setVerbose;exports._ = levelToMinLevel;exports.a = getResolvedLoggerSettings;exports.b = isVerbose;exports.c = setLoggerConfigLoaderForTests;exports.d = toPinoLikeLogger;exports.f = formatLocalIsoWithOffset;exports.g = void 0;exports.h = resolveEnvLogLevelOverride;exports.i = getLogger;exports.l = setLoggerOverride;exports.m = isValidTimeZone;exports.n = void 0;exports.o = isFileLogLevelEnabled;exports.p = formatTimestamp;exports.r = getChildLogger;exports.s = resetLogger;exports.u = exports.t = void 0;exports.v = normalizeLogLevel;exports.x = isYes;exports.y = tryParseLogLevel;var _stringCoerceLndEvhRk = require("./string-coerce-LndEvhRk.js");
...
var _diagnosticEventsDQZruzVY = require("./diagnostic-events-DQZruzVY.js");
var _tmpOpenclawDirDJsd5QL = require("./tmp-openclaw-dir-DJsd5QL4.js");
...
function resolveEnvLogLevelOverride() {
  const trimmed = (0, _stringCoerceLndEvhRk.c)(process.env.OPENCLAW_LOG_LEVEL) ?? "";
  if (!trimmed) {
    _stateCiZRzmxP.t.invalidEnvLogLevelValue = null;
    return;
  }
  ...
  if (_stateCiZRzmxP.t.invalidEnvLogLevelValue !== trimmed) {
    _stateCiZRzmxP.t.invalidEnvLogLevelValue = trimmed;
...
function resolveDefaultLogDir() {
  return canUseNodeFs() ? (0, _tmpOpenclawDirDJsd5QL.n)() : _tmpOpenclawDirDJsd5QL.t;
}
function resolveDefaultLogFile(defaultLogDir) {
  return canUseNodeFs() ? _nodePath.default.join(defaultLogDir, "openclaw.log") : `${_tmpOpenclawDirDJsd5QL.t}/openclaw.log`;
}
...
const loadLoggerConfigDefault = () => (0, _redactCxqQQvmK.f)();
...
function setLoggerConfigLoaderForTests(loader) {
  ...
  _stateCiZRzmxP.t.cachedLogger = null;
  _stateCiZRzmxP.t.cachedSettings = null;
}
...
function sanitizeDiagnosticLogText(value, maxChars) {
  return clampDiagnosticLogText((0, _redactCxqQQvmK.s)(clampDiagnosticLogText(value, maxChars)), maxChars);
}
...
function assignDiagnosticLogAttribute(attributes, state, key, value) {
  ...
  if ((0, _prototypeKeysCxs5UffD.t)(normalizedKey)) return;
  if ((0, _redactCxqQQvmK.s)(normalizedKey) !== normalizedKey) return;
...
function normalizeTraceContext(value) {
  ...
  if (!(0, _diagnosticEventsDQZruzVY.b)(candidate.traceId)) return;
  if (candidate.spanId !== void 0 && !(0, _diagnosticEventsDQZruzVY.v)(candidate.spanId)) return;
  if (candidate.parentSpanId !== void 0 && !(0, _diagnosticEventsDQZruzVY.v)(candidate.parentSpanId)) return;
  if (candidate.traceFlags !== void 0 && !(0, _diagnosticEventsDQZruzVY.y)(candidate.traceFlags)) return;
  return {
...
function buildTraceFileLogFields(logObj) {
  ...
  const trace = findLogTraceContext(bindings, args) ?? (0, _diagnosticEventsDQZruzVY._)();
  if (!trace) return;
  return {
...
function buildDiagnosticLogRecord(logObj) {
  ...
  const trace = findLogTraceContext(bindings, numericArgs) ?? (0, _diagnosticEventsDQZruzVY._)();
...
function isLogRedactionDisabled() {
  return (0, _redactCxqQQvmK.f)()?.redactSensitive === "off";
}
function redactLogRecordForTransport(record) {
  return isLogRedactionDisabled() ? record : (0, _redactCxqQQvmK.r)(record);
}
function attachDiagnosticEventTransport(logger) {
  logger.attachTransport((logObj) => {
    try {
      (0, _diagnosticEventsDQZruzVY.n)(buildDiagnosticLogRecord(redactLogRecordForTransport(logObj)));
    } catch {}
  });
}
function canUseSilentVitestFileLogFastPath(envLevel) {
  return process.env.VITEST === "true" && process.env.OPENCLAW_TEST_FILE_LOG !== "1" && !envLevel && !_stateCiZRzmxP.t.overrideSettings;
}
function resolveSettings() {
  ...
  const cfg = _stateCiZRzmxP.t.overrideSettings ?? loadLoggerConfig();
...
function isFileLogLevelEnabled(level) {
  const settings = _stateCiZRzmxP.t.cachedSettings ?? resolveSettings();
  if (!_stateCiZRzmxP.t.cachedSettings) _stateCiZRzmxP.t.cachedSettings = settings;
...
function buildLogger(settings) {
  ...
  logger.attachTransport((logObj) => {
    try {
      ...
      const payload = `${(0, _redactCxqQQvmK.s)(JSON.stringify(redactLogRecordForTransport(record)))}\n`;
...
function getLogger() {
  ...
  const cachedLogger = _stateCiZRzmxP.t.cachedLogger;
  const cachedSettings = _stateCiZRzmxP.t.cachedSettings;
  if (!cachedLogger || settingsChanged(cachedSettings, settings)) {
    _stateCiZRzmxP.t.cachedLogger = buildLogger(settings);
    _stateCiZRzmxP.t.cachedSettings = settings;
  }
  return _stateCiZRzmxP.t.cachedLogger;
}
...
function setLoggerOverride(settings) {
  _stateCiZRzmxP.t.overrideSettings = settings;
  _stateCiZRzmxP.t.cachedLogger = null;
  _stateCiZRzmxP.t.cachedSettings = null;
  _stateCiZRzmxP.t.cachedConsoleSettings = null;
}
function resetLogger() {
  _stateCiZRzmxP.t.cachedLogger = null;
  _stateCiZRzmxP.t.cachedSettings = null;
  _stateCiZRzmxP.t.cachedConsoleSettings = null;
  _stateCiZRzmxP.t.overrideSettings = null;
  loadLoggerConfig = loadLoggerConfigDefault;
}
const testApi = exports.u = {
  resolveActiveLogFile,
  shouldSkipMutatingLoggingConfigRead: _redactCxqQQvmK.p
};
...
function resolveActiveLogFile(file) {
  const expandedFile = (0, _homeDirIZwpu.t)(file);
