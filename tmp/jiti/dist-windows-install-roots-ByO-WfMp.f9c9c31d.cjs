"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.n = getWindowsProgramFilesRoots;exports.t = getWindowsInstallRoots;var _stringCoerceLndEvhRk = require("./string-coerce-LndEvhRk.js");
...
function defaultIsReadableFile(filePath) {
  try {
    _nodeFs.default.accessSync(filePath, _nodeFs.default.constants.R_OK);
    return true;
  } catch {
    return false;
  }
...
function trimTrailingSeparators(value) {
  const parsed = _nodePath.default.win32.parse(value);
  let trimmed = value;
...
function normalizeWindowsInstallRoot(raw) {
  ...
  const normalized = trimTrailingSeparators(_nodePath.default.win32.normalize(trimmed));
  if (!_nodePath.default.win32.isAbsolute(normalized) || normalized.startsWith("\\\\")) return null;
  const parsed = _nodePath.default.win32.parse(normalized);
...
function getWindowsRegExeCandidates() {
  return [_nodePath.default.win32.join(DEFAULT_WINDOWS_SYSTEM_ROOT, "System32", "reg.exe")];
}
...
function runRegQuery(regExe, key, valueName, use64BitView) {
  ...
  return (0, _nodeChild_process.execFileSync)(regExe, args, {
    encoding: "utf8",
    stdio: [
    "ignore",
    "pipe",
...
function getWindowsProgramFilesRoots(env = process.env) {
  ...
  {
    if (!value) continue;
    const key = (0, _stringCoerceLndEvhRk.a)(value);
