"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.a = resolvePluginSourceRoots;exports.c = readLegacyNpmPluginDeclaration;exports.d = normalizeBundledLookupPath;exports.f = shouldRejectHardlinkedPluginFiles;exports.i = resolvePluginCacheInputs;exports.l = isBundledSourceOverlayPath;exports.n = buildPluginDependencyStatus;exports.o = tracePluginLifecyclePhase;exports.r = normalizePluginDependencySpecs;exports.s = tracePluginLifecyclePhaseAsync;exports.t = discoverOpenClawPlugins;exports.u = buildBundledPluginLoadPathAliases;var _stringCoerceLndEvhRk = require("./string-coerce-LndEvhRk.js");
...
var _npmRegistrySpecBT2PTNSZ = require("./npm-registry-spec-BT2PTNSZ.js");
var _manifestCkPySoxh = require("./manifest-CkPySoxh.js");
var _packageEntryResolutionCLCACcAx = require("./package-entry-resolution-CLCACcAx.js");
...
function isNixStorePluginRoot(rootDir, realpathCache) {
  const rootRealPath = (0, _pathB5B_oAT.d)(rootDir, realpathCache) ?? _nodePath.default.resolve(rootDir);
...
function shouldRejectHardlinkedPluginFiles(params) {
  ...
  if ((0, _pathsCnwfh6dH.f)(params.env) && isNixStorePluginRoot(params.rootDir, params.realpathCache)) return false;
  return true;
}
//#endregion
...
function isSameOrInside(baseDir, targetPath) {
  ...
  return target === base || (0, _pathB5B_oAT.i)(base, target);
}
...
function sourceOverlaysDisabled(env) {
  const raw = (0, _stringCoerceLndEvhRk.s)(env.OPENCLAW_DISABLE_BUNDLED_SOURCE_OVERLAYS);
...
function readLegacyNpmPluginDeclaration(pluginDir) {
  const source = _nodePath.default.join(pluginDir, LEGACY_NPM_DECLARATION_FILE);
  const parsed = (0, _jsonFiles1SmAauRO.u)(source);
  ...
  if (!pluginId || (0, _installPathsBm0q0NPS.c)(pluginId) || !(0, _npmRegistrySpecBT2PTNSZ.s)(npmSpec)) return null;
  return {
    pluginId,
    npmSpec,
    source
...
function resolvePluginSourceRoots(params) {
  ...
  const workspaceRoot = params.workspaceDir ? (0, _utilsCKsuXgDI.p)(params.workspaceDir, env) : void 0;
  return {
    stock: (0, _bundledDirBocaljIz.n)(env),
    global: _nodePath.default.join((0, _utilsCKsuXgDI.d)(env), "extensions"),
...
function resolvePluginCacheInputs(params) {
  ...
  return {
    ...
    loadPaths: (params.loadPaths ?? []).filter((entry) => typeof entry === "string").map((entry) => entry.trim()).filter(Boolean).map((entry) => (0, _utilsCKsuXgDI.p)(entry, env))
  };
}
//#endregion
...
function checkSourceEscapesRoot(params) {
  const sourceRealPath = (0, _pathB5B_oAT.d)(params.source, params.realpathCache);
  const rootRealPath = (0, _pathB5B_oAT.d)(params.rootDir, params.realpathCache);
  if (!sourceRealPath || !rootRealPath) return null;
  if ((0, _pathB5B_oAT.i)(rootRealPath, sourceRealPath)) return null;
  return {
...
function checkPathStatAndPermissions(params) {
  ...
  for (const targetPath of pathsToCheck) {
    ...
    seen.add(normalized);
    let stat = (0, _pathB5B_oAT.f)(targetPath);
    if (!stat) return {
    ...
    if ((modeBits & 2) !== 0 && params.origin === "bundled") try {
      ...
      const repairedStat = (0, _pathB5B_oAT.f)(targetPath);
...
function isExtensionFile(filePath) {
  ...
  const baseName = (0, _stringCoerceLndEvhRk.a)(_nodePath.default.basename(filePath));
...
function shouldIgnoreScannedDirectory(dirName) {
  const normalized = (0, _stringCoerceLndEvhRk.a)(dirName);
...
function resolveScannedEntryType(entry, fullPath) {
  ...
  const stat = (0, _pathB5B_oAT.f)(fullPath);
  if (!stat) return null;
...
function resolvesToSameDirectory(left, right, realpathCache) {
  ...
  const leftRealPath = (0, _pathB5B_oAT.d)(left, realpathCache);
  const rightRealPath = (0, _pathB5B_oAT.d)(right, realpathCache);
...
function resolveManagedPluginDirKey(installedPath, realpathCache) {
  const stat = (0, _pathB5B_oAT.f)(installedPath);
  if (!stat) return null;
  const pluginDir = stat.isFile() ? _nodePath.default.dirname(installedPath) : installedPath;
  return (0, _pathB5B_oAT.d)(pluginDir, realpathCache) ?? _nodePath.default.resolve(pluginDir);
}
...
function isManagedPluginDir(params) {
  ...
  const key = params.realpath ?? (0, _pathB5B_oAT.d)(params.dir, params.realpathCache) ?? _nodePath.default.resolve(params.dir);
  return params.managedPluginDirs.has(key);
}
function readPackageManifest(dir, rejectHardlinks = true, rootRealPath) {
  const result = (0, _jsonFiles1SmAauRO.o)({
    rootDir: dir,
...
function readTrustedPackageManifest(dir) {
  return (0, _jsonFiles1SmAauRO.u)(_nodePath.default.join(dir, "package.json"));
}
...
function resolveIdHintManifestId(rootDir, rejectHardlinks, rootRealPath) {
  const manifest = (0, _manifestCkPySoxh.i)(rootDir, rejectHardlinks, rootRealPath);
...
function addCandidate(params) {
  ...
  const resolvedRoot = (0, _pathB5B_oAT.d)(params.rootDir, params.realpathCache) ?? _nodePath.default.resolve(params.rootDir);
  ...
  params.candidates.push({
    ...
    workspaceDir: params.workspaceDir,
    packageName: (0, _stringCoerceLndEvhRk.c)(manifest?.name),
    packageVersion: (0, _stringCoerceLndEvhRk.c)(manifest?.version),
    packageDescription: (0, _stringCoerceLndEvhRk.c)(manifest?.description),
    packageDir: params.packageDir,
    packageManifest: (0, _manifestCkPySoxh.r)(manifest ?? void 0),
...
function discoverBundleInRoot(params) {
  const bundleFormat = (0, _bundleManifestDpCbfxOo.i)(params.rootDir);
  if (!bundleFormat) return "none";
  const rootRealPath = (0, _pathB5B_oAT.d)(params.rootDir, params.realpathCache) ?? void 0;
  ...
  const bundleManifest = (0, _bundleManifestDpCbfxOo.a)({
    rootDir: params.rootDir,
    ...(rootRealPath !== void 0 ? { rootRealPath } : {}),
    bundleFormat,
    rejectHardlinks
...
function discoverInDirectory(params) {
  ...
  const resolvedDir = (0, _pathB5B_oAT.d)(params.dir, params.realpathCache) ?? _nodePath.default.resolve(params.dir);
  ...
  for (const entry of entries) {
    ...
    const fullPathRealPath = (0, _pathB5B_oAT.d)(fullPath, params.realpathCache) ?? void 0;
    const fullPathDirKey = fullPathRealPath ?? _nodePath.default.resolve(fullPath);
    ...
    const extensionResolution = (0, _manifestCkPySoxh.o)(manifest ?? void 0);
    ...
    const setupSource = (0, _packageEntryResolutionCLCACcAx.n)({
      packageDir: fullPath,
    ...
    if (extensions.length > 0) {
      const resolvedRuntimeSources = (0, _packageEntryResolutionCLCACcAx.t)({
        packageDir: fullPath,
        ...(fullPathRealPath !== void 0 ? { packageRootRealPath: fullPathRealPath } : {}),
        manifest,
        extensions,
    ...
    const indexFile = [..._manifestCkPySoxh.t].map((candidate) => _nodePath.default.join(fullPath, candidate)).find((candidate) => _nodeFs.default.existsSync(candidate));
...
function discoverFromPath(params) {
  const resolved = (0, _utilsCKsuXgDI.p)(params.rawPath, params.env);
  ...
  if (stat.isDirectory()) {
    const resolvedRealPath = (0, _pathB5B_oAT.d)(resolved, params.realpathCache) ?? void 0;
    const requireBuiltRuntimeEntry = params.requireBuiltRuntimeEntry ?? isManagedPluginDir({
      dir: resolved,
    ...
    const indexFile = [..._manifestCkPySoxh.t].map((candidate) => _nodePath.default.join(resolved, candidate)).find((candidate) => _nodeFs.default.existsSync(candidate));
    if (indexFile && isExtensionFile(indexFile)) {
      addCandidate({
        candidates: params.candidates,
        diagnostics: params.diagnostics,
...
function discoverOpenClawPlugins(params) {
  const env = params.env ?? process.env;
  const workspaceDir = (0, _stringCoerceLndEvhRk.c)(params.workspaceDir);
  const workspaceRoot = workspaceDir ? (0, _utilsCKsuXgDI.p)(workspaceDir, env) : void 0;
  ...
  const scopedResult = tracePluginLifecyclePhase("discovery scan", () => {
    ...
    for (const extraPath of extra) {
      ...
      const bundledAlias = resolvePackagedBundledLoadPathAlias({
        ...
        loadPath: (0, _utilsCKsuXgDI.p)(trimmed, env)
      });
      if (bundledAlias) {
  ...
  const sharedResult = tracePluginLifecyclePhase("discovery scan", () => {
    ...
    const sourceCheckoutDependencyDiagnostic = (0, _bundledDirBocaljIz.r)(env);
