"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.ProxylineNodeProxyAgent = exports.CALLER_AGENT_TLS_OPTION_KEYS = void 0;exports.bindNodeHttpMethod = bindNodeHttpMethod;exports.createAmbientNodeProxyAgent = createAmbientNodeProxyAgent;exports.createDirectNodeAgent = createDirectNodeAgent;exports.createNodeProxyAgent = createNodeProxyAgent;exports.hasAmbientNodeProxyConfigured = hasAmbientNodeProxyConfigured;var _nodeHttp = _interopRequireDefault(require("node:http"));
var _nodeHttps = _interopRequireDefault(require("node:https"));
var _nodeNet = _interopRequireDefault(require("node:net"));
var _nodeTls = _interopRequireDefault(require("node:tls"));
...
const CALLER_AGENT_TLS_OPTION_KEYS = exports.CALLER_AGENT_TLS_OPTION_KEYS = [
"ca",
"cert",
"ciphers",
"clientCertEngine",
...
function preserveDestinationTlsIdentity(url, options) {
  ...
  if (_nodeNet.default.isIP(hostname) === 0) {
...
function proxyConnectOptions(proxy, proxyTls) {
  ...
  const ca = (0, _shared.resolveProxyTlsCa)(proxyTls);
  return {
    ...base,
    ALPNProtocols: ["http/1.1"],
    ...(_nodeNet.default.isIP(host) === 0 ? { servername: host } : {}),
    ...(ca !== undefined ? { ca } : {})
  };
}
function assertSupportedNodeProxyProtocol(proxy) {
  if (proxy.protocol !== "http:" && proxy.protocol !== "https:") {
    throw new _shared.ProxylineError("UNSUPPORTED_PROXY_PROTOCOL", `Node HTTP agents support http:// and https:// proxy endpoints: ${proxy.protocol}`);
  }
}
...
function requestAuthority(options) {
  ...
  const authorityHost = _nodeNet.default.isIPv6(host) ? `[${host}]` : host;
...
function connectToProxy(proxy, proxyTls) {
  ...
  _nodeTls.default.connect(options) :
  _nodeNet.default.connect(options);
}
...
function normalizeProxyTargetHost(host) {
  ...
  if (_nodeNet.default.isIP(unbracketedHost) !== 0) {
    return unbracketedHost;
  }
  if (INVALID_PROXY_TARGET_HOST_DELIMITER_PATTERN.test(host)) {
    throw new _shared.ProxylineError("INVALID_CONNECT_TARGET", "CONNECT target host contains unsafe delimiters.");
  }
  if (INVALID_PROXY_TARGET_HOST_CONTROL_PATTERN.test(host)) {
    throw new _shared.ProxylineError("INVALID_CONNECT_TARGET", "CONNECT target host contains unsafe characters.");
  }
  const asciiHost = (0, _nodeUrl.domainToASCII)(host);
  if (!asciiHost) {
    throw new _shared.ProxylineError("INVALID_CONNECT_TARGET", "CONNECT target host is not a valid host name.");
  }
  return asciiHost;
}
function connectTarget(options) {
  ...
  if (typeof rawHost !== "string") {
    throw new _shared.ProxylineError("INVALID_CONNECT_TARGET", "CONNECT target is missing host.");
  }
  const parsed = splitHostPort(rawHost);
  const port = parsed.port ?? Number(options.port);
  if (!parsed.host || !Number.isInteger(port)) {
    throw new _shared.ProxylineError("INVALID_CONNECT_TARGET", "CONNECT target is missing host or port.");
  }
  const host = normalizeProxyTargetHost(parsed.host);
  (0, _connect.formatConnectAuthority)(host, port);
  return { host, port };
}
...
class ProxylineHttpForwardAgent extends _nodeHttp.default.Agent {
  options;
  #keepAlive;
  #proxy;
  #proxyTls;
  ...
  addRequest(req, options) {
    ...
    _nodeHttp.default.Agent.prototype.addRequest.call(this, req, options);
  }
...
class ProxylineConnectAgent extends _nodeHttp.default.Agent {
  options;
  #keepAlive;
  ...
  createConnection(options, callback) {
    ...
    if (callback === undefined) {
      throw new _shared.ProxylineError("INVALID_CONNECT_CALLBACK", "CONNECT agents require an async socket callback.");
    }
    ...
    const startPendingTimeout = (timeoutMs) => {
      ...
      pendingTimeout = setTimeout(() => {
        ...
        if (!settled) {
          fail(new _shared.ProxylineError("CONNECT_FAILED", "proxy CONNECT timed out"));
        }
      }, timeoutMs);
    ...
    const onConnected = () => {
      ...
      const authority = (0, _connect.formatConnectAuthority)(host, port);
      const headers = [
    ...
    const onData = (chunk) => {
      ...
      if (headerEnd === -1) {
        if (responseBuffer.length > MAX_CONNECT_RESPONSE_HEADER_BYTES) {
          fail(new _shared.ProxylineError("CONNECT_FAILED", "proxy CONNECT response headers were too large"));
        }
        return;
      }
      ...
      if (!/^HTTP\/1\.[01] 2\d\d\b/.test(statusLine)) {
        fail(new _shared.ProxylineError("CONNECT_FAILED", statusLine || "proxy returned an invalid CONNECT response"));
        return;
      }
      ...
      const currentTlsSocket = _nodeTls.default.connect(destinationTlsConnectOptions(options, proxySocket));
      ...
      const onTlsClosed = () => {
        finish(new _shared.ProxylineError("CONNECT_FAILED", "destination TLS socket closed before secureConnect"), currentTlsSocket);
      };
    ...
    const onClosed = () => {
      fail(new _shared.ProxylineError("CONNECT_FAILED", "proxy socket closed before CONNECT completed"));
    };
    const onRequestClosed = () => {
      if (!settled) {
        fail(new _shared.ProxylineError("CONNECT_FAILED", "request closed before proxy CONNECT completed"));
      }
    };
...
class ProxylineNodeProxyAgent extends _nodeHttp.default.Agent {
  options;
  #agents = new Map();
  #defaultProtocol;
  #getProxyForUrl;
  ...
  constructor(options) {
    ...
    this.#proxyTls = proxyTls;
    this.#httpAgent = new _nodeHttp.default.Agent(agentOptions);
    this.#httpsAgent = new _nodeHttps.default.Agent(agentOptions);
  }
  get defaultPort() {
...
}exports.ProxylineNodeProxyAgent = ProxylineNodeProxyAgent;
function createNodeProxyAgent(resolver, proxyCa, defaultProtocol = "http") {
...
function createDirectNodeAgent() {
  return new ProxylineNodeProxyAgent({
    getProxyForUrl: () => ""
  });
}
...
function hasAmbientNodeProxyConfigured(options = {}) {
  const env = options.env ?? (0, _env.readProxyEnv)();
  const protocol = options.protocol ?? "https";
  return (0, _env.resolveAmbientProxyForUrl)(ambientProbeUrl(protocol), env) !== undefined;
}
function createAmbientNodeProxyAgent(options = {}) {
  const env = options.env ?? (0, _env.readProxyEnv)();
  const protocol = options.protocol ?? "https";
  if ((0, _env.resolveAmbientProxyForUrl)(ambientProbeUrl(protocol), env) === undefined) {
    return undefined;
  }
  return new ProxylineNodeProxyAgent({
    defaultProtocol: protocol,
    getProxyForUrl: (url) => (0, _env.resolveAmbientProxyForUrl)(url, env) ?? "",
