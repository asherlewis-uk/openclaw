...
function shouldRemoveDeadOwnerOrExpiredLock(params) {
  ...
  if (payload?.pid) return (params.isPidDefinitelyDead ?? _pidAliveB3tYqreQ.r)(payload.pid);
...
function resetFileLockStateForTest() {
  (0, _fileLockBmUfoFn.i)(FILE_LOCK_MANAGER_KEY, FILE_LOCK_MANAGER_KEY);
}
async function drainFileLockStateForTest() {
  await (0, _fileLockBmUfoFn.r)(FILE_LOCK_MANAGER_KEY, FILE_LOCK_MANAGER_KEY);
}
/** Acquire a re-entrant process-local file lock backed by a `.lock` sidecar file. */
async function acquireFileLock(filePath, options) {
  try {
    const lock = await (0, _fileLockBmUfoFn.t)(filePath, {
