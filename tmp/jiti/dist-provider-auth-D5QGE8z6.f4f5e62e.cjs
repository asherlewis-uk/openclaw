...
require("./types.secrets-BxqheYvy.js");
...
require("./provider-env-vars-Bdegz5z9.js");
var _storeA4exFSck = require("./store-a4exFSck.js");
require("./model-auth-markers-UDEQVW7W.js");
var _modelAuthEnvCblHr3q = require("./model-auth-env-CblHr3q1.js");
require("./models-config.providers.secrets-DzUJ8-IV.js");
var _providerAttributionBJpDy8Xw = require("./provider-attribution-BJpDy8Xw.js");
var _oauthB8qSiLIG = require("./oauth-B8qSiLIG.js");
var _profileList1dJMPDMe = require("./profile-list-1dJMPDMe.js");
require("./repair-CzDi-eiN.js");
var _orderYU9choem = require("./order-YU9choem.js");
require("./profiles-Bj_dclxz.js");
require("./provider-model-shared-D-slKnZa.js");
require("./provider-auth-input-BB3v7mkT.js");
require("./provider-auth-helpers-Hp3hWZu2.js");
require("./provider-api-key-auth-DaaaGg6p.js");
require("./provider-auth-result-Bwa-OosX.js");
...
function resolveOpenClawAgentDir(env = process.env) {
  ...
  return override ? (0, _utilsCKsuXgDI.p)(override, env) : (0, _agentScopeConfig26EcJVc.s)({}, env);
}
//#endregion
...
/** @deprecated GitHub Copilot provider-owned helper; do not use from third-party plugins. */
const COPILOT_USER_AGENT = exports.a = "GitHubCopilotChat/0.26.7";
/** @deprecated GitHub Copilot provider-owned helper; do not use from third-party plugins. */
...
function deriveCopilotApiBaseUrlFromToken(token) {
  ...
  return (0, _providerAttributionBJpDy8Xw.n)(baseUrl).endpointClass === "invalid" ? null : baseUrl;
}
/** @deprecated GitHub Copilot provider-owned helper; do not use from third-party plugins. */
async function resolveCopilotApiToken(params) {
  const env = params.env ?? process.env;
  const cachePath = params.cachePath?.trim() || resolveCopilotTokenCachePath(env);
  const loadJsonFileFn = params.loadJsonFileImpl ?? _jsonFileC3OPVumq.t;
  const saveJsonFileFn = params.saveJsonFileImpl ?? _jsonFileC3OPVumq.n;
...
function isProviderApiKeyConfigured(params) {
  if ((0, _modelAuthEnvCblHr3q.t)(params.provider)?.apiKey) return true;
  const agentDir = params.agentDir?.trim();
  if (!agentDir) return false;
  return (0, _profileList1dJMPDMe.n)((0, _storeA4exFSck.n)(agentDir, { allowKeychainPrompt: false }), params.provider).length > 0;
}
function listUsableProviderAuthProfileIds(params) {
  try {
    const agentDir = params.agentDir?.trim() || (0, _agentScopeConfig26EcJVc.s)(params.cfg ?? {});
    const store = (0, _storeA4exFSck.n)(agentDir, { allowKeychainPrompt: false });
    return {
      agentDir,
      profileIds: (0, _orderYU9choem.i)({
        cfg: params.cfg,
        store,
...
async function resolveProviderAuthProfileApiKey(params) {
  ...
  for (const profileId of profileIds) {
    const resolved = await (0, _oauthB8qSiLIG.n)({
      cfg: params.cfg,
      store,
      agentDir,
      profileId
