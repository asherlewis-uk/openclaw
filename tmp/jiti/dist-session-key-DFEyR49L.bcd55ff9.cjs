...
var _accountIdCwBWagLE = require("./account-id-CwBWagLE.js");
...
function scopedHeartbeatWakeOptions(sessionKey, wakeOptions, mainKey, scope) {
  const parsed = (0, _sessionKeyUtilsQDNZHCY.o)(sessionKey);
  if (!parsed) return wakeOptions;
  if ((0, _sessionKeyUtilsQDNZHCY.r)(sessionKey)) {
...
function resolveEventSessionKey(sessionKey, mainKey, scope) {
  const parsed = (0, _sessionKeyUtilsQDNZHCY.o)(sessionKey);
  if (!parsed || !(0, _sessionKeyUtilsQDNZHCY.r)(sessionKey)) return sessionKey;
...
function toAgentRequestSessionKey(storeKey) {
  ...
  if (!raw) return;
  return (0, _sessionKeyUtilsQDNZHCY.o)(raw)?.rest ?? raw;
}
...
function resolveAgentIdFromSessionKey(sessionKey) {
  return normalizeAgentId((0, _sessionKeyUtilsQDNZHCY.o)(sessionKey)?.agentId ?? "main");
}
function classifySessionKeyShape(sessionKey) {
  const raw = (sessionKey ?? "").trim();
  if (!raw) return "missing";
  if ((0, _sessionKeyUtilsQDNZHCY.o)(raw)) return "agent";
...
function buildAgentPeerSessionKey(params) {
  ...
  if (peerKind === "direct") {
    ...
    if (dmScope === "per-account-channel-peer" && peerId) {
      ...
      const accountId = (0, _accountIdCwBWagLE.n)(params.accountId);
  ...
  const channel = (0, _stringCoerceLndEvhRk.a)(params.channel) || "unknown";
  const peerId = (0, _stringCoerceLndEvhRk.a)(params.peerId) || "unknown";
