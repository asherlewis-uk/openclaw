...
function resolveDefaultHomeDir() {
  return (0, _homeDirIZwpu.o)(process.env, _nodeOs.default.homedir);
}
/** Build a homedir thunk that respects OPENCLAW_HOME for the given env. */
function envHomedir(env) {
  return () => (0, _homeDirIZwpu.o)(env, _nodeOs.default.homedir);
}
...
function resolveStateDir(env = process.env, homedir = envHomedir(env)) {
  const effectiveHomedir = () => (0, _homeDirIZwpu.o)(env, homedir);
...
function resolveIncludeRoots(env = process.env, homedir = envHomedir(env)) {
  ...
  for (const entry of raw.split(_nodePath.default.delimiter)) {
    ...
    if (!trimmed) continue;
    const resolved = _nodePath.default.resolve((0, _homeDirIZwpu.r)(trimmed, {
      env,
