...
function resolveExecutionConfig(value, fallback) {
  const record = (0, _recordCoerceCZkSQL9n.n)(value);
...
function resolveMemoryDreamingPluginId(cfg) {
  const configuredSlot = normalizeTrimmedString((0, _recordCoerceCZkSQL9n.n)((0, _recordCoerceCZkSQL9n.n)((0, _recordCoerceCZkSQL9n.n)(cfg)?.plugins)?.slots)?.memory);
...
function resolveMemoryDreamingPluginConfig(cfg) {
  const entries = (0, _recordCoerceCZkSQL9n.n)((0, _recordCoerceCZkSQL9n.n)((0, _recordCoerceCZkSQL9n.n)(cfg)?.plugins)?.entries);
  const pluginId = resolveMemoryDreamingPluginId(cfg);
  return (0, _recordCoerceCZkSQL9n.n)((0, _recordCoerceCZkSQL9n.n)(entries?.[pluginId])?.config) ?? void 0;
}
...
function resolveMemoryDreamingConfig(params) {
  const dreaming = (0, _recordCoerceCZkSQL9n.n)(params.pluginConfig?.dreaming);
  ...
  const storage = (0, _recordCoerceCZkSQL9n.n)(dreaming?.storage);
  const execution = (0, _recordCoerceCZkSQL9n.n)(dreaming?.execution);
  const phases = (0, _recordCoerceCZkSQL9n.n)(dreaming?.phases);
  ...
  const light = (0, _recordCoerceCZkSQL9n.n)(phases?.light);
  const deep = (0, _recordCoerceCZkSQL9n.n)(phases?.deep);
  const rem = (0, _recordCoerceCZkSQL9n.n)(phases?.rem);
  const deepRecovery = (0, _recordCoerceCZkSQL9n.n)(deep?.recovery);
...
function resolveMemoryDreamingWorkspaces(cfg, options = {}) {
  ...
  if (agentIds.length === 0) agentIds.push((0, _agentScopeConfig26EcJVc.c)(cfg));
  const byWorkspace = /* @__PURE__ */new Map();
  const addWorkspace = (workspaceDirRaw, agentIdRaw) => {
    const workspaceDir = workspaceDirRaw?.trim();
    if (!workspaceDir) return;
    const agentId = (0, _stringCoerceLndEvhRk.s)(agentIdRaw) || (0, _agentScopeConfig26EcJVc.c)(cfg);
  ...
  for (const agentId of agentIds) addWorkspace((0, _agentScopeConfig26EcJVc.o)(cfg, agentId), agentId);
  addWorkspace(options.primaryWorkspaceDir ?? void 0, options.primaryAgentId ?? (0, _agentScopeConfig26EcJVc.c)(cfg));
  return [...byWorkspace.values()];
}
//#endregion /* v9-2fad5d44da57e63b */
