"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.a = emitDiagnosticsTimelineEvent;exports.c = measureDiagnosticsTimelineSpanSync;exports.i = loadPluginMetadataSnapshot;exports.n = isPluginMetadataSnapshotCompatible;exports.o = isDiagnosticsTimelineEnabled;exports.r = listPluginOriginsFromMetadataSnapshot;exports.s = measureDiagnosticsTimelineSpan;exports.t = clearLoadPluginMetadataSnapshotMemo;var _env2YgvpOLV = require("./env-2YgvpOLV.js");
...
function isDiagnosticsTimelineEnabled(options = {}) {
  ...
  return ((0, _diagnosticFlagsBAYJHjtr.t)("timeline", config, env) || (0, _diagnosticFlagsBAYJHjtr.t)("diagnostics.timeline", config, env) || (0, _env2YgvpOLV.t)(env.OPENCLAW_DIAGNOSTICS)) && typeof env.OPENCLAW_DIAGNOSTICS_TIMELINE_PATH === "string" && env.OPENCLAW_DIAGNOSTICS_TIMELINE_PATH.trim().length > 0;
}
...
function fileFingerprint(filePath) {
  try {
    const stat = _nodeFs.default.statSync(filePath, { bigint: true });
    return [
    filePath,
...
function isPathInsideOrEqual(childPath, parentPath) {
  const relative = _nodePath.default.relative(parentPath, childPath);
...
function resolvePluginFilePath(pluginDir, filePath, options = {}) {
  ...
  const rootDir = _nodePath.default.resolve(pluginDir);
  const resolved = _nodePath.default.isAbsolute(filePath) ? _nodePath.default.resolve(filePath) : _nodePath.default.resolve(rootDir, filePath);
...
function resolveInstallRecordPath(value, env) {
  ...
  return normalized ? (0, _utilsCKsuXgDI.p)(normalized, env) : void 0;
}
function installRecordPathFingerprints(env, records, watchedFiles) {
  ...
  return Object.entries(records).toSorted(([left], [right]) => left.localeCompare(right)).map(([pluginId, rawRecord]) => {
    ...
    installPath,
    sourcePath,
    watchedFileFingerprint(resolvedInstallPath ? _nodePath.default.join(resolvedInstallPath, "package.json") : void 0, watchedFiles),
    watchedFileFingerprint(resolvedInstallPath ? _nodePath.default.join(resolvedInstallPath, "openclaw.plugin.json") : void 0, watchedFiles),
    watchedFileFingerprint(resolvedSourcePath, watchedFiles),
    watchedFileFingerprint(resolvedSourcePath ? _nodePath.default.join(resolvedSourcePath, "package.json") : void 0, watchedFiles),
    watchedFileFingerprint(resolvedSourcePath ? _nodePath.default.join(resolvedSourcePath, "openclaw.plugin.json") : void 0, watchedFiles)];

  });
}
function managedNpmDependencyMetadataFingerprints(npmRoot, watchedFiles) {
  const rootManifest = readJsonObject(_nodePath.default.join(npmRoot, "package.json"));
  const dependencies = isRecord(rootManifest?.dependencies) ? rootManifest.dependencies : {};
  const nodeModulesRoot = _nodePath.default.join(npmRoot, "node_modules");
  return Object.entries(dependencies).toSorted(([left], [right]) => left.localeCompare(right)).map(([packageName, rawSpec]) => {
    ...
    const packageDir = _nodePath.default.resolve(nodeModulesRoot, packageName);
    if (!isPathInsideOrEqual(packageDir, _nodePath.default.resolve(nodeModulesRoot))) return [
    packageName,
    dependencySpec,
    ...
    packageName,
    dependencySpec,
    watchedFileFingerprint(_nodePath.default.join(packageDir, "package.json"), watchedFiles),
    watchedFileFingerprint(_nodePath.default.join(packageDir, "openclaw.plugin.json"), watchedFiles)];
...
function resolvePersistedRegistryFastMemoFingerprint(params) {
  ...
  const indexPath = (0, _installedPluginIndexRecordReaderDR3h__QQ.a)({
    env: params.env,
    ...(params.stateDir ? { stateDir: params.stateDir } : {})
  });
  const npmRoot = params.stateDir ? _nodePath.default.join(params.stateDir, "npm") : (0, _installPathsBm0q0NPS.a)(params.env);
  return {
    index: fileFingerprint(indexPath),
    npmPackageJson: fileFingerprint(_nodePath.default.join(npmRoot, "package.json"))
  };
}
function resolvePersistedRegistryMemoContextHash(params) {
  return (0, _installedPluginIndexStoreDEo2ZAhx.b)({
...
function hashWatchedFiles(watchedFiles) {
  return (0, _installedPluginIndexStoreDEo2ZAhx.b)(watchedFiles.map((filePath) => fileFingerprint(filePath)));
}
function resolvePersistedRegistryMemoState(params) {
  ...
  const fastHash = (0, _installedPluginIndexStoreDEo2ZAhx.b)(fastFingerprint);
  ...
  if (isRecord(fastFingerprint) && fastFingerprint.disabled === true) return {
    ...
    watchedFiles: [],
    watchedFilesHash: (0, _installedPluginIndexStoreDEo2ZAhx.b)([])
  };
  const indexPath = (0, _installedPluginIndexRecordReaderDR3h__QQ.a)({
    env: params.env,
    ...(params.stateDir ? { stateDir: params.stateDir } : {})
  ...
  const installRecords = params.index?.installRecords ?? (0, _installedPluginIndexRecordReaderDR3h__QQ.n)({
    env: params.env,
    ...(params.stateDir ? { stateDir: params.stateDir } : {})
  });
  ...
  return {
    ...
    fingerprint: {
      ...fastFingerprint,
      indexHash: (0, _installedPluginIndexStoreDEo2ZAhx.b)(stableMemoValue(index) ?? null),
      installRecords: (0, _installedPluginIndexStoreDEo2ZAhx.b)(stableMemoValue(installRecords)),
      installRecordFiles,
...
function computePluginMetadataSnapshotMemoKey(params) {
  ...
  const indexFingerprint = snapshotParams.index ? (0, _manifestRegistryInstalledCRoA7rSF.n)(snapshotParams.index) : void 0;
  return (0, _installedPluginIndexStoreDEo2ZAhx.b)({
    controlPlane: (0, _pluginRegistryBZohWtpt.C)({
      config: snapshotParams.config,
      env,
      workspaceDir: snapshotParams.workspaceDir,
      policyHash: (0, _installedPluginIndexStoreDEo2ZAhx._)(snapshotParams.config),
    ...
    pathPolicy: {
      compatibilityHostVersion: (0, _versionDmRkhYua.o)(env),
      nixMode: (0, _pathsCnwfh6dH.f)(env)
    },
    preferPersisted: snapshotParams.preferPersisted ?? null,
    registry: registryState.fingerprint,
    stateDir: snapshotParams.stateDir ? (0, _utilsCKsuXgDI.p)(snapshotParams.stateDir, env) : null,
    workspaceDir: snapshotParams.workspaceDir ?? null
  });
}
function resolvePluginMetadataControlPlaneFingerprint(params) {
  return (0, _pluginRegistryBZohWtpt.C)(params);
}
function indexesMatch(left, right) {
  if (!left || !right) return true;
  return (0, _manifestRegistryInstalledCRoA7rSF.n)(left) === (0, _manifestRegistryInstalledCRoA7rSF.n)(right);
}
...
function isPluginMetadataSnapshotCompatible(params) {
  ...
  return params.snapshot.policyHash === (0, _installedPluginIndexStoreDEo2ZAhx._)(params.config) && (!params.snapshot.configFingerprint || params.snapshot.configFingerprint === resolvePluginMetadataControlPlaneFingerprint({
...
function loadPluginMetadataSnapshot(params) {
  ...
  const registryState = resolvePersistedRegistryMemoStateForLookup({
    env,
    ...(params.stateDir ? { stateDir: (0, _utilsCKsuXgDI.p)(params.stateDir, env) } : {}),
...
function loadPluginMetadataSnapshotImpl(params) {
  ...
  const registryResult = (0, _pluginRegistryBZohWtpt.m)({
    config: params.config,
    workspaceDir: params.workspaceDir,
    ...(params.stateDir ? { stateDir: params.stateDir } : {}),
    env: params.env,
  ...
  const manifestRegistry = index.plugins.length === 0 ? (0, _manifestRegistryDbnX5kdP.t)({
    config: params.config,
    workspaceDir: params.workspaceDir,
    env: params.env,
    diagnostics: [...index.diagnostics],
    installRecords: index.installRecords
  }) : (0, _manifestRegistryInstalledCRoA7rSF.t)({
    index,
    config: params.config,
    workspaceDir: params.workspaceDir,
    env: params.env,
  ...
  const normalizePluginId = (0, _pluginRegistryBZohWtpt.x)(index, { manifestRegistry });
