...
function resolveAgentSessionsDir(agentId, env = process.env, homedir = () => (0, _homeDirIZwpu.o)(env, _nodeOs.default.homedir)) {
  ...
  const id = (0, _sessionKeyDFEyR49L.c)(agentId ?? "main");
...
function resolveSessionTranscriptsDir(env = process.env, homedir = () => (0, _homeDirIZwpu.o)(env, _nodeOs.default.homedir)) {
  return resolveAgentSessionsDir(_sessionKeyDFEyR49L.t, env, homedir);
}
...
function validateSessionId(sessionId) {
  ...
  if (!SAFE_SESSION_ID_RE.test(trimmed) || (0, _artifactsCZN5dsCJ.n)(`${trimmed}.jsonl`)) throw new Error(`Invalid session ID: ${sessionId}`);
  return trimmed;
}
...
function resolveSiblingAgentSessionsDir(baseSessionsDir, agentId) {
  ...
  return _nodePath.default.join(rootDir, "agents", (0, _sessionKeyDFEyR49L.c)(agentId), "sessions");
}
...
function resolveStructuralSessionFallbackPath(candidateAbsPath, expectedAgentId) {
  ...
  const normalizedAgentId = (0, _sessionKeyDFEyR49L.c)(agentIdPart);
  if (normalizedAgentId !== (0, _stringCoerceLndEvhRk.a)(agentIdPart)) return;
  if (normalizedAgentId !== (0, _sessionKeyDFEyR49L.c)(expectedAgentId)) return;
...
function resolveStorePath(store, opts) {
  const agentId = (0, _sessionKeyDFEyR49L.c)(opts?.agentId ?? "main");
