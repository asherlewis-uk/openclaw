...
function resolveUserPath(input, env = process.env, homedir = _nodeOs.default.homedir) {
  if (!input) return "";
  return (0, _homeDirIZwpu.r)(input, {
    env,
    homedir
  });
}
function resolveConfigDir(env = process.env, homedir = _nodeOs.default.homedir) {
  ...
  const newDir = _nodePath.default.join((0, _homeDirIZwpu.o)(env, homedir), ".openclaw");
  try {
...
function resolveHomeDir() {
  return (0, _homeDirIZwpu.n)(process.env, _nodeOs.default.homedir);
}
...
async function pathExists(targetPath) {
  return await (0, _fsSafeDpJlqO1z.T)(targetPath);
}
//#endregion /* v9-715becc384cbd97a */
