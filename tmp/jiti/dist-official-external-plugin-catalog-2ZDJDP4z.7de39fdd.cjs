"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.a = listOfficialExternalPluginCatalogEntries;exports.c = resolveOfficialExternalPluginInstall;exports.i = listOfficialExternalChannelCatalogEntries;exports.l = resolveOfficialExternalPluginLabel;exports.n = getOfficialExternalPluginCatalogEntryForPackage;exports.o = listOfficialExternalProviderCatalogEntries;exports.r = getOfficialExternalPluginCatalogManifest;exports.s = resolveOfficialExternalPluginId;exports.t = getOfficialExternalPluginCatalogEntry;var _stringCoerceLndEvhRk = require("./string-coerce-LndEvhRk.js");
...
function parseCatalogEntries(raw) {
  if (Array.isArray(raw)) return raw.filter((entry) => (0, _utilsCKsuXgDI.c)(entry));
  if (!(0, _utilsCKsuXgDI.c)(raw)) return [];
  const list = raw.entries ?? raw.packages ?? raw.plugins;
  if (!Array.isArray(list)) return [];
  return list.filter((entry) => (0, _utilsCKsuXgDI.c)(entry));
}
...
function getOfficialExternalPluginCatalogManifest(entry) {
  const manifest = entry[_legacyNamesB770VN3J.n];
  return (0, _utilsCKsuXgDI.c)(manifest) ? manifest : void 0;
}
function resolveOfficialExternalPluginId(entry) {
  const manifest = getOfficialExternalPluginCatalogManifest(entry);
  return (0, _stringCoerceLndEvhRk.c)(manifest?.plugin?.id) ?? (0, _stringCoerceLndEvhRk.c)(manifest?.channel?.id) ?? (0, _stringCoerceLndEvhRk.c)(manifest?.providers?.[0]?.id);
}
function resolveOfficialExternalPluginLookupIds(entry) {
  const manifest = getOfficialExternalPluginCatalogManifest(entry);
  return [
  (0, _stringCoerceLndEvhRk.c)(manifest?.plugin?.id),
  (0, _stringCoerceLndEvhRk.c)(manifest?.channel?.id),
  (0, _stringCoerceLndEvhRk.c)(manifest?.providers?.[0]?.id)].
  filter((value, index, all) => Boolean(value) && all.indexOf(value) === index);
}
function resolveOfficialExternalPluginLabel(entry) {
  const manifest = getOfficialExternalPluginCatalogManifest(entry);
  return (0, _stringCoerceLndEvhRk.c)(manifest?.plugin?.label) ?? (0, _stringCoerceLndEvhRk.c)(manifest?.channel?.label) ?? (0, _stringCoerceLndEvhRk.c)(manifest?.providers?.[0]?.name) ?? (0, _stringCoerceLndEvhRk.c)(entry.name) ?? resolveOfficialExternalPluginId(entry) ?? "plugin";
}
function resolveOfficialExternalPluginInstall(entry) {
  ...
  const npmSpec = (0, _stringCoerceLndEvhRk.c)(install?.npmSpec) ?? (0, _stringCoerceLndEvhRk.c)(entry.name);
...
function getOfficialExternalPluginCatalogEntryForPackage(packageName) {
  ...
  if (!normalized) return;
  return listOfficialExternalPluginCatalogEntries().find((entry) => (0, _stringCoerceLndEvhRk.c)(entry.name) === normalized);
}
//#endregion /* v9-4c095495b75aee33 */
