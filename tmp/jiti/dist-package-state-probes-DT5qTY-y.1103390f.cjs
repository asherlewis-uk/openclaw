...
function loadChannelPackageStateModule(params) {
  try {
    return (0, _bundledQ4WXUmmu.h)(params);
  } catch (error) {
    ...
    return (0, _pluginModuleLoaderCacheMuKAXPrS.n)({
      ...
      importerUrl: "file:///Users/asherlewis/.nvm/versions/node/v22.22.2/lib/node_modules/openclaw/dist/package-state-probes-DT5qTY-y.js",
      tryNative: true,
...
function listChannelPackageStateCatalog(metadataKey) {
  return (0, _channelCatalogRegistryDqw0ZSIq.t)({ origin: "bundled" }).filter((entry) => Boolean(resolveChannelPackageStateMetadata(entry, metadataKey)));
}
function resolveChannelPackageStateChecker(params) {
  ...
  try {
    const checker = loadChannelPackageStateModule({
      modulePath: (0, _bundledQ4WXUmmu.g)(params.entry.rootDir, metadata.specifier),
