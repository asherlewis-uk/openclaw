"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.i = formatAuthDoctorHint;exports.n = resolveApiKeyForProfile;exports.r = resolveEffectiveOAuthCredential;exports.t = refreshOAuthCredentialForRuntime;var _stringCoerceLndEvhRk = require("./string-coerce-LndEvhRk.js");
var _redactCxqQQvmK = require("./redact-CxqQQvmK.js");
...
var _resolveBe4XLgF = require("./resolve-Be4XLgF0.js");
...
var _storeCacheBCuMtdkq = require("./store-cache-BCuMtdkq.js");
...
var _chutesOauthBVjLgGE = require("./chutes-oauth-BVj-LgGE.js");
var _policyBDIoMZN = require("./policy-BDIoMZN5.js");
var _profilesBu_OqYpj = require("./profiles-Bu_OqYpj.js");
var _repairCwqn0mQn = require("./repair-Cwqn0mQn.js");
var _oauth = require("@earendil-works/pi-ai/oauth");
...
var OAuthManagerRefreshError = class extends Error {
  ...
  constructor(params) {
    ...
    super(`OAuth token refresh failed for ${params.credential.provider}: ${causeMessage}`, { cause: createRedactedOAuthRefreshCause(delegatedCause, secrets) });
...
function formatRedactedOAuthRefreshError(error, secrets) {
  return (0, _redactCxqQQvmK.s)(redactOAuthCredentialSecrets(formatRawErrorMessage(error), secrets));
}
...
function loadStoredOAuthRefreshStore(agentDir) {
  return (0, _storeCMBbDiib.l)(agentDir, {
...
async function loadFreshStoredOAuthCredential(params) {
  ...
  if (reloaded?.type !== "oauth" || reloaded.provider !== params.provider || !(0, _storeCMBbDiib.D)(reloaded)) return null;
...
function resolveEffectiveOAuthCredential(params) {
  ...
  if ((0, _storeCMBbDiib.D)(params.credential)) {
    _storeCacheBCuMtdkq.f.debug("resolved oauth credential from canonical local store", {
      profileId: params.profileId,
  ...
  if (!(0, _storeCMBbDiib.O)(params.credential, imported)) {
    _storeCacheBCuMtdkq.f.warn("refused external oauth bootstrap credential: identity mismatch or missing binding", {
      profileId: params.profileId,
      provider: params.credential.provider
    });
    return params.credential;
  }
  if ((0, _storeCMBbDiib.A)({
    existing: params.credential,
    imported
  })) {
    _storeCacheBCuMtdkq.f.debug("resolved oauth credential from external cli bootstrap", {
      profileId: params.profileId,
...
function createOAuthManager(adapter) {
  function adoptNewerMainOAuthCredential(params) {
    ...
    try {
      const mainCred = (0, _storeCMBbDiib.i)(void 0, { allowKeychainPrompt: false }).profiles[params.profileId];
      if (mainCred?.type === "oauth" && mainCred.provider === params.credential.provider && (0, _storeCMBbDiib.D)(mainCred) && Number.isFinite(mainCred.expires) && (!Number.isFinite(params.credential.expires) || mainCred.expires > params.credential.expires) && (0, _storeCMBbDiib.k)(params.credential, mainCred)) {
        params.store.profiles[params.profileId] = { ...mainCred };
        _storeCacheBCuMtdkq.f.info("adopted newer OAuth credentials from main agent", {
          profileId: params.profileId,
      ...
      _storeCacheBCuMtdkq.f.debug("adoptNewerMainOAuthCredential failed", {
        profileId: params.profileId,
  ...
  async function mirrorRefreshedCredentialIntoMainStore(params) {
    try {
      (0, _storeCMBbDiib.S)((0, _runtimeSnapshotsBSv4EtU.l)(void 0));
      await (0, _storeCMBbDiib.p)({
        agentDir: void 0,
        updater: (store) => {
          ...
          if (!decision.shouldMirror) {
            if (decision.reason === "identity-mismatch-or-regression") _storeCacheBCuMtdkq.f.warn("refused to mirror OAuth credential: identity mismatch or regression", { profileId: params.profileId });
            return false;
          }
          store.profiles[params.profileId] = { ...params.refreshed };
          _storeCacheBCuMtdkq.f.debug("mirrored refreshed OAuth credential to main agent store", {
            profileId: params.profileId,
      ...
      _storeCacheBCuMtdkq.f.debug("mirrorRefreshedCredentialIntoMainStore failed", {
        profileId: params.profileId,
        error: (0, _errorsIxwfrboQ.i)(err)
      });
    }
  ...
  async function doRefreshOAuthTokenWithLock(params) {
    const ownerAgentDir = (0, _storeCMBbDiib.d)(params);
    const authPath = (0, _runtimeSnapshotsBSv4EtU.l)(ownerAgentDir);
    (0, _storeCMBbDiib.S)(authPath);
    const globalRefreshLockPath = (0, _runtimeSnapshotsBSv4EtU.f)(params.provider, params.profileId);
    try {
      return await (0, _fileLockCCOJxG.o)(globalRefreshLockPath, _storeCacheBCuMtdkq.u, async () => (0, _fileLockCCOJxG.o)(authPath, _storeCacheBCuMtdkq.i, async () => {
        const store = loadStoredOAuthRefreshStore(ownerAgentDir);
        const cred = store.profiles[params.profileId];
        if (!cred || cred.type !== "oauth") return null;
        let credentialToRefresh = cred;
        if (!params.forceRefresh && (0, _storeCMBbDiib.D)(cred)) return {
        ...
        if (params.agentDir) try {
          ...
          if (mainCred?.type === "oauth" && mainCred.provider === cred.provider && (0, _storeCMBbDiib.D)(mainCred) && !params.forceRefresh && (0, _storeCMBbDiib.k)(cred, mainCred)) {
            store.profiles[params.profileId] = { ...mainCred };
            _storeCacheBCuMtdkq.f.info("adopted fresh OAuth credential from main store (under refresh lock)", {
              profileId: params.profileId,
              agentDir: params.agentDir,
              expires: new Date(mainCred.expires).toISOString()
            });
          ...
          } else if (mainCred?.type === "oauth" && mainCred.provider === cred.provider && (0, _storeCMBbDiib.D)(mainCred) && !(0, _storeCMBbDiib.k)(cred, mainCred)) _storeCacheBCuMtdkq.f.warn("refused to adopt fresh main-store OAuth credential: identity mismatch", {
            profileId: params.profileId,
            agentDir: params.agentDir
          });
        } catch (err) {
          _storeCacheBCuMtdkq.f.debug("inside-lock main-store adoption failed; proceeding to refresh", {
            profileId: params.profileId,
            error: (0, _errorsIxwfrboQ.i)(err)
          });
        }
        ...
        if (externallyManaged) if (externallyManaged.provider !== cred.provider) _storeCacheBCuMtdkq.f.warn("refused external oauth bootstrap credential: provider mismatch", {
          profileId: params.profileId,
          provider: cred.provider
        });else
        if (!(0, _storeCMBbDiib.O)(cred, externallyManaged)) _storeCacheBCuMtdkq.f.warn("refused external oauth bootstrap credential: identity mismatch or missing binding", {
          profileId: params.profileId,
          provider: cred.provider
        });else
        {
          if ((0, _storeCMBbDiib.j)(cred, externallyManaged) && !(0, _storeCMBbDiib.T)(cred, externallyManaged)) {
            store.profiles[params.profileId] = { ...externallyManaged };
            (0, _storeCMBbDiib.f)(store, ownerAgentDir);
          }
          credentialToRefresh = externallyManaged;
          if (!params.forceRefresh && (0, _storeCMBbDiib.D)(externallyManaged)) return {
        ...
        const refreshedCredentials = await withRefreshCallTimeout(`refreshOAuthCredential(${cred.provider})`, _storeCacheBCuMtdkq.l, async () => {
        ...
        (0, _storeCMBbDiib.f)(store, ownerAgentDir);
        if (ownerAgentDir) {
          if ((0, _runtimeSnapshotsBSv4EtU.l)(void 0) !== authPath) await mirrorRefreshedCredentialIntoMainStore({
            profileId: params.profileId,
  ...
  async function resolveOAuthAccess(params) {
    ...
    if (!params.forceRefresh && (0, _storeCMBbDiib.D)(effectiveCredential)) return {
    ...
    try {
      ...
      if (refreshed?.type === "oauth" && (0, _storeCMBbDiib.D)(refreshed) && canReuseOAuthCredentialAfterRefreshFailure({
        forceRefresh: params.forceRefresh,
      ...
      if (params.agentDir) try {
        const mainCred = (0, _storeCMBbDiib.i)(void 0, { allowKeychainPrompt: false }).profiles[params.profileId];
        if (mainCred?.type === "oauth" && mainCred.provider === params.credential.provider && (0, _storeCMBbDiib.D)(mainCred) && canReuseOAuthCredentialAfterRefreshFailure({
          forceRefresh: params.forceRefresh,
          attempted: effectiveCredential,
          candidate: mainCred
        }) && (0, _storeCMBbDiib.k)(params.credential, mainCred)) {
          refreshedStore.profiles[params.profileId] = { ...mainCred };
          _storeCacheBCuMtdkq.f.info("inherited fresh OAuth credentials from main agent", {
            profileId: params.profileId,
            agentDir: params.agentDir,
            expires: new Date(mainCred.expires).toISOString()
          });
      ...
      if (fallback && fallback.provider === params.credential.provider && (0, _storeCMBbDiib.D)(fallback) && (0, _storeCMBbDiib.E)(params.credential, fallback) && canReuseOAuthCredentialAfterRefreshFailure({
        forceRefresh: params.forceRefresh,
        attempted: effectiveCredential,
        candidate: fallback
      })) {
        _storeCacheBCuMtdkq.f.info("using external OAuth credential after refresh failure", {
          profileId: params.profileId,
...
function extractErrorMessage(error) {
  return (0, _errorsIxwfrboQ.i)(error);
}
function isRefreshTokenReusedError(error) {
  const message = (0, _stringCoerceLndEvhRk.a)(extractErrorMessage(error));
...
async function refreshOAuthCredential(credential) {
  ...
  if (credential.provider === "chutes") return await (0, _chutesOauthBVjLgGE.a)({ credential });
...
const oauthManager = createOAuthManager({
  ...
  readBootstrapCredential: ({ profileId, credential }) => (0, _storeCMBbDiib.w)({
    profileId,
    credential
  }),
  readFallbackCredential: ({ profileId, credential }) => credential.provider === "openai-codex" ? (0, _storeCMBbDiib.C)({
    profileId,
    credential,
...
async function resolveProfileSecretString(params) {
  ...
  if (resolvedValue) {
    const inlineRef = (0, _typesSecretsCJS3n8Im.o)(resolvedValue, params.refDefaults);
    if (inlineRef) try {
      resolvedValue = await (0, _resolveBe4XLgF.i)(inlineRef, {
        config: params.configForRefResolution,
        env: process.env,
        cache: params.cache
      });
    } catch (err) {
      _storeCacheBCuMtdkq.f.debug(params.inlineFailureMessage, {
        profileId: params.profileId,
        provider: params.provider,
        error: (0, _errorsIxwfrboQ.i)(err)
      });
  ...
  const explicitRef = (0, _typesSecretsCJS3n8Im.o)(params.valueRef, params.refDefaults);
  if (!resolvedValue && explicitRef) try {
    resolvedValue = await (0, _resolveBe4XLgF.i)(explicitRef, {
      config: params.configForRefResolution,
      env: process.env,
      cache: params.cache
    });
  } catch (err) {
    _storeCacheBCuMtdkq.f.debug(params.refFailureMessage, {
      profileId: params.profileId,
      provider: params.provider,
      error: (0, _errorsIxwfrboQ.i)(err)
    });
  }
  return (0, _normalizeSecretInputCH0hjbpb.t)(resolvedValue);
}
async function resolveApiKeyForProfile(params) {
  ...
  const configForRefResolution = cfg ?? (0, _ioCiCdMMvQ.i)();
  ...
  if (cred.type === "token") {
    const expiryState = (0, _storeCMBbDiib.F)(cred.expires);
  ...
  try {
    ...
    let refreshedStore = error instanceof OAuthManagerRefreshError ? error.getRefreshedStore() : (0, _storeCMBbDiib.c)(params.agentDir);
    ...
    if (isRefreshTokenReusedError(surfacedCause)) {
      const ownerAgentDir = (0, _storeCMBbDiib.d)({
        agentDir: params.agentDir,
        profileId
      });
      await (0, _profilesBu_OqYpj.t)({
        provider: cred.provider,
        profileId,
        agentDir: ownerAgentDir
      });
      if (params.agentDir !== ownerAgentDir && (0, _runtimeSnapshotsBSv4EtU.i)(params.agentDir)) {
        const snapshot = (0, _runtimeSnapshotsBSv4EtU.n)(params.agentDir);
        const providerKey = (0, _providerAuthAliasesDEhinO0g.r)(cred.provider);
        if (snapshot?.lastGood?.[providerKey] === profileId) {
          ...
          (0, _runtimeSnapshotsBSv4EtU.o)(snapshot, params.agentDir);
        }
      }
      refreshedStore = (0, _storeCMBbDiib.c)(params.agentDir);
    }
    const fallbackProfileId = (0, _repairCwqn0mQn.n)({
      cfg,
