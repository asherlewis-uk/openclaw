"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.a = resolvePluginSetupRegistry;exports.i = resolvePluginSetupProvider;exports.n = resolvePluginSetupAutoEnableReasons;exports.o = runPluginSetupConfigMigrations;exports.r = resolvePluginSetupCliBackend;exports.s = setPluginSetupRegistryModuleLoaderFactoryForTest;exports.t = clearPluginSetupRegistryCache;var _providerIdCz7K6wgK = require("./provider-id-Cz7K6wgK.js");
...
var _configContractsBLQjSeyt = require("./config-contracts-BLQjSeyt.js");
...
const CURRENT_MODULE_PATH = (0, _nodeUrl.fileURLToPath)("file:///Users/asherlewis/.nvm/versions/node/v22.22.2/lib/node_modules/openclaw/dist/setup-registry-BGTB8int.js");
...
function getModuleLoader(modulePath) {
  return (0, _pluginModuleLoaderCacheCXUDv7JF.n)({
    cache: moduleLoaders,
    modulePath,
    importerUrl: "file:///Users/asherlewis/.nvm/versions/node/v22.22.2/lib/node_modules/openclaw/dist/setup-registry-BGTB8int.js",
...
function resolveSetupApiPath(rootDir, options) {
  ...
  const repoRootCandidates = [_nodePath.default.resolve(_nodePath.default.dirname(CURRENT_MODULE_PATH), "..", "..")];
...
function resolveRelevantSetupMigrationPluginIds(params) {
  ...
  for (const plugin of registry.plugins) {
    ...
    if (paths.some((pathPattern) => (0, _configContractsBLQjSeyt.t)({
      root: params.config,
      pathPattern
...
function findUniqueSetupManifestOwner(params) {
  const matches = params.registry.plugins.filter((entry) => params.listIds(entry).some((id) => (0, _providerIdCz7K6wgK.r)(id) === params.normalizedId));
...
function mapNormalizedIds(ids) {
  ...
  for (const id of ids) {
    const normalized = (0, _providerIdCz7K6wgK.r)(id);
    ...
    const api = buildSetupPluginApi({
      ...
      handlers: {
        registerProvider(provider) {
          const key = `${record.id}:${(0, _providerIdCz7K6wgK.r)(provider.id)}`;
        ...
        registerCliBackend(backend) {
          const key = `${record.id}:${(0, _providerIdCz7K6wgK.r)(backend.id)}`;
...
function resolvePluginSetupProvider(params) {
  ...
  const api = buildSetupPluginApi({
    ...
    handlers: {
      registerProvider(provider) {
        const key = (0, _providerIdCz7K6wgK.r)(provider.id);
...
function resolvePluginSetupCliBackend(params) {
  const normalized = (0, _providerIdCz7K6wgK.r)(params.backend);
  const env = params.env ?? process.env;
  const record = findUniqueSetupManifestOwner({
    registry: loadSetupManifestRegistry({
      config: params.config,
  ...
  const api = buildSetupPluginApi({
    ...
    handlers: {
      ...
      registerAutoEnableProbe() {},
      registerCliBackend(backend) {
        const key = (0, _providerIdCz7K6wgK.r)(backend.id);
