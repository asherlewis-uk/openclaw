"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.a = readWindowsProcessArgsSync;exports.i = readWindowsListeningPidsOnPortSync;exports.n = findGatewayPidsOnPortSync;exports.r = getSelfAndAncestorPidsSync;exports.t = cleanStaleGatewayProcessesSync;var _stringCoerceLndEvhRk = require("./string-coerce-LndEvhRk.js");
var _pathsCnwfh6dH = require("./paths-Cnwfh6dH.js");
var _windowsInstallRootsD8UF6BY = require("./windows-install-roots-D8UF6B-y.js");
var _subsystemCH8Q21Y = require("./subsystem-C-H8Q21Y.js");
var _portsLsofB3uEEffK = require("./ports-lsof-B3uEEffK.js");
var _gatewayProcessArgvCCetDqqU = require("./gateway-process-argv-CCetDqqU.js");
var _cmdArgvZIYC0eG = require("./cmd-argv-ZIYC0eG4.js");
...
function readListeningPidsViaPowerShell(port, timeoutMs) {
  const ps = (0, _nodeChild_process.spawnSync)("powershell", [
  "-NoProfile",
  "-Command",
...
function readWindowsListeningPidsResultSync(port, timeoutMs = DEFAULT_TIMEOUT_MS) {
  ...
  const netstat = (0, _nodeChild_process.spawnSync)("netstat", [
  "-ano",
  "-p",
  "tcp"],
  {
...
function extractWindowsCommandLine(raw) {
  ...
  for (const line of lines) {
    if (!(0, _stringCoerceLndEvhRk.a)(line).startsWith("commandline=")) continue;
    return line.slice(12).trim() || null;
  }
  return lines.find((line) => (0, _stringCoerceLndEvhRk.a)(line) !== "commandline") ?? null;
}
...
function readWindowsProcessArgsResultSync(pid, timeoutMs = DEFAULT_TIMEOUT_MS) {
  const powershell = (0, _nodeChild_process.spawnSync)("powershell", [
  "-NoProfile",
  "-Command",
  ...
  if (!powershell.error && powershell.status === 0) {
    ...
    return {
      ok: true,
      args: command ? (0, _cmdArgvZIYC0eG.t)(command) : null
    };
  }
  const wmic = (0, _nodeChild_process.spawnSync)("wmic", [
  "process",
  "where",
  `ProcessId=${pid}`,
  "get",
...
const restartLog = (0, _subsystemCH8Q21Y.t)("restart");
function getTimeMs() {
  return Date.now();
}
...
function readParentPidFromProc(pid) {
  try {
    const match = (0, _nodeFs.readFileSync)(`/proc/${pid}/status`, "utf8").match(/^PPid:\s*(\d+)/m);
...
function readUnixProcessArgsSync(pid, spawnTimeoutMs) {
  if (process.platform === "linux") try {
    const args = (0, _gatewayProcessArgvCCetDqqU.n)((0, _nodeFs.readFileSync)(`/proc/${pid}/cmdline`, "utf8"));
    if (args.length > 0) return args;
  } catch {}
  const res = (0, _nodeChild_process.spawnSync)("ps", [
  "-ww",
  "-p",
  String(pid),
  "-o",
...
function parsePidsFromLsofOutput(stdout, spawnTimeoutMs) {
  ...
  for (const entry of parseLsofEntries(stdout)) {
    ...
    if (entry.cmd && (0, _stringCoerceLndEvhRk.a)(entry.cmd).includes("openclaw")) {
      pids.push(entry.pid);
      continue;
    }
...
function filterVerifiedWindowsGatewayPidsResult(rawPids, processArgsResult) {
  ...
  for (const pid of Array.from(new Set(rawPids))) {
    ...
    if (argsResult.args != null && (0, _gatewayProcessArgvCCetDqqU.t)(argsResult.args, { allowGatewayBinary: true })) verified.push(pid);
  }
  return {
    ok: true,
    pids: verified
...
function findGatewayPidsOnPortSync(port, spawnTimeoutMs = SPAWN_TIMEOUT_MS) {
  ...
  const res = (0, _nodeChild_process.spawnSync)((0, _portsLsofB3uEEffK.n)(), [
  "-nP",
  `-iTCP:${port}`,
  "-sTCP:LISTEN",
  "-Fpc"],
...
function terminateStaleProcessesWindows(pids) {
  const taskkillPath = _nodePath.default.win32.join((0, _windowsInstallRootsD8UF6BY.t)().systemRoot, "System32", "taskkill.exe");
  const killed = [];
  for (const pid of pids) {
    const graceful = (0, _nodeChild_process.spawnSync)(taskkillPath, [
    "/T",
    "/PID",
    String(pid)],
    {
    ...
    const forced = (0, _nodeChild_process.spawnSync)(taskkillPath, [
    "/F",
    "/T",
    "/PID",
    String(pid)],
...
function cleanStaleGatewayProcessesSync(portOverride) {
  try {
    const port = typeof portOverride === "number" && Number.isFinite(portOverride) && portOverride > 0 ? Math.floor(portOverride) : (0, _pathsCnwfh6dH.u)(void 0, process.env);
