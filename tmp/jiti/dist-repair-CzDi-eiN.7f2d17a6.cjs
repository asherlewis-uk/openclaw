"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.n = suggestOAuthProfileIdForLegacyDefault;exports.t = repairOAuthProfileIdMismatch;var _providerIdCz7K6wgK = require("./provider-id-Cz7K6wgK.js");
...
function suggestOAuthProfileIdForLegacyDefault(params) {
  ...
  if (legacyCfg && (0, _providerIdCz7K6wgK.r)(legacyCfg.provider) === providerKey && legacyCfg.mode !== "oauth") return null;
  const oauthProfiles = (0, _profileList1dJMPDMe.n)(params.store, providerKey).filter((id) => params.store.profiles[id]?.type === "oauth");
  if (oauthProfiles.length === 0) return null;
  const configuredEmail = legacyCfg?.email?.trim();
  if (configuredEmail) {
    const byEmail = oauthProfiles.find((id) => {
      return (0, _identityDX9yGpYy.n)({
        cfg: params.cfg,
        store: params.store,
        profileId: id
...
function repairOAuthProfileIdMismatch(params) {
  const legacyProfileId = params.legacyProfileId ?? `${(0, _providerIdCz7K6wgK.r)(params.provider)}:default`;
  ...
  if ((0, _providerIdCz7K6wgK.r)(legacyCfg.provider) !== (0, _providerIdCz7K6wgK.r)(params.provider)) return {
    config: params.cfg,
    changes: [],
    migrated: false
  };
  ...
  const { email: toEmail, displayName: toDisplayName } = (0, _identityDX9yGpYy.n)({
    store: params.store,
  ...
  const nextOrder = (() => {
    ...
    if (!order) return;
    const resolvedKey = (0, _providerIdCz7K6wgK.t)(order, providerKey);
    if (!resolvedKey) return order;
    const existing = order[resolvedKey];
    if (!Array.isArray(existing)) return order;
    const deduped = (0, _profileList1dJMPDMe.t)(existing.map((id) => id === legacyProfileId ? toProfileId : id).filter((id) => typeof id === "string" && id.trim().length > 0));
    return {
      ...order,
      [resolvedKey]: deduped
    };
