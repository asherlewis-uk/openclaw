...
var _execCjEB16Q = require("./exec-Cj-EB16Q.js");
var _privateTempWorkspaceScYdgL = require("./private-temp-workspace-ScYdgL29.js");
var _resolveSystemBinDakSmk6u = require("./resolve-system-bin-DakSmk6u.js");
var _fileNameCLJgXLYy = require("./file-name-CLJgXLYy.js");
...
function requireSystemBin(name) {
  const resolved = (0, _resolveSystemBinDakSmk6u.t)(name, { trust: "standard" });
  if (!resolved) {
...
function parseFfprobeCsvFields(stdout, maxFields) {
  return stdout.trim().split(/[,\r\n]+/, maxFields).map((field) => (0, _stringCoerceLndEvhRk.a)(field));
}
...
function normalizeOutputFileName(value) {
  const baseName = (0, _fileNameCLJgXLYy.t)(value?.trim() || DEFAULT_OUTPUT_FILE_NAME);
...
async function transcodeAudioBufferToOpus(params) {
  return await (0, _privateTempWorkspaceScYdgL.r)({
    rootDir: (0, _tmpOpenclawDirDJsd5QL.n)(),
...
async function transcodeAudioBuffer(params) {
  ...
  const tmp = (0, _privateTempWorkspaceScYdgL.n)({
    rootDir: (0, _tmpOpenclawDirDJsd5QL.n)(),
...
const mediaAttachmentImageOpsLoader = (0, _lazyPromiseSFT4i6yI.n)(async () => {
  const { loadBundledPluginPublicArtifactModuleSync } = await Promise.resolve().then(() => jitiImport("./public-surface-loader-B2FmLqy6.js").then((m) => _interopRequireWildcard(m)));
...
function resolveImageTool(backend) {
  ...
  if (backend === "windows-native") {
    const powershell = (0, _resolveSystemBinDakSmk6u.t)("powershell", { trust: "strict" });
  ...
  if (backend === "imagemagick") {
    const magick = (0, _resolveSystemBinDakSmk6u.t)("magick", { trust: "standard" });
    if (magick) return {
      backend,
      flavor: "magick",
      command: magick
    ...
    if (process.platform !== "win32") {
      const convert = (0, _resolveSystemBinDakSmk6u.t)("convert", { trust: "standard" });
      if (convert) return {
        backend,
        flavor: "convert",
        command: convert
  ...
  if (backend === "graphicsmagick") {
    const gm = (0, _resolveSystemBinDakSmk6u.t)("gm", { trust: "standard" });
    return gm ? {
      backend,
      flavor: "gm",
      command: gm
  ...
  const ffmpeg = (0, _resolveSystemBinDakSmk6u.t)("ffmpeg", { trust: "standard" });
  return ffmpeg ? {
    backend,
    flavor: "ffmpeg",
    command: ffmpeg
...
async function runPowerShellImageScript(scriptName, script, args) {
  ...
  return await withImageTemp(async (workspace) => {
    ...
    return await (0, _execCjEB16Q.i)(tool.command, [
    "-NoProfile",
    "-NonInteractive",
    "-ExecutionPolicy",
    "Bypass",
...
async function runConvertTool(tool, args) {
  await (0, _execCjEB16Q.i)(tool.command, convertToolArgs(tool, args), {
    timeoutMs: IMAGE_PROCESS_TIMEOUT_MS,
    maxBuffer: IMAGE_TOOL_MAX_BUFFER
  });
}
...
async function externalResizeToJpeg(backend, params) {
  ...
  return await withImageTemp(async (workspace) => {
    ...
    if (tool.flavor === "ffmpeg") {
      ...
      await (0, _execCjEB16Q.i)(tool.command, [
      "-y",
      "-i",
      input,
      "-vf",
...
async function externalConvertToJpeg(backend, buffer) {
  ...
  return await withImageTemp(async (workspace) => {
    ...
    const output = workspace.path("out.jpg");
    if (tool.flavor === "ffmpeg") await (0, _execCjEB16Q.i)(tool.command, [
    "-y",
    "-i",
    input,
    "-frames:v",
...
async function sipsResizeToJpeg(params) {
  return await withImageTemp(async (workspace) => {
    const input = await workspace.write("in.img", params.buffer);
    const output = workspace.path("out.jpg");
    await (0, _execCjEB16Q.i)("/usr/bin/sips", [
    "-Z",
