"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.a = resolveSecretRefValue;exports.i = resolveSecretRefString;exports.n = void 0;exports.o = resolveSecretRefValues;exports.r = isProviderScopedSecretResolutionError;exports.t = void 0;var _errorsIxwfrboQ = require("./errors-ixwfrboQ.js");
...
var _sharedQs9fkknV = require("./shared-Qs9fkknV.js");
var _refContractBR8wwaMv = require("./ref-contract-BR8wwaMv.js");
require("./audit-fs-BQG65_V2.js");
require("./scan-paths-BBag79CC.js");
var _runWithConcurrencyC2_KPmI = require("./run-with-concurrency-C2_KPmI6.js");
var _jsonPointerD3Muu4EB = require("./json-pointer-D3Muu4EB.js");
...
function throwUnknownProviderResolutionError(params) {
  ...
  throw providerResolutionError({
    ...
    message: (0, _errorsIxwfrboQ.i)(params.err),
    cause: params.err
  });
}
...
function resolveResolutionLimits(config) {
  ...
  return {
    maxProviderConcurrency: (0, _sharedQs9fkknV.r)(resolution?.maxProviderConcurrency, DEFAULT_PROVIDER_CONCURRENCY),
    maxRefsPerProvider: (0, _sharedQs9fkknV.r)(resolution?.maxRefsPerProvider, DEFAULT_MAX_REFS_PER_PROVIDER),
    maxBatchBytes: (0, _sharedQs9fkknV.r)(resolution?.maxBatchBytes, DEFAULT_MAX_BATCH_BYTES)
  };
}
...
function resolveConfiguredProvider(ref, config) {
  ...
  if (!providerConfig) {
    if (ref.source === "env" && ref.provider === (0, _refContractBR8wwaMv.l)(config, "env")) return { source: "env" };
    throw providerResolutionError({
      source: ref.source,
...
async function readFileProviderPayload(params) {
  ...
  const readPromise = (async () => {
    const timeoutMs = (0, _sharedQs9fkknV.r)(params.providerConfig.timeoutMs, DEFAULT_FILE_TIMEOUT_MS);
    const maxBytes = (0, _sharedQs9fkknV.r)(params.providerConfig.maxBytes, DEFAULT_FILE_MAX_BYTES);
    try {
...
async function resolveEnvRefs(params) {
  ...
  for (const ref of params.refs) {
    ...
    if (!(0, _sharedQs9fkknV.n)(envValue)) throw refResolutionError({
      source: "env",
      provider: params.providerName,
      refId: ref.id,
...
async function resolveFileRefs(params) {
  ...
  if (mode === "singleValue") {
    for (const ref of params.refs) {
      if (ref.id !== "value") throw refResolutionError({
        ...
        provider: params.providerName,
        refId: ref.id,
        message: `singleValue file provider "${params.providerName}" expects ref id "${_refContractBR8wwaMv.i}".`
      });
  ...
  for (const ref of params.refs) try {
    resolved.set(ref.id, (0, _jsonPointerD3Muu4EB.n)(payload, ref.id, { onMissing: "throw" }));
  } catch (err) {
    throw refResolutionError({
      source: "file",
      provider: params.providerName,
      refId: ref.id,
      message: (0, _errorsIxwfrboQ.i)(err),
      cause: err
    });
  }
  return resolved;
...
async function resolveExecRefs(params) {
  ...
  const timeoutMs = (0, _sharedQs9fkknV.r)(params.providerConfig.timeoutMs, DEFAULT_EXEC_TIMEOUT_MS);
  const noOutputTimeoutMs = (0, _sharedQs9fkknV.r)(params.providerConfig.noOutputTimeoutMs, timeoutMs);
  const maxOutputBytes = (0, _sharedQs9fkknV.r)(params.providerConfig.maxOutputBytes, DEFAULT_EXEC_MAX_OUTPUT_BYTES);
...
async function resolveSecretRefValues(refs, options) {
  ...
  for (const ref of refs) {
    ...
    if (ref.source === "exec" && !(0, _refContractBR8wwaMv.o)(id)) throw new Error(`${(0, _refContractBR8wwaMv.a)()} (ref: ${ref.source}:${ref.provider}:${id}).`);
    uniqueRefs.set((0, _refContractBR8wwaMv.u)(ref), {
      ...ref,
      id
    });
  }
  ...
  const taskResults = await (0, _runWithConcurrencyC2_KPmI.t)({
  ...
  for (const result of taskResults.results) for (const ref of result.group.refs) {
    ...
    resolved.set((0, _refContractBR8wwaMv.u)(ref), result.values.get(ref.id));
  }
  return resolved;
}
async function resolveSecretRefValue(ref, options) {
  const cache = options.cache;
  const key = (0, _refContractBR8wwaMv.u)(ref);
...
async function resolveSecretRefString(ref, options) {
  ...
  if (!(0, _sharedQs9fkknV.n)(resolved)) throw new Error(`Secret reference "${ref.source}:${ref.provider}:${ref.id}" resolved to a non-string or empty value.`);
  return resolved;
}
//#endregion /* v9-f81429ee1b39d3e3 */
