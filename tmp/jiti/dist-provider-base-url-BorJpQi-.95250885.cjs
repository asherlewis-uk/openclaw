...
require("./ssrf-runtime-B7YsbRmp.js");
...
function resolveOllamaApiBase(configuredBaseUrl) {
  if (!configuredBaseUrl) return _defaultsB7Swclqd.n;
...
async function queryOllamaModelShowInfo(apiBase, modelName) {
  ...
  try {
    const { response, release } = await (0, _fetchGuardB7xT8hT.n)({
...
function buildOllamaModelDefinition(modelId, contextWindow, capabilities) {
  ...
  return {
    ...
    reasoning,
    input,
    cost: _defaultsB7Swclqd.i,
    contextWindow: contextWindow ?? 128e3,
    maxTokens: _defaultsB7Swclqd.a,
    compat
  };
}
