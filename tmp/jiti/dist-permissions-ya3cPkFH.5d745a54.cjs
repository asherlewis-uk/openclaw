"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.a = inspectPathPermissions;exports.c = isWorldReadable;exports.d = safeStat;exports.i = formatPermissionRemediation;exports.l = isWorldWritable;exports.n = formatIcaclsResetCommand;exports.o = isGroupReadable;exports.r = formatPermissionDetail;exports.s = isGroupWritable;exports.t = createIcaclsResetCommand;exports.u = modeBits;var _stringCoerce6TL5VVOL = require("./string-coerce-6TL5VVOL.js");
...
const normalize$1 = (value) => (0, _stringCoerce6TL5VVOL.t)(value);
const defaultWindowsUserInfo = () => _nodeOs.default.userInfo();
...
async function safeStat(targetPath) {
  try {
    const lst = await _promises.default.lstat(targetPath);
    return {
      ok: true,
...
async function inspectPathPermissions(targetPath, opts) {
  ...
  if (st.isSymlink) try {
    const target = await _promises.default.stat(targetPath);
...
function normalizeWindowsInstallRoot(value) {
  ...
  if (!trimmed || !_nodePath.default.win32.isAbsolute(trimmed)) return null;
...
function resolveWindowsSystemCommand(command, env) {
  ...
  return _nodePath.default.win32.join(root, "System32", command);
}
...
function summarizeWindowsAcl(entries, env) {
  ...
  for (const entry of entries) {
    ...
    if (classification === "trusted") trusted.push(entry);else
    if (classification === "world") untrustedWorld.push(entry);else
