...
function normalizeApiKeyInput(raw) {
  const trimmed = (0, _stringCoerceLndEvhRk.d)(raw) ?? "";
  if (!trimmed) return "";
...
function normalizeTokenProviderInput(tokenProvider) {
  return (0, _stringCoerceLndEvhRk.s)(tokenProvider);
}
function normalizeSecretInputModeInput(secretInputMode) {
  const normalized = (0, _stringCoerceLndEvhRk.s)(secretInputMode);
...
async function ensureApiKeyFromEnvOrPrompt(params) {
  const selectedMode = await (0, _providerAuthModeIzYTvQNA.t)({
    prompter: params.prompter,
    explicitMode: params.secretInputMode
  });
  const env = params.env ?? process.env;
  const envKey = (0, _modelAuthEnvPIWXP99L.t)(params.provider, env);
  if (selectedMode === "ref") {
    if (typeof params.prompter.select !== "function") {
      const fallback = (0, _providerAuthRefNjeMi9pi.r)({
        config: params.config,
        provider: params.provider,
        preferredEnvVar: envKey?.source ? (0, _providerAuthRefNjeMi9pi.t)(envKey.source) : void 0,
        env
      });
      await params.setCredential(fallback.ref, selectedMode);
      return fallback.resolvedValue;
    }
    const resolved = await (0, _providerAuthRefNjeMi9pi.n)({
      provider: params.provider,
      config: params.config,
      prompter: params.prompter,
      preferredEnvVar: envKey?.source ? (0, _providerAuthRefNjeMi9pi.t)(envKey.source) : void 0,
