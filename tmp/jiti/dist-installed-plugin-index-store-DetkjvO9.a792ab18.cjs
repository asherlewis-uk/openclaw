...
var _installSourceInfoBGhKPNa = require("./install-source-info-BGhKPNa2.js");
...
var _zodParse7tICWiqC = require("./zod-parse-7tICWiqC.js");
...
function resolveInstalledPluginIndexPolicyHash(config) {
  const normalized = (0, _configStateBgyjpLHd.s)(config?.plugins);
  const channelPolicy = {};
...
function buildStartupInfo(record) {
  return {
    ...
    memory: (0, _slotsDLJ2h3Qv.r)(record.kind, "memory"),
...
function resolvePackageJsonPath(candidate) {
  ...
  return _nodeFs.default.existsSync(packageJsonPath) ? packageJsonPath : void 0;
}
...
function describePackageInstallSource(candidate) {
  ...
  if (!install) return;
  return (0, _installSourceInfoBGhKPNa.t)(install, { expectedPackageName: candidate?.packageName });
}
...
function buildInstalledPluginIndexRecords(params) {
  ...
  const normalizedConfig = (0, _configStateBgyjpLHd.s)(params.config?.plugins);
  return params.registry.plugins.map((record) => {
    ...
    const enabled = (0, _configStateBgyjpLHd.c)({
      id: record.id,
...
function resolveInstalledPluginIndexRegistry(params) {
  if (params.candidates) return {
    ...
    registry: (0, _manifestRegistryDgt5vVG.t)({
  ...
  const normalized = (0, _configStateBgyjpLHd.s)(params.config?.plugins);
  const installRecords = params.installRecords ?? (0, _installedPluginIndexRecordReaderBieSpyRG.n)({ env: params.env });
...
function buildInstalledPluginIndex(params) {
  ...
  const installRecords = normalizeInstallRecordMap(params.installRecords ?? (0, _installedPluginIndexRecordReaderBieSpyRG.n)({
    env,
  ...
  return {
    ...
    hostContractVersion: (0, _versionB2G3zXnp.o)(env),
...
function isInstalledPluginEnabled(index, pluginId, config) {
  ...
  const normalizedConfig = (0, _configStateBgyjpLHd.s)(config?.plugins);
  const state = (0, _configStateBgyjpLHd.l)({
    id: record.pluginId,
    origin: record.origin,
    config: normalizedConfig,
    rootConfig: config,
...
const StringArraySchema = _zod.z.array(_zod.z.string());
const InstalledPluginIndexStartupSchema = _zod.z.object({
  sidecar: _zod.z.boolean(),
  memory: _zod.z.boolean(),
  deferConfiguredChannelFullLoadUntilAfterListen: _zod.z.boolean(),
  agentHarnesses: StringArraySchema
});
const InstalledPluginFileSignatureSchema = _zod.z.object({
  size: _zod.z.number(),
  mtimeMs: _zod.z.number(),
  ctimeMs: _zod.z.number().optional()
});
const InstalledPluginIndexRecordSchema = _zod.z.object({
  pluginId: _zod.z.string(),
  packageName: _zod.z.string().optional(),
  packageVersion: _zod.z.string().optional(),
  installRecord: _zod.z.record(_zod.z.string(), _zod.z.unknown()).optional(),
  installRecordHash: _zod.z.string().optional(),
  packageInstall: _zod.z.unknown().optional(),
  packageChannel: _zod.z.unknown().optional(),
  manifestPath: _zod.z.string(),
  manifestHash: _zod.z.string(),
  manifestFile: InstalledPluginFileSignatureSchema.optional(),
  format: _zod.z.string().optional(),
  bundleFormat: _zod.z.string().optional(),
  source: _zod.z.string().optional(),
  setupSource: _zod.z.string().optional(),
  packageJson: _zod.z.object({
    path: _zod.z.string(),
    hash: _zod.z.string(),
    fileSignature: InstalledPluginFileSignatureSchema.optional()
  }).optional(),
  rootDir: _zod.z.string(),
  origin: _zod.z.string(),
  enabled: _zod.z.boolean(),
  enabledByDefault: _zod.z.boolean().optional(),
  ...
  compat: _zod.z.array(_zod.z.string())
});
const InstalledPluginInstallRecordSchema = _zod.z.record(_zod.z.string(), _zod.z.unknown());
const PluginDiagnosticSchema = _zod.z.object({
  level: _zod.z.union([_zod.z.literal("warn"), _zod.z.literal("error")]),
  message: _zod.z.string(),
  pluginId: _zod.z.string().optional(),
  source: _zod.z.string().optional()
});
const InstalledPluginIndexSchema = _zod.z.object({
  version: _zod.z.literal(1),
  warning: _zod.z.string().optional(),
  hostContractVersion: _zod.z.string(),
  compatRegistryVersion: _zod.z.string(),
  migrationVersion: _zod.z.literal(1),
  policyHash: _zod.z.string(),
  generatedAtMs: _zod.z.number(),
  refreshReason: _zod.z.string().optional(),
  installRecords: _zod.z.record(_zod.z.string(), InstalledPluginInstallRecordSchema).optional(),
  plugins: _zod.z.array(InstalledPluginIndexRecordSchema),
  diagnostics: _zod.z.array(PluginDiagnosticSchema)
});
function copySafeInstallRecords(records) {
  if (!records) return;
  const safeRecords = {};
  for (const [pluginId, record] of Object.entries(records)) {
    if ((0, _prototypeKeysDna4GplE.t)(pluginId)) continue;
    safeRecords[pluginId] = record;
  }
  return safeRecords;
}
function parseInstalledPluginIndex(value) {
  const parsed = (0, _zodParse7tICWiqC.n)(InstalledPluginIndexSchema, value);
...
async function readPersistedInstalledPluginIndex(options = {}) {
  return parseInstalledPluginIndex(await (0, _jsonFilesCahFuwKs.l)((0, _installedPluginIndexRecordReaderBieSpyRG.a)(options)));
}
function readPersistedInstalledPluginIndexSync(options = {}) {
  return parseInstalledPluginIndex((0, _jsonFilesCahFuwKs.u)((0, _installedPluginIndexRecordReaderBieSpyRG.a)(options)));
}
async function writePersistedInstalledPluginIndex(index, options = {}) {
  const filePath = (0, _installedPluginIndexRecordReaderBieSpyRG.a)(options);
  await (0, _jsonFilesCahFuwKs.d)(filePath, {
    ...index,
    warning: INSTALLED_PLUGIN_INDEX_WARNING
  }, {
...
function writePersistedInstalledPluginIndexSync(index, options = {}) {
  const filePath = (0, _installedPluginIndexRecordReaderBieSpyRG.a)(options);
  (0, _jsonFileC3OPVumq.n)(filePath, {
    ...index,
    warning: INSTALLED_PLUGIN_INDEX_WARNING
  });
  clearCurrentPluginMetadataSnapshotState();
...
function canRefreshPersistedPolicyState(persisted, params) {
  ...
  const env = params.env ?? process.env;
  if (persisted.version !== 1 || persisted.hostContractVersion !== (0, _versionB2G3zXnp.o)(env) || persisted.compatRegistryVersion !== resolveCompatRegistryVersion() || persisted.migrationVersion !== 1) return false;
...
function refreshPersistedPolicyState(persisted, params) {
  ...
  return {
    ...
    plugins: persisted.plugins.map((plugin) => ({
      ...plugin,
      enabled: (0, _configStateBgyjpLHd.c)({
        id: plugin.pluginId,
