"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.a = resolveSecretRefValue;exports.i = resolveSecretRefString;exports.n = void 0;exports.o = resolveSecretRefValues;exports.r = isProviderScopedSecretResolutionError;exports.t = void 0;var _errorsVfATXfah = require("./errors-VfATXfah.js");
...
var _sharedYONM2O7r = require("./shared-yONM2O7r.js");
var _refContractBLk8mcqQ = require("./ref-contract-BLk8mcqQ.js");
require("./audit-fs-B7MzqwqM.js");
require("./scan-paths-CQGIktzD.js");
var _runWithConcurrencyU5TU0MMx = require("./run-with-concurrency-u5TU0MMx.js");
var _jsonPointerCmLFufbV = require("./json-pointer-CmLFufbV.js");
...
function throwUnknownProviderResolutionError(params) {
  ...
  throw providerResolutionError({
    ...
    message: (0, _errorsVfATXfah.i)(params.err),
    cause: params.err
  });
}
async function readFileStatOrThrow(pathname, label) {
  const stat = await (0, _permissionsBwHqNako.d)(pathname);
...
function resolveResolutionLimits(config) {
  ...
  return {
    maxProviderConcurrency: (0, _sharedYONM2O7r.r)(resolution?.maxProviderConcurrency, DEFAULT_PROVIDER_CONCURRENCY),
    maxRefsPerProvider: (0, _sharedYONM2O7r.r)(resolution?.maxRefsPerProvider, DEFAULT_MAX_REFS_PER_PROVIDER),
    maxBatchBytes: (0, _sharedYONM2O7r.r)(resolution?.maxBatchBytes, DEFAULT_MAX_BATCH_BYTES)
  };
}
...
function resolveConfiguredProvider(ref, config) {
  ...
  if (!providerConfig) {
    if (ref.source === "env" && ref.provider === (0, _refContractBLk8mcqQ.l)(config, "env")) return { source: "env" };
    throw providerResolutionError({
      source: ref.source,
...
async function assertSecurePath(params) {
  ...
  if (params.trustedDirs && params.trustedDirs.length > 0) {
    if (!params.trustedDirs.map((entry) => (0, _utilsCKsuXgDI.p)(entry)).some((dir) => (0, _pathB5B_oAT.i)(dir, effectivePath))) throw new Error(`${params.label} is outside trustedDirs: ${effectivePath}`);
  }
  if (params.allowInsecurePath) return effectivePath;
  const perms = await (0, _permissionsBwHqNako.a)(effectivePath);
...
async function readFileProviderPayload(params) {
  ...
  const filePath = (0, _utilsCKsuXgDI.p)(params.providerConfig.path);
  const readPromise = (async () => {
    const timeoutMs = (0, _sharedYONM2O7r.r)(params.providerConfig.timeoutMs, DEFAULT_FILE_TIMEOUT_MS);
    const maxBytes = (0, _sharedYONM2O7r.r)(params.providerConfig.maxBytes, DEFAULT_FILE_MAX_BYTES);
    try {
      const { buffer: payload } = await (0, _fsSafeDpJlqO1z.s)({
        filePath,
      ...
      if (!(0, _utilsCKsuXgDI.c)(parsed)) throw new Error(`File provider "${params.providerName}" payload is not a JSON object.`);
      return parsed;
    } catch (error) {
      if (error instanceof _pathB5B_oAT.p && error.code === "timeout") throw new Error(`File provider "${params.providerName}" timed out after ${timeoutMs}ms.`, { cause: error });
      throw error;
    }
  })();
  if (cache) {
...
async function resolveEnvRefs(params) {
  ...
  for (const ref of params.refs) {
    ...
    if (!(0, _sharedYONM2O7r.n)(envValue)) throw refResolutionError({
      source: "env",
      provider: params.providerName,
      refId: ref.id,
...
async function resolveFileRefs(params) {
  ...
  if (mode === "singleValue") {
    for (const ref of params.refs) {
      if (ref.id !== "value") throw refResolutionError({
        ...
        provider: params.providerName,
        refId: ref.id,
        message: `singleValue file provider "${params.providerName}" expects ref id "${_refContractBLk8mcqQ.i}".`
      });
  ...
  for (const ref of params.refs) try {
    resolved.set(ref.id, (0, _jsonPointerCmLFufbV.n)(payload, ref.id, { onMissing: "throw" }));
  } catch (err) {
    throw refResolutionError({
      source: "file",
      provider: params.providerName,
      refId: ref.id,
      message: (0, _errorsVfATXfah.i)(err),
      cause: err
    });
  }
  return resolved;
...
function parseExecValues(params) {
  ...
  if (!(0, _utilsCKsuXgDI.c)(responseValues)) throw providerResolutionError({
    source: "exec",
    provider: params.providerName,
    message: `Exec provider "${params.providerName}" response missing "values".`
  });
  const responseErrors = (0, _utilsCKsuXgDI.c)(parsed.errors) ? parsed.errors : null;
  const out = {};
  for (const id of params.ids) {
    if (responseErrors && id in responseErrors) {
      const entry = responseErrors[id];
      if ((0, _utilsCKsuXgDI.c)(entry) && typeof entry.message === "string" && entry.message.trim()) throw refResolutionError({
        source: "exec",
        provider: params.providerName,
        refId: id,
...
async function resolveExecRefs(params) {
  ...
  const commandPath = (0, _utilsCKsuXgDI.p)(params.providerConfig.command);
  ...
  const timeoutMs = (0, _sharedYONM2O7r.r)(params.providerConfig.timeoutMs, DEFAULT_EXEC_TIMEOUT_MS);
  const noOutputTimeoutMs = (0, _sharedYONM2O7r.r)(params.providerConfig.noOutputTimeoutMs, timeoutMs);
  const maxOutputBytes = (0, _sharedYONM2O7r.r)(params.providerConfig.maxOutputBytes, DEFAULT_EXEC_MAX_OUTPUT_BYTES);
...
async function resolveSecretRefValues(refs, options) {
  ...
  for (const ref of refs) {
    ...
    if (ref.source === "exec" && !(0, _refContractBLk8mcqQ.o)(id)) throw new Error(`${(0, _refContractBLk8mcqQ.a)()} (ref: ${ref.source}:${ref.provider}:${id}).`);
    uniqueRefs.set((0, _refContractBLk8mcqQ.u)(ref), {
      ...ref,
      id
    });
  }
  ...
  const taskResults = await (0, _runWithConcurrencyU5TU0MMx.t)({
  ...
  for (const result of taskResults.results) for (const ref of result.group.refs) {
    ...
    resolved.set((0, _refContractBLk8mcqQ.u)(ref), result.values.get(ref.id));
  }
  return resolved;
}
async function resolveSecretRefValue(ref, options) {
  const cache = options.cache;
  const key = (0, _refContractBLk8mcqQ.u)(ref);
...
async function resolveSecretRefString(ref, options) {
  ...
  if (!(0, _sharedYONM2O7r.n)(resolved)) throw new Error(`Secret reference "${ref.source}:${ref.provider}:${ref.id}" resolved to a non-string or empty value.`);
  return resolved;
}
//#endregion /* v9-9804d35c8859f27d */
