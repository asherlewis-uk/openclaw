"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.n = resolveBundledPluginCompatibleLoadValues;exports.r = withActivatedPluginIds;exports.t = resolveBundledPluginCompatibleActivationInputs;var _configStateBgyjpLHd = require("./config-state-BgyjpLHd.js");
...
function applyPluginCompatibilityOverrides(params) {
  const allowlistCompat = params.compat?.allowlistPluginIds?.length ? (0, _bundledCompatDfs9mbab.t)({
    config: params.config,
    pluginIds: params.compat.allowlistPluginIds
  }) : params.config;
  const enablementCompat = params.compat?.enablementPluginIds?.length ? (0, _bundledCompatDfs9mbab.n)({
    config: allowlistCompat,
    pluginIds: params.compat.enablementPluginIds
  }) : allowlistCompat;
  return params.compat?.vitestPluginIds?.length ? (0, _bundledCompatDfs9mbab.r)({
...
function applyPluginAutoEnableForActivation(params) {
  const currentSnapshot = (0, _currentPluginMetadataSnapshotDHmOxzz.n)({
    config: params.config,
    env: params.env,
    workspaceDir: params.workspaceDir,
    allowWorkspaceScopedSnapshot: true
  });
  const defaultDiscoverySnapshot = (0, _configStateBgyjpLHd.s)(params.config.plugins).loadPaths.length === 0 ? (0, _currentPluginMetadataSnapshotDHmOxzz.n)({
    env: params.env,
    workspaceDir: params.workspaceDir,
    allowWorkspaceScopedSnapshot: true,
  ...
  return (0, _pluginAutoEnableDLIxCAM.t)({
    config: params.config,
    env: params.env,
...
function resolvePluginActivationSnapshot(params) {
  ...
  return {
    ...
    normalized: (0, _configStateBgyjpLHd.s)(resolvedConfig?.plugins),
    activationSourceConfig: rawConfig,
    activationSource: (0, _configStateBgyjpLHd.n)({ config: rawConfig }),
...
function resolvePluginActivationInputs(params) {
  ...
  return {
    ...
    config,
    normalized: (0, _configStateBgyjpLHd.s)(config?.plugins),
