"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.a = resolveContextEngineOwnerPluginId;exports.i = resolveContextEngine;exports.n = registerContextEngine;exports.r = registerContextEngineForOwner;exports.t = clearContextEnginesForOwner;var _ansiBk0Jp_0O = require("./ansi-Bk0Jp_0O.js");
...
const contextEngineRegistryState = (0, _globalSingletonB2nbp4Tq.n)(CONTEXT_ENGINE_REGISTRY_STATE, () => ({ engines: /* @__PURE__ */new Map() }));
...
function registerContextEngineForOwner(id, factory, owner, opts) {
  ...
  if (id === (0, _slotsMFUVcT3n.n)("contextEngine") && normalizedOwner !== CORE_CONTEXT_ENGINE_OWNER) return {
    ok: false,
...
function describeResolvedContextEngineContractError(engineId, engine) {
  ...
  if (!info || typeof info !== "object") issues.push("missing info");else
  {
...
async function resolveContextEngine(config, options) {
  ...
  const engineId = typeof slotValue === "string" && slotValue.trim() ? slotValue.trim() : (0, _slotsMFUVcT3n.n)("contextEngine");
  const defaultEngineId = (0, _slotsMFUVcT3n.n)("contextEngine");
  ...
  if (!entry) {
    ...
    console.error(`[context-engine] Context engine "${(0, _ansiBk0Jp_0O.t)(engineId)}" is not registered; falling back to default engine "${defaultEngineId}".`);
  ...
  try {
    ...
    console.error(`[context-engine] Context engine "${(0, _ansiBk0Jp_0O.t)(engineId)}" factory threw during resolution: ${(0, _ansiBk0Jp_0O.t)(factoryError instanceof Error ? factoryError.message : String(factoryError))}; falling back to default engine "${defaultEngineId}".`);
    return resolveDefaultContextEngine(defaultEngineId, factoryCtx);
  }
  let contractError;
  try {
    ...
    console.error(`[context-engine] Context engine "${(0, _ansiBk0Jp_0O.t)(engineId)}" contract validation threw: ${(0, _ansiBk0Jp_0O.t)(validationError instanceof Error ? validationError.message : String(validationError))}; falling back to default engine "${defaultEngineId}".`);
    return resolveDefaultContextEngine(defaultEngineId, factoryCtx);
  }
  if (contractError) {
    if (isDefaultEngine) throw new Error(contractError);
    console.error(`[context-engine] ${(0, _ansiBk0Jp_0O.t)(contractError)}; falling back to default engine "${defaultEngineId}".`);
    return resolveDefaultContextEngine(defaultEngineId, factoryCtx);
  }
