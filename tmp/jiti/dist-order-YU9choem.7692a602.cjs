"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.a = clearExpiredCooldowns;exports.c = isAuthCooldownBypassedForProvider;exports.i = resolveAuthProfileOrder;exports.l = isProfileInCooldown;exports.n = isStoredCredentialCompatibleWithAuthProvider;exports.o = getSoonestCooldownExpiry;exports.r = resolveAuthProfileEligibility;exports.s = isActiveUnusableWindow;exports.t = isConfiguredAwsSdkAuthProfileForProvider;exports.u = resolveProfileUnusableUntil;var _providerIdCz7K6wgK = require("./provider-id-Cz7K6wgK.js");
...
function isAuthCooldownBypassedForProvider(provider) {
  const normalized = (0, _providerIdCz7K6wgK.r)(provider ?? "");
...
function isOpenAIApiKeyCompatibleWithCodexAuth(params) {
  ...
  const providerKey = (0, _providerAuthAliases3NFJcokO.r)(params.profileProvider ?? "", { config: params.cfg });
...
function isCredentialProviderCompatibleWithAuthProvider(params) {
  return (0, _providerAuthAliases3NFJcokO.r)(params.credential.provider, { config: params.cfg }) === params.providerAuthKey || isOpenAIApiKeyCompatibleWithCodexAuth({
    cfg: params.cfg,
...
function isStoredCredentialCompatibleWithAuthProvider(params) {
  return isCredentialProviderCompatibleWithAuthProvider({
    cfg: params.cfg,
    providerAuthKey: (0, _providerAuthAliases3NFJcokO.r)(params.provider, { config: params.cfg }),
    credential: params.credential
  });
}
function isConfiguredProfileCompatibleWithAuthProvider(params) {
  return (0, _providerAuthAliases3NFJcokO.r)(params.provider, { config: params.cfg }) === params.providerAuthKey || isOpenAIApiKeyCompatibleWithCodexAuth({
    cfg: params.cfg,
    providerAuthKey: params.providerAuthKey,
    credential: params.credential,
...
function listProfilesCompatibleWithAuthProvider(params) {
  if (params.providerAuthKey !== OPENAI_CODEX_PROVIDER_ID) return (0, _profileList1dJMPDMe.n)(params.store, params.provider);
...
function resolveProviderAuthMode(cfg, provider) {
  ...
  if (!providers) return;
  const auth = (0, _providerIdCz7K6wgK.n)(providers, provider)?.auth;
...
function providerAllowsAwsSdkAuth(cfg, provider) {
  ...
  return authMode === "aws-sdk" || authMode === void 0 && (0, _providerIdCz7K6wgK.r)(provider) === "amazon-bedrock";
}
function isConfiguredAwsSdkAuthProfileForProvider(params) {
  ...
  const providerAuthKey = (0, _providerAuthAliases3NFJcokO.r)(params.provider, { config: params.cfg });
  if ((0, _providerAuthAliases3NFJcokO.r)(profileConfig.provider, { config: params.cfg }) !== providerAuthKey) return false;
...
function resolveAuthProfileEligibility(params) {
  ...
  const credentialEligibility = (0, _storeA4exFSck.E)({
    credential: cred,
    now: params.now
  });
  return {
...
function resolveAuthProfileOrder(params) {
  ...
  const providerAuthKey = (0, _providerAuthAliases3NFJcokO.r)(provider, { config: cfg });
  const now = Date.now();
  ...
  const deduped = (0, _profileList1dJMPDMe.t)(filtered);
...
function resolveAuthOrder(order, provider) {
  return (0, _providerIdCz7K6wgK.n)(order, provider);
}
function isNativeCredentialProviderCompatibleWithAuthProvider(params) {
  if (!params.credential) return false;
  return (0, _providerAuthAliases3NFJcokO.r)(params.credential.provider, { config: params.cfg }) === params.providerAuthKey;
}
function mergeAliasOrderWithNativeProfiles(params) {
  const nativeIds = new Set(params.nativeProfiles);
  return (0, _profileList1dJMPDMe.t)(params.aliasOrder.some((profileId) => nativeIds.has(profileId)) ? [...params.aliasOrder, ...params.nativeProfiles] : [...params.nativeProfiles, ...params.aliasOrder]);
}
function orderProfilesByMode(order, store) {
  const now = Date.now();
  const available = [];
  const inCooldown = [];
  for (const profileId of order) if (isProfileInCooldown(store, profileId)) inCooldown.push(profileId);else
