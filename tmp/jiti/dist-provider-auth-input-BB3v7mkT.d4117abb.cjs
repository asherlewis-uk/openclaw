...
var _providerAuthModeBTbGLgZT = require("./provider-auth-mode-BTbGLgZT.js");
var _providerAuthRefYtsr5yIt = require("./provider-auth-ref-Ytsr5yIt.js");
...
async function ensureApiKeyFromEnvOrPrompt(params) {
  const selectedMode = await (0, _providerAuthModeBTbGLgZT.t)({
    prompter: params.prompter,
    explicitMode: params.secretInputMode
  });
  const env = params.env ?? process.env;
  const envKey = (0, _modelAuthEnvCblHr3q.t)(params.provider, env);
  if (selectedMode === "ref") {
    if (typeof params.prompter.select !== "function") {
      const fallback = (0, _providerAuthRefYtsr5yIt.r)({
        config: params.config,
        provider: params.provider,
        preferredEnvVar: envKey?.source ? (0, _providerAuthRefYtsr5yIt.t)(envKey.source) : void 0,
        env
      });
      await params.setCredential(fallback.ref, selectedMode);
      return fallback.resolvedValue;
    }
    const resolved = await (0, _providerAuthRefYtsr5yIt.n)({
      provider: params.provider,
      config: params.config,
      prompter: params.prompter,
      preferredEnvVar: envKey?.source ? (0, _providerAuthRefYtsr5yIt.t)(envKey.source) : void 0,
