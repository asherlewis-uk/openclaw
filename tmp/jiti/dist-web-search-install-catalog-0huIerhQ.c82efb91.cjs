...
var _enableCzpxjsaG = require("./enable-CzpxjsaG.js");
...
function buildProviderEntry(params) {
  ...
  return {
    ...
    applySelectionConfig: (config) => (0, _enableCzpxjsaG.t)(config, params.pluginId).config,
    createTool: () => null
  };
}
function resolveWebSearchInstallCatalogEntries() {
  const entries = [];
  for (const entry of (0, _officialExternalPluginCatalogDBaj9fiE.a)()) {
    const manifest = (0, _officialExternalPluginCatalogDBaj9fiE.r)(entry);
    const pluginId = normalizeString(manifest?.plugin?.id);
    const install = (0, _officialExternalPluginCatalogDBaj9fiE.c)(entry);
    ...
    for (const provider of manifest.webSearchProviders ?? []) {
      ...
      entries.push({
        pluginId,
        label: (0, _officialExternalPluginCatalogDBaj9fiE.l)(entry),
        install,
