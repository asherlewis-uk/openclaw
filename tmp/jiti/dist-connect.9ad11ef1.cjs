"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.formatConnectAuthority = formatConnectAuthority;exports.openProxyConnectTunnel = openProxyConnectTunnel;var _nodeNet = _interopRequireDefault(require("node:net"));
...
function formatConnectAuthority(targetHost, targetPort) {
  if (!Number.isInteger(targetPort) || targetPort < 1 || targetPort > 65_535) {
    throw new _shared.ProxylineError("INVALID_CONNECT_TARGET", `Invalid CONNECT target port: ${targetPort}`);
  }
  if (!targetHost || INVALID_CONNECT_AUTHORITY_PATTERN.test(targetHost)) {
    throw new _shared.ProxylineError("INVALID_CONNECT_TARGET", "CONNECT target host is empty or unsafe.");
  }
  const unbracketedHost = targetHost.startsWith("[") && targetHost.endsWith("]") ?
  targetHost.slice(1, -1) :
  targetHost;
  if (_nodeNet.default.isIP(unbracketedHost) === 6) {
    return `[${unbracketedHost}]:${targetPort}`;
  }
  if (targetHost.includes("[") || targetHost.includes("]")) {
    throw new _shared.ProxylineError("INVALID_CONNECT_TARGET", "CONNECT target host has invalid brackets.");
  }
  if (targetHost.includes(":") || INVALID_CONNECT_HOST_DELIMITER_PATTERN.test(targetHost)) {
    throw new _shared.ProxylineError("INVALID_CONNECT_TARGET", "CONNECT target host is not a host name.");
  }
...
function connectToProxy(proxy, proxyTls) {
  ...
  if (proxy.protocol === "https:") {
    ...
    const servername = _nodeNet.default.isIP(host) === 0 ? host : undefined;
    return _nodeTls.default.connect({
      ...connectOptions,
  ...
  if (proxy.protocol === "http:") {
    return _nodeNet.default.connect(connectOptions);
  }
  throw new _shared.ProxylineError("UNSUPPORTED_PROXY_PROTOCOL", `CONNECT tunnels support http:// and https:// proxy endpoints: ${proxy.protocol}`);
}
...
function failConnect(proxy, error) {
  ...
  return new _shared.ProxylineError("CONNECT_FAILED", `Proxy CONNECT failed via ${(0, _shared.redactProxyUrl)(proxy)}: ${message}`);
}
async function openProxyConnectTunnel(options) {
