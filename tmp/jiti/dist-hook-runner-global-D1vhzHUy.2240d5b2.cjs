"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.a = resetGlobalHookRunner;exports.c = joinPresentTextSegments;exports.d = formatHookErrorForLog;exports.i = initializeGlobalHookRunner;exports.l = fireAndForgetBoundedHook;exports.n = getGlobalPluginRegistry;exports.o = runGlobalGatewayStopSafely;exports.r = hasGlobalHooks;exports.s = resolveBlockMessage;exports.t = getGlobalHookRunner;exports.u = fireAndForgetHook;var _errorsIxwfrboQ = require("./errors-ixwfrboQ.js");
...
const getFireAndForgetHookState = () => (0, _globalSingletonB2nbp4Tq.n)(Symbol.for("openclaw.fireAndForgetHookState"), () => ({
  active: 0,
  queue: []
}));
...
function formatHookErrorForLog(err) {
  return (replaceLogControlCharacters((0, _errorsIxwfrboQ.i)(err)).replace(/\s+/g, " ").trim() || "unknown error").slice(0, MAX_HOOK_LOG_MESSAGE_LENGTH);
}
function fireAndForgetHook(task, label, logger = _globalsBJh4IgxP.r) {
  task.catch((err) => {
...
function fireAndForgetBoundedHook(task, label, logger = _globalsBJh4IgxP.r, options = {}) {
...
function createHookRunner(registry, options = {}) {
  ...
  const sanitizeHookError = (error) => {
    return (0, _errorsIxwfrboQ.i)(error).split("\n")[0]?.trim() || "unknown error";
  };
  ...
  async function runVoidHook(hookName, event, ctx) {
    ...
    const promises = hooks.map(async (hook) => {
      try {
        ...
        if (timeoutMs) await withHookTimeout(promise, timeoutMs, { unref: true });else
        await promise;
      } catch (err) {
        handleHookError({
          hookName,
  ...
  async function runModifyingHook(hookName, event, ctx, policy = {}) {
    ...
    for (const hook of hooks) try {
      ...
      if (handlerResult !== void 0 && (handlerResult !== null || policy.mergeNullResults)) {
        if (policy.mergeResults) result = policy.mergeResults(result, handlerResult, hook);else
  ...
  */
  function runToolResultPersist(event, ctx) {
    ...
    for (const hook of hooks) try {
      ...
      if (shouldCatchHookErrors("tool_result_persist")) logger?.error(msg);else
  ...
  */
  function runBeforeMessageWrite(event, ctx) {
    ...
    for (const hook of hooks) try {
      ...
      if (shouldCatchHookErrors("before_message_write")) logger?.error(msg);else
      throw new Error(msg, { cause: err });
    }
...
const getState = () => (0, _globalSingletonB2nbp4Tq.n)(hookRunnerGlobalStateKey, () => ({
  hookRunner: null,
  registry: null
}));
const getLog = () => (0, _subsystemCH8Q21Y.t)("plugins");
/**
