"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.a = prepareArchiveDestinationDir;exports.c = resolvePackedRootDir;exports.f = exports.d = void 0;exports.h = isWindowsDrivePath;exports.i = mergeExtractedTreeIntoDestination;exports.m = exports.l = void 0;exports.n = loadZipArchiveWithPreflight;exports.o = withStagedArchiveDestination;exports.p = void 0;exports.r = createTarEntryPreflightChecker;exports.s = resolveArchiveKind;exports.t = extractArchive;exports.u = void 0;var _pathB5B_oAT = require("./path-B5B-_oAT.js");
var _stringCoerceCx5273Cj = require("./string-coerce-Cx5273Cj.js");
var _writeQueueBKRKUV0w = require("./write-queue-BKRKUV0w.js");
var _fsSafeDpJlqO1z = require("./fs-safe-DpJlqO1z.js");
...
function resolveArchiveOutputPath(params) {
  const safeBase = (0, _pathB5B_oAT.l)(params.rootDir);
...
const DEFAULT_MAX_ARCHIVE_BYTES_ZIP = exports.d = 256 * 1024 * 1024;
const DEFAULT_MAX_ENTRIES = exports.f = 5e4;
const DEFAULT_MAX_EXTRACTED_BYTES = exports.m = 512 * 1024 * 1024;
const DEFAULT_MAX_ENTRY_BYTES = exports.p = 256 * 1024 * 1024;
const ARCHIVE_LIMIT_ERROR_CODE = exports.l = {
...
function resolveArchiveKind(filePath) {
  const lower = (0, _stringCoerceCx5273Cj.t)(filePath);
...
async function createDirectoryIdentityGuard(dir) {
  try {
    return await (0, _writeQueueBKRKUV0w.u)(dir);
  } catch (err) {
    if (err instanceof _pathB5B_oAT.p && err.code === "not-file") throw new ArchiveSecurityError("destination-symlink", "archive destination is a symlink");
    throw err;
  }
}
async function assertDirectoryIdentityGuard(guard) {
  try {
    ...
    if (err instanceof _pathB5B_oAT.p) throw new ArchiveSecurityError("destination-symlink-traversal", "archive destination changed during extraction");
    throw err;
  }
}
...
async function assertNoSymlinkTraversal(params) {
  ...
  for (const part of parts) {
    ...
    try {
      ...
      if ((0, _pathB5B_oAT.r)(err)) continue;
      throw err;
    }
...
async function assertResolvedInsideDestination(params) {
  ...
  if (!(0, _pathB5B_oAT.i)(params.destinationRealDir, resolved)) throw symlinkTraversalError(params.originalPath);
}
async function prepareArchiveOutputPath(params) {
  const targetRoot = await (0, _secureTempDirGC3bO7Qi.o)(params.destinationRealDir);
  const destinationGuard = await createDirectoryIdentityGuard(params.destinationRealDir);
  const relPath = params.relPath.split(_nodePath.default.sep).join(_nodePath.default.posix.sep);
  ...
  if (params.isDirectory) {
    await (0, _secureTempDirGC3bO7Qi.s)()?.beforeArchiveOutputMutation?.("mkdir", params.outPath);
  ...
  const parentRel = _nodePath.default.posix.dirname(relPath);
  if (parentRel !== ".") {
    await (0, _secureTempDirGC3bO7Qi.s)()?.beforeArchiveOutputMutation?.("mkdir", _nodePath.default.dirname(params.outPath));
    await assertDirectoryIdentityGuard(destinationGuard);
  ...
  await assertResolvedInsideDestination({
    destinationRealDir: params.destinationRealDir,
    targetPath: _nodePath.default.dirname(params.outPath),
    originalPath: params.originalPath
  });
}
async function chmodInsideDestinationBestEffort(params) {
  await (0, _secureTempDirGC3bO7Qi.s)()?.beforeArchiveOutputMutation?.("chmod", params.destinationPath);
  const destinationGuard = await createDirectoryIdentityGuard(params.destinationRealDir);
  await assertDirectoryIdentityGuard(destinationGuard);
  const noFollowFlag = process.platform !== "win32" && "O_NOFOLLOW" in _nodeFs.default.constants ? _nodeFs.default.constants.O_NOFOLLOW : 0;
  const handle = await _promises.default.open(params.destinationPath, _nodeFs.default.constants.O_RDONLY | noFollowFlag).catch(() => null);
  if (!handle) {
    if ((await _promises.default.lstat(params.destinationPath).catch(() => null))?.isSymbolicLink()) throw symlinkTraversalError(params.originalPath);
    return;
  }
  try {
    const stat = await handle.stat();
    if (!stat.isDirectory() && !stat.isFile()) return;
    const realPath = await (0, _secureTempDirGC3bO7Qi.a)(handle, params.destinationPath);
    if (!(0, _pathB5B_oAT.i)(params.destinationRealDir, realPath)) throw symlinkTraversalError(params.originalPath);
...
async function assertExtractedFileHasNoHardlinkAlias(params) {
  ...
  const stat = await _promises.default.lstat(destinationPath);
...
async function removeExtractedDestinationFile(params) {
  ...
  let resolved;
  try {
    resolved = await _promises.default.realpath(destinationPath);
  } catch {
    return;
  }
  if (!(0, _pathB5B_oAT.i)(params.destinationRealDir, resolved)) return;
  await _promises.default.rm(destinationPath, { force: true }).catch(() => void 0);
}
async function withStagedArchiveDestination(params) {
  const stagingRoot = (0, _secureTempDirGC3bO7Qi.t)({
    fallbackPrefix: "fs-safe-archive",
    unsafeFallbackLabel: "archive staging temp dir",
    warn: () => void 0
  });
  if ((0, _pathB5B_oAT.i)(params.destinationRealDir, stagingRoot)) throw new Error(`archive staging root must be outside destination: ${stagingRoot}`);
  const stagingDir = await _promises.default.mkdtemp(_nodePath.default.join(stagingRoot, params.stagingDirPrefix ?? "fs-safe-archive-"));
  try {
    await _promises.default.chmod(stagingDir, ARCHIVE_STAGING_MODE).catch(() => void 0);
  ...
  const walk = async (currentSourceDir) => {
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        if (err instanceof _pathB5B_oAT.p && (err.code === "hardlink" || err.code === "path-alias")) throw symlinkTraversalError(originalPath);
        throw err;
      }
    }
  };
...
async function importOptionalJsZip() {
  try {
    const module = await Promise.resolve().then(() => jitiImport("jszip").then((m) => _interopRequireWildcard(m)));
...
async function writeZipFileEntry(params) {
  ...
  try {
    await (0, _fsSafeDpJlqO1z.c)({
      dir: _nodePath.default.dirname(destinationPath),
      tempPrefix: `.${_nodePath.default.basename(destinationPath)}.fs-safe-archive`,
      chmodDir: false,
      writeTemp: async (tempPath) => {
        tempHandle = await _promises.default.open(tempPath, OPEN_WRITE_CREATE_FLAGS, 438);
    ...
    if (typeof params.entry.unixPermissions === "number") {
      ...
      if (mode !== 0) await _promises.default.chmod(destinationPath, mode).catch(() => void 0);
    }
  } catch (err) {
    throw err;
  } finally {
...
async function extractZip(params) {
  ...
  await withStagedArchiveDestination({
    ...
    run: async (stagingDir) => {
      const stagingRealDir = await _promises.default.realpath(stagingDir);
      for (const entry of entries) {
...
async function extractArchive(params) {
  ...
  if (kind === "tar") {
    await (0, _fsSafeDpJlqO1z.u)((async () => {
  ...
  await (0, _fsSafeDpJlqO1z.u)(extractZip({
...
async function importOptionalTar() {
  try {
    return await Promise.resolve().then(() => jitiImport("tar").then((m) => _interopRequireWildcard(m)));
  } catch (err) {
