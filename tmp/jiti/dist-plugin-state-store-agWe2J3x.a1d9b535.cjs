"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.i = sweepExpiredPluginStateEntries;exports.n = closePluginStateSqliteStore;exports.r = isPluginStateDatabaseOpen;exports.t = createPluginStateKeyedStore;var _pathsCnwfh6dH = require("./paths-Cnwfh6dH.js");
var _sqliteWalCWnVmfPu = require("./sqlite-wal-CWnVmfPu.js");
...
function resolvePluginStateDir(env = process.env) {
  return _nodePath.default.join((0, _pathsCnwfh6dH.v)(env), "plugin-state");
}
function resolvePluginStateSqlitePath(env = process.env) {
  return _nodePath.default.join(resolvePluginStateDir(env), "state.sqlite");
}
//#endregion
...
function ensurePluginStatePermissions(pathname) {
  ...
  (0, _nodeFs.chmodSync)(dir, PLUGIN_STATE_DIR_MODE);
  for (const suffix of PLUGIN_STATE_SIDECAR_SUFFIXES) {
    const candidate = `${pathname}${suffix}`;
    if ((0, _nodeFs.existsSync)(candidate)) (0, _nodeFs.chmodSync)(candidate, PLUGIN_STATE_FILE_MODE);
  }
}
...
function openPluginStateDatabase(operation = "open") {
  ...
  try {
    ...
    const walMaintenance = (0, _sqliteWalCWnVmfPu.t)(db);
