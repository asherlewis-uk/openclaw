"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.t = buildOauthProviderAuthResult;var _modelInputB9pBobB = require("./model-input-B9p-bobB.js");
...
function normalizeAgentModelConfigForAuthResult(value) {
  if (typeof value === "string") return (0, _modelInputB9pBobB.n)(value);
  if (!value || typeof value !== "object" || Array.isArray(value)) return value;
  let mutated = false;
  const next = { ...value };
  if (typeof next.primary === "string") {
    const primary = (0, _modelInputB9pBobB.n)(next.primary);
  ...
  if (Array.isArray(next.fallbacks)) {
    ...
    const fallbacks = originalFallbacks.map((fallback) => typeof fallback === "string" ? (0, _modelInputB9pBobB.n)(fallback) : fallback);
...
function normalizeProviderConfigModelIdsForAuthResult(provider, providerConfig) {
  ...
  const nextModels = models.map((model) => {
    const id = (0, _modelRefSharedRt54Iaru.r)(provider, model.id);
...
function normalizeProviderAuthConfigPatchModelRefs(patch) {
  ...
  if (defaults) {
    ...
    if (defaults.models) {
      const models = (0, _modelInputB9pBobB.t)(defaults.models);
...
function buildOauthProviderAuthResult(params) {
  ...
  const defaultModel = (0, _modelInputB9pBobB.n)(params.defaultModel);
  return {
    profiles: [{
      profileId: (0, _identityDX9yGpYy.t)({
