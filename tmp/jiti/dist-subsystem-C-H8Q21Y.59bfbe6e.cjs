...
var _console77zKX8_k = require("./console-77zKX8_k.js");
...
function shouldLogToConsole(level, settings) {
  ...
  return (0, _loggerD7Ndr2M._)(level) >= (0, _loggerD7Ndr2M._)(settings.level);
}
const inspectValue = (() => {
...
function formatConsoleLine(opts) {
  ...
  if (opts.style === "json") return (0, _redactCxqQQvmK.s)(JSON.stringify({
    time: (0, _console77zKX8_k.n)("json"),
    level: opts.level,
  ...
  const displayMessage = stripRedundantSubsystemPrefixForConsole((0, _redactCxqQQvmK.s)(opts.message), displaySubsystem);
  return `${[(() => {
    if (opts.style === "pretty") return color.gray((0, _console77zKX8_k.n)("pretty"));
    if (_stateCiZRzmxP.t.consoleTimestampPrefix) return color.gray((0, _console77zKX8_k.n)(opts.style));
    return "";
...
function writeConsoleLine(level, line, opts = {}) {
  ...
  const redacted = opts.redacted ? sanitized : (0, _redactCxqQQvmK.s)(sanitized);
  const sink = _stateCiZRzmxP.t.rawConsole ?? console;
  if (_stateCiZRzmxP.t.forceConsoleToStderr || level === "error" || level === "fatal") (sink.error ?? console.error)(redacted);else
...
function createSubsystemLogger(subsystem) {
  ...
  const emitLog = (level, message, meta) => {
    const consoleSettings = (0, _console77zKX8_k.r)();
    const consoleEnabled = shouldLogToConsole(level, { level: consoleSettings.level }) && (0, _console77zKX8_k.l)(resolvedSubsystem);
    const fileEnabled = (0, _loggerD7Ndr2M.o)(level);
    ...
    if (fileEnabled) logToFile((0, _loggerD7Ndr2M.r)({ subsystem: resolvedSubsystem }), level, message, fileMeta);
  ...
  return {
    ...
    isEnabled(level, target = "any") {
      const isConsoleEnabled = shouldLogToConsole(level, { level: (0, _console77zKX8_k.r)().level }) && (0, _console77zKX8_k.l)(resolvedSubsystem);
      const isFileEnabled = (0, _loggerD7Ndr2M.o)(level);
    ...
    raw(message) {
      if ((0, _loggerD7Ndr2M.o)("info")) logToFile((0, _loggerD7Ndr2M.r)({ subsystem: resolvedSubsystem }), "info", message, { raw: true });
      if (shouldLogToConsole("info", { level: (0, _console77zKX8_k.r)().level }) && (0, _console77zKX8_k.l)(resolvedSubsystem)) {
        if (shouldSuppressProbeConsoleLine({
          level: "info",
          subsystem: resolvedSubsystem,
          message
