...
var _channelTargetBJoc4Exk = require("./channel-target-BJoc4Exk.js");
...
function listBundledChannelIdsWithPersistedAuthState() {
  return (0, _packageStateProbesDT5qTYY.n)("persistedAuthState");
}
...
function listPotentialConfiguredChannelPresenceSignals(cfg, env = process.env, options = {}) {
  ...
  const channelEnvPrefixes = listChannelEnvPrefixes(options.channelIds ?? (0, _bootstrapRegistryBkdH0XC.r)(env));
  const channels = (0, _utilsCKsuXgDI.c)(cfg.channels) ? cfg.channels : null;
  ...
  for (const [key, value] of Object.entries(env)) {
    if (!(0, _channelTargetBJoc4Exk.i)(value)) continue;
