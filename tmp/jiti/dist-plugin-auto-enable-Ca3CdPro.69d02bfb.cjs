...
var _modelRefProfileDLmuIJtf = require("./model-ref-profile-DLmuIJtf.js");
...
var _harnessRuntimesZd_FDjDQ = require("./harness-runtimes-zd_FDjDQ.js");
var _configPresenceC1GfUvRp = require("./config-presence-C1GfUvRp.js");
var _channelConfiguredCoO3CraP = require("./channel-configured-CoO3CraP.js");
var _setupRegistryBGTB8int = require("./setup-registry-BGTB8int.js");
var _pluginsAllowlistCC0TQMua = require("./plugins-allowlist-CC0TQMua.js");
...
function loadProviderRegistrySnapshot(params) {
  ...
  return (0, _pluginRegistryBZohWtpt.p)({
...
function loadScopedProviderRegistry(params) {
  return {
    ...
    onlyPluginIdSet: (0, _pluginScopeCEsy4RJw.t)(params.onlyPluginIds)
  };
}
...
function resolveProviderOwnerPluginIds(params) {
  ...
  const normalizedConfig = (0, _pluginRegistryBZohWtpt.r)(params.config?.plugins, registry, { manifestRegistry: params.manifestRegistry });
...
function toManifestOwnerRecord(plugin) {
  return {
    ...
    enabledByDefault: (0, _installedPluginIndexStoreDEo2ZAhx.g)(plugin)
  };
}
function withBundledProviderVitestCompat(params) {
  return (0, _bundledCompatDMyny7PJ.r)(params);
}
function resolveBundledProviderCompatPluginIds(params) {
  if (params.manifestRegistry) {
    const onlyPluginIdSet = (0, _pluginScopeCEsy4RJw.t)(params.onlyPluginIds);
...
function isProviderPluginEligibleForSetupDiscovery(params) {
  ...
  return (0, _manifestOwnerPolicyCNc1CXKa.n)({
    plugin: toManifestOwnerRecord(params.plugin),
    normalizedConfig: params.normalizedConfig,
    rootConfig: params.rootConfig
  });
...
function stripModelProfileSuffix(value) {
  return (0, _modelRefProfileDLmuIJtf.t)(value).model;
}
function splitExplicitModelRef(rawModel) {
  ...
  const provider = (0, _providerIdCz7K6wgK.r)(trimmed.slice(0, slash));
...
function resolveOwningPluginIdsForProvider(params) {
  ...
  if (params.manifestRegistry) {
    const pluginIds = params.manifestRegistry.plugins.filter((plugin) => plugin.providers.some((providerId) => (0, _providerIdCz7K6wgK.r)(providerId) === normalizedProvider) || plugin.cliBackends.some((backendId) => (0, _providerIdCz7K6wgK.r)(backendId) === normalizedProvider)).map((plugin) => plugin.id);
    return pluginIds.length > 0 ? pluginIds : void 0;
  }
  const env = params.env ?? process.env;
  const deduped = dedupeSortedPluginIds([...(0, _pluginRegistryBZohWtpt.c)({
    config: params.config,
    workspaceDir: params.workspaceDir,
    env,
    providerId: normalizedProvider,
    includeDisabled: true
  }), ...(0, _pluginRegistryBZohWtpt.s)({
    config: params.config,
    workspaceDir: params.workspaceDir,
    env,
    contribution: "cliBackends",
    matches: (backendId) => (0, _providerIdCz7K6wgK.r)(backendId) === normalizedProvider,
    includeDisabled: true
  })]);
...
function resolveCatalogHookProviderPluginIds(params) {
  ...
  const normalizedConfig = (0, _pluginRegistryBZohWtpt.r)(params.config?.plugins, registry);
...
function splitEnvPaths(value) {
  ...
  if (!trimmed) return [];
  return (0, _stringNormalizationDgUPESoD.s)(trimmed.split(/[;,]/g).flatMap((chunk) => chunk.split(_nodePath.default.delimiter)));
}
function resolveExternalCatalogPaths(env) {
  for (const key of ENV_CATALOG_PATHS) {
    const raw = (0, _stringCoerceLndEvhRk.c)(env[key]);
    if (raw) return splitEnvPaths(raw);
  }
  const configDir = (0, _utilsCKsuXgDI.d)(env);
  return [
...
function parseExternalCatalogChannelEntries(raw) {
  ...
  for (const entry of list) {
    if (!(0, _utilsCKsuXgDI.c)(entry) || !(0, _utilsCKsuXgDI.c)(entry.openclaw) || !(0, _utilsCKsuXgDI.c)(entry.openclaw.channel)) continue;
    const channel = entry.openclaw.channel;
    const id = (0, _stringCoerceLndEvhRk.c)(channel.id) ?? "";
    if (!id) continue;
...
function resolveExternalCatalogPreferOver(channelId, env) {
  for (const rawPath of resolveExternalCatalogPaths(env)) {
    const resolved = (0, _utilsCKsuXgDI.p)(rawPath, env);
...
function resolveBuiltInChannelPreferOver(channelId) {
  const builtInChannelId = (0, _idsCRqUrRLW.r)(channelId);
  if (!builtInChannelId) return [];
  return (0, _chatMetaBtJ8Pxrm.t)(builtInChannelId)?.preferOver ?? [];
}
...
function canReuseUnscopedCurrentPluginMetadataSnapshot(config) {
  return (0, _configStateCOhlckC.s)(config.plugins).loadPaths.length === 0;
}
function extractProviderFromModelRef(value) {
  const trimmed = value.trim();
  const slash = trimmed.indexOf("/");
  if (slash <= 0) return null;
  return (0, _providerIdCz7K6wgK.r)(trimmed.slice(0, slash));
}
function hasConfiguredEmbeddedHarnessRuntime(cfg, env) {
  return (0, _harnessRuntimesZd_FDjDQ.t)(cfg, env, { includeEnvRuntime: false }).length > 0;
}
function resolveAgentHarnessOwnerPluginIds(registry, runtime) {
  const normalizedRuntime = (0, _stringCoerceLndEvhRk.s)(runtime);
  if (!normalizedRuntime) return [];
  return registry.plugins.filter((plugin) => [...(plugin.activation?.onAgentHarnesses ?? []), ...(plugin.cliBackends ?? [])].some((entry) => (0, _stringCoerceLndEvhRk.s)(entry) === normalizedRuntime)).map((plugin) => plugin.id).toSorted((left, right) => left.localeCompare(right));
}
function isProviderConfigured(cfg, providerId) {
  ...
  if (profiles && typeof profiles === "object") for (const profile of Object.values(profiles)) {
    if (!(0, _utilsCKsuXgDI.c)(profile)) continue;
    if ((0, _providerIdCz7K6wgK.r)(profile.provider ?? "") === normalized) return true;
  }
  const providerConfig = cfg.models?.providers;
  if (providerConfig && typeof providerConfig === "object") {
    for (const key of Object.keys(providerConfig)) if ((0, _providerIdCz7K6wgK.r)(key) === normalized) return true;
  }
  for (const { value: ref } of (0, _modelRefsByh1O5l.n)(cfg, { includeChannelModelOverrides: false })) {
...
function hasPluginOwnedWebSearchConfig(cfg, pluginId) {
  ...
  return (0, _utilsCKsuXgDI.c)(pluginConfig) && (0, _utilsCKsuXgDI.c)(pluginConfig.webSearch);
}
function hasPluginOwnedWebFetchConfig(cfg, pluginId) {
  const pluginConfig = cfg.plugins?.entries?.[pluginId]?.config;
  return (0, _utilsCKsuXgDI.c)(pluginConfig) && (0, _utilsCKsuXgDI.c)(pluginConfig.webFetch);
}
function resolvePluginOwnedToolConfigKeys(plugin) {
  if ((plugin.contracts?.tools?.length ?? 0) === 0) return [];
  const properties = (0, _utilsCKsuXgDI.c)(plugin.configSchema) ? plugin.configSchema.properties : void 0;
  if (!(0, _utilsCKsuXgDI.c)(properties)) return [];
...
function hasPluginOwnedToolConfig(cfg, plugin) {
  ...
  if (!(0, _utilsCKsuXgDI.c)(pluginConfig)) return false;
...
function resolvePluginIdForConfiguredWebFetchProvider(providerId, registry) {
  const normalizedProviderId = (0, _stringCoerceLndEvhRk.s)(providerId);
  if (!normalizedProviderId) return;
  return registry.plugins.find((plugin) => plugin.origin === "bundled" && (plugin.contracts?.webFetchProviders ?? []).some((candidate) => (0, _stringCoerceLndEvhRk.s)(candidate) === normalizedProviderId))?.id;
}
function resolvePluginIdForConfiguredWebSearchProvider(providerId, registry) {
  const normalizedProviderId = (0, _stringCoerceLndEvhRk.s)(providerId);
  if (!normalizedProviderId) return;
  return registry.plugins.find((plugin) => (plugin.contracts?.webSearchProviders ?? []).some((candidate) => (0, _stringCoerceLndEvhRk.s)(candidate) === normalizedProviderId))?.id;
}
function normalizeManifestChannelId(channelId) {
  return (0, _idsCRqUrRLW.r)(channelId) ?? channelId;
}
...
function collectPluginIdsForConfiguredChannel(channelId, registry) {
  ...
  const builtInId = (0, _idsCRqUrRLW.r)(normalizedChannelId);
  const claims = [];
...
function collectConfiguredChannelIds(cfg, env) {
  const configuredStateChannelIds = new Set((0, _channelConfiguredCoO3CraP.i)());
  return (0, _configPresenceC1GfUvRp.a)(cfg, env, { includePersistedAuthState: false }).map((signal) => ({
    source: signal.source,
    channelId: (0, _idsCRqUrRLW.r)(signal.channelId) ?? signal.channelId
...
function isAutoEnableConfiguredChannelSignal(params) {
  if (params.source === "env" && params.configuredStateChannelIds.has(params.channelId) && !(0, _channelConfiguredCoO3CraP.r)({
    channelId: params.channelId,
    cfg: params.cfg,
    env: params.env
  })) return false;
  return (0, _channelConfiguredCoO3CraP.t)(params.cfg, params.channelId, params.env);
}
function hasConfiguredWebSearchPluginEntry(cfg) {
  const entries = cfg.plugins?.entries;
  return !!entries && typeof entries === "object" && Object.values(entries).some((entry) => (0, _utilsCKsuXgDI.c)(entry) && (0, _utilsCKsuXgDI.c)(entry.config) && (0, _utilsCKsuXgDI.c)(entry.config.webSearch));
}
...
function hasConfiguredWebFetchPluginEntry(cfg) {
  const entries = cfg.plugins?.entries;
  return !!entries && typeof entries === "object" && Object.values(entries).some((entry) => (0, _utilsCKsuXgDI.c)(entry) && (0, _utilsCKsuXgDI.c)(entry.config) && (0, _utilsCKsuXgDI.c)(entry.config.webFetch));
}
function hasConfiguredPluginConfigEntry(cfg) {
  const entries = cfg.plugins?.entries;
  return !!entries && typeof entries === "object" && Object.values(entries).some((entry) => (0, _utilsCKsuXgDI.c)(entry) && (0, _utilsCKsuXgDI.c)(entry.config));
}
function listContainsNormalized(value, expected) {
  return Array.isArray(value) && value.some((entry) => (0, _stringCoerceLndEvhRk.s)(entry) === expected);
}
function toolPolicyReferencesBrowser(value) {
  return (0, _utilsCKsuXgDI.c)(value) && (listContainsNormalized(value.allow, "browser") || listContainsNormalized(value.alsoAllow, "browser"));
}
function hasBrowserToolReference(cfg) {
  if (toolPolicyReferencesBrowser(cfg.tools)) return true;
  const agentList = cfg.agents?.list;
  return Array.isArray(agentList) ? agentList.some((entry) => (0, _utilsCKsuXgDI.c)(entry) && toolPolicyReferencesBrowser(entry.tools)) : false;
}
...
function hasBrowserSetupAutoEnableRelevantConfig(cfg) {
  ...
  if ((0, _utilsCKsuXgDI.c)(cfg.browser)) return true;
...
function hasAcpxSetupAutoEnableRelevantConfig(cfg) {
  ...
  if (!(0, _utilsCKsuXgDI.c)(cfg.acp)) return false;
  const backend = (0, _stringCoerceLndEvhRk.s)(cfg.acp.backend);
  return (cfg.acp.enabled === true || (0, _utilsCKsuXgDI.c)(cfg.acp.dispatch) && cfg.acp.dispatch.enabled === true || backend === "acpx") && (!backend || backend === "acpx");
}
function hasXaiSetupAutoEnableRelevantConfig(cfg) {
  ...
  return (0, _utilsCKsuXgDI.c)(pluginConfig) && ((0, _utilsCKsuXgDI.c)(pluginConfig.xSearch) || (0, _utilsCKsuXgDI.c)(pluginConfig.codeExecution)) || (0, _utilsCKsuXgDI.c)(cfg.tools?.web) && (0, _utilsCKsuXgDI.c)(cfg.tools.web.x_search);
}
...
function hasConfiguredProviderModelOrHarness(cfg, env) {
  ...
  if ((0, _modelRefsByh1O5l.n)(cfg, { includeChannelModelOverrides: false }).length > 0) return true;
...
function resolvePluginAutoEnableReadiness(cfg, env) {
  ...
  return {
    mayNeedAutoEnable: (0, _setupRegistryBGTB8int.n)({
      config: cfg,
      env,
...
function resolveConfiguredPluginAutoEnableCandidates(params) {
  ...
  for (const { value: modelRef } of (0, _modelRefsByh1O5l.n)(params.config, { includeChannelModelOverrides: false })) {
  ...
  for (const runtime of (0, _harnessRuntimesZd_FDjDQ.t)(params.config, params.env, { includeEnvRuntime: false })) {
  ...
  if (webSearchPluginId) changes.push({
    ...
    providerId: (0, _stringCoerceLndEvhRk.s)(webSearchProvider) ?? ""
  });
  ...
  if (webFetchPluginId) changes.push({
    ...
    providerId: (0, _stringCoerceLndEvhRk.s)(webFetchProvider) ?? ""
  });
  ...
  if (hasSetupAutoEnableRelevantConfig(params.config)) {
    ...
    for (const entry of (0, _setupRegistryBGTB8int.n)({
      config: params.config,
      env: params.env,
...
function disableImplicitPreferredOverPlugin(params) {
  ...
  if (!(0, _idsCRqUrRLW.r)(params.pluginId) && !isKnownPluginId(params.pluginId, params.manifestRegistry)) return params.config;
...
function resolveAutoEnableChannelId(params) {
  const builtInChannelId = (0, _idsCRqUrRLW.r)(params.entry.pluginId);
...
function hasMaterialPluginEntryConfig(entry) {
  if (!(0, _utilsCKsuXgDI.c)(entry)) return false;
  return entry.enabled === true || (0, _utilsCKsuXgDI.c)(entry.config) || (0, _utilsCKsuXgDI.c)(entry.hooks) || (0, _utilsCKsuXgDI.c)(entry.subagent) || (0, _utilsCKsuXgDI.c)(entry.llm) || entry.apiKey !== void 0 || entry.env !== void 0;
}
function isKnownPluginId(pluginId, manifestRegistry) {
  if ((0, _idsCRqUrRLW.r)(pluginId)) return true;
...
function materializeConfiguredPluginEntryAllowlist(params) {
  ...
  for (const pluginId of Object.keys(entries).toSorted((left, right) => left.localeCompare(right))) {
    ...
    next = (0, _pluginsAllowlistCC0TQMua.t)(next, pluginId);
...
function resolveChannelAutoEnableDisplayLabel(entry, manifestRegistry) {
  const builtInChannelId = (0, _idsCRqUrRLW.r)(entry.channelId);
  const plugin = manifestRegistry.plugins.find((record) => record.id === entry.pluginId);
  return (builtInChannelId ? (0, _chatMetaBtJ8Pxrm.t)(builtInChannelId)?.label : void 0) ?? plugin?.channelConfigs?.[entry.channelId]?.label ?? plugin?.channelCatalogMeta?.label;
}
...
function resolvePluginAutoEnableManifestRegistry(params) {
  ...
  return ((0, _pluginRegistryBZohWtpt.v)({
    config: params.config,
    env: params.env,
    allowWorkspaceScopedSnapshot: true
  }) ?? (() => {
    if (!canReuseUnscopedCurrentPluginMetadataSnapshot(params.config)) return;
    const snapshot = (0, _pluginRegistryBZohWtpt.v)({
      env: params.env,
      allowWorkspaceScopedSnapshot: true,
      requireDefaultDiscoveryContext: true
    });
    return snapshot?.policyHash === (0, _installedPluginIndexStoreDEo2ZAhx._)(params.config) ? snapshot : void 0;
  })())?.manifestRegistry ?? (0, _pluginMetadataSnapshotYo9W2SZ.i)({
    config: params.config,
    env: params.env
  }).manifestRegistry;
}
function materializePluginAutoEnableCandidatesInternal(params) {
  ...
  for (const entry of params.candidates) {
    ...
    next = (0, _pluginsAllowlistCC0TQMua.t)(next, entry.pluginId);
  ...
  for (const [pluginId, reasons] of autoEnabledReasons) if (!(0, _prototypeKeysCxs5UffD.t)(pluginId)) autoEnabledReasonRecord[pluginId] = [...reasons];
  return {
    config: next,
    changes,
