"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.A = shouldBootstrapFromExternalCliCredential;exports.C = readExternalCliFallbackCredential;exports.D = hasUsableOAuthCredential;exports.E = hasMatchingOAuthIdentity;exports.F = resolveTokenExpiryState;exports.I = readClaudeCliCredentialsCached;exports.L = readCodexCliCredentialsCached;exports.M = void 0;exports.N = evaluateStoredCredentialEligibility;exports.O = isSafeToAdoptBootstrapOAuthIdentity;exports.P = hasUsableOAuthCredential$1;exports.R = readGeminiCliCredentialsCached;exports.S = ensureAuthStoreFile;exports.T = areOAuthCredentialsEquivalent;exports._ = isLegacyOAuthRef;exports.a = findPersistedAuthProfileCredential;exports.b = loadLegacyOAuthSidecarMaterial;exports.c = loadAuthProfileStoreForSecretsRuntime;exports.d = resolvePersistedAuthProfileOwnerAgentDir;exports.f = saveAuthProfileStore;exports.g = loadPersistedAuthProfileStore;exports.h = coercePersistedAuthProfileStore;exports.i = ensureAuthProfileStoreWithoutExternalProfiles;exports.j = shouldReplaceStoredOAuthCredential;exports.k = isSafeToAdoptMainStoreOAuthIdentity;exports.l = loadAuthProfileStoreWithoutExternalProfiles;exports.m = buildPersistedAuthProfileSecretsStore;exports.n = ensureAuthProfileStore;exports.o = loadAuthProfileStore;exports.p = updateAuthProfileStoreWithLock;exports.r = ensureAuthProfileStoreForLocalUpdate;exports.s = loadAuthProfileStoreForRuntime;exports.t = clearRuntimeAuthProfileStoreSnapshots;exports.u = replaceRuntimeAuthProfileStoreSnapshots;exports.v = isLegacyOAuthSidecarPayload;exports.w = void 0;exports.x = resolveLegacyOAuthSidecarPath;exports.y = void 0;var _stringCoerceLndEvhRk = require("./string-coerce-LndEvhRk.js");
var _pathsCnwfh6dH = require("./paths-Cnwfh6dH.js");
require("./errors-ixwfrboQ.js");
...
require("./source-check-D7EVFd4C.js");
...
const log = (0, _subsystemCH8Q21Y.t)("agents/auth-profiles");
...
function readPortalCliOauthCredentials(credPath, provider) {
  const raw = (0, _jsonFileDIl8If_.t)(credPath);
...
function readGeminiCliCredentials(options) {
  const raw = (0, _jsonFileDIl8If_.t)(resolveGeminiCliCredentialsPath(options?.homeDir));
  if (!raw || typeof raw !== "object") return null;
  const data = raw;
  const accessToken = data.access_token;
  const refreshToken = data.refresh_token;
...
function readClaudeCliCredentials(options) {
  ...
  const raw = (0, _jsonFileDIl8If_.t)(resolveClaudeCliCredentialsPath(options?.homeDir));
  if (!raw || typeof raw !== "object") return null;
...
function readCodexCliCredentials(options) {
  ...
  const raw = (0, _jsonFileDIl8If_.t)(authPath);
  if (!raw || typeof raw !== "object") return null;
...
const DEFAULT_OAUTH_REFRESH_MARGIN_MS = exports.M = 300 * 1e3;
...
function hasConfiguredSecretString(value) {
  return (0, _typesSecretsCJS3n8Im.d)(value) !== void 0;
}
function evaluateStoredCredentialEligibility(params) {
  ...
  if ((0, _typesSecretsCJS3n8Im.d)(credential.access) === void 0 && (0, _typesSecretsCJS3n8Im.d)(credential.refresh) === void 0) return {
    eligible: false,
    reasonCode: "missing_credential"
  };
  return {
...
function overlayRuntimeExternalOAuthProfiles(store, profiles) {
  ...
  const next = (0, _runtimeSnapshotsBSv4EtU.g)(store);
...
{
  profileId: _storeCacheBCuMtdkq.d,
  provider: "openai-codex",
  aliases: [
  "codex",
  "codex-cli",
  ...
  readCredentials: (options) => readCodexCliCredentialsCached({
    ttlMs: _storeCacheBCuMtdkq.s,
    allowKeychainPrompt: options?.allowKeychainPrompt
  }),
  bootstrapOnly: true
},
{
  profileId: _storeCacheBCuMtdkq.a,
  provider: "claude-cli",
...
{
  profileId: _storeCacheBCuMtdkq.c,
  provider: "minimax-portal",
  aliases: ["minimax", "minimax-cli"],
  readCredentials: () => readMiniMaxCliCredentialsCached({ ttlMs: _storeCacheBCuMtdkq.s })
}];
...
const readManagedExternalCliCredential = exports.w = readExternalCliBootstrapCredential;
...
function resolveExternalCliAuthProfiles(store, options) {
  ...
  for (const providerConfig of EXTERNAL_CLI_SYNC_PROVIDERS) {
    ...
    if (existing && !existingOAuth) {
      _storeCacheBCuMtdkq.f.debug("kept explicit local auth over external cli bootstrap", {
    ...
    if (providerConfig.bootstrapOnly && existingOAuth && hasInlineOAuthTokenMaterial$1(existingOAuth)) {
      _storeCacheBCuMtdkq.f.debug("kept local oauth over external cli bootstrap-only provider", {
        profileId: providerConfig.profileId,
        provider: providerConfig.provider
      });
      continue;
    ...
    if (existingOAuth && !isSafeToUseExternalCliCredential(existingOAuth, creds)) {
      _storeCacheBCuMtdkq.f.warn("refused external cli oauth bootstrap: identity mismatch", {
        profileId: providerConfig.profileId,
        provider: providerConfig.provider
      });
      continue;
    ...
    if (existingOAuth && !isSafeToAdoptBootstrapOAuthIdentity(existingOAuth, creds) && !areOAuthCredentialsEquivalent(existingOAuth, creds)) {
      _storeCacheBCuMtdkq.f.warn("refused external cli oauth bootstrap: identity mismatch or missing binding", {
        profileId: providerConfig.profileId,
        provider: providerConfig.provider
      });
      continue;
    ...
    if (!shouldBootstrapFromExternalCliCredential({
      ...
      if (existingOAuth) _storeCacheBCuMtdkq.f.debug("kept usable local oauth over external cli bootstrap", {
        profileId: providerConfig.profileId,
        provider: providerConfig.provider,
    ...
    _storeCacheBCuMtdkq.f.debug("used external cli oauth bootstrap because local oauth was missing or unusable", {
      profileId: providerConfig.profileId,
      provider: providerConfig.provider,
      localExpires: existingOAuth?.expires,
      externalExpires: creds.expires
...
function resolveExternalAuthProfileMap(params) {
  ...
  const profiles = (resolveExternalAuthProfilesForRuntime ?? _providerRuntimeO0ZjFTlv.S)({
    env,
...
function hasPersistableExternalCliSyncCandidate(store, params) {
  ...
  for (const profileId of [_storeCacheBCuMtdkq.a, _storeCacheBCuMtdkq.c]) if (store.profiles[profileId]?.type === "oauth") return true;
  return false;
}
...
function syncPersistedExternalCliAuthProfiles(store, params) {
  ...
  for (const profile of persistedProfiles) {
    ...
    next ??= (0, _runtimeSnapshotsBSv4EtU.g)(store);
...
function resolveLegacyOAuthSidecarPath(ref, env = process.env) {
  return _nodePath.default.join((0, _pathsCnwfh6dH.g)(env), LEGACY_OAUTH_SECRET_DIRNAME, `${ref.id}.json`);
}
...
function loadLegacyOAuthSidecarMaterial(params) {
  const env = params.env ?? process.env;
  const raw = (0, _jsonFileDIl8If_.t)(resolveLegacyOAuthSidecarPath(params.ref, env));
  if (!isRecord$2(raw)) return null;
...
function normalizeAuthProfileOrder(raw) {
  ...
  const normalized = Object.entries(raw).reduce((acc, [provider, value]) => {
    ...
    const providerKey = (0, _providerIdCz7K6wgK.r)(provider);
...
function normalizeLastGood(raw) {
  ...
  for (const [provider, profileId] of Object.entries(raw)) {
    const providerKey = (0, _providerIdCz7K6wgK.r)(provider);
    const normalizedProfileId = (0, _stringCoerceLndEvhRk.c)(profileId);
...
function normalizeUsageStatsEntry(raw) {
  ...
  const stats = {
    ...
    blockedModel: (0, _stringCoerceLndEvhRk.c)(raw.blockedModel),
    ...
    cooldownModel: (0, _stringCoerceLndEvhRk.c)(raw.cooldownModel),
...
function loadPersistedAuthProfileState(agentDir) {
  return coerceAuthProfileState((0, _jsonFileDIl8If_.t)((0, _runtimeSnapshotsBSv4EtU.s)(agentDir)));
}
...
function savePersistedAuthProfileState(store, agentDir) {
  ...
  const statePath = (0, _runtimeSnapshotsBSv4EtU.s)(agentDir);
  if (!payload) {
    try {
  ...
  (0, _jsonFileDIl8If_.n)(statePath, payload);
  return payload;
}
//#endregion
...
function normalizeSecretBackedField(params) {
  ...
  if (ref && !(0, _typesSecretsCJS3n8Im.o)(params.entry[params.refField])) params.entry[params.refField] = ref;
  delete params.entry[params.valueField];
}
function normalizeCommonCredentialFields(entry) {
  const normalized = { provider: typeof entry.provider === "string" ? (0, _providerIdCz7K6wgK.r)(entry.provider) : "" };
...
function normalizeRawCredentialEntry(raw) {
  ...
  if (entry.type === "api_key") {
    ...
    const keyRef = (0, _typesSecretsCJS3n8Im.o)(entry.keyRef);
  ...
  if (entry.type === "token") {
    ...
    const tokenRef = (0, _typesSecretsCJS3n8Im.o)(entry.tokenRef);
...
function parseCredentialEntry(raw, fallbackProvider) {
  ...
  const normalizedProvider = typeof provider === "string" ? (0, _providerIdCz7K6wgK.r)(provider) : "";
...
function warnRejectedCredentialEntries(source, rejected) {
  ...
  _storeCacheBCuMtdkq.f.warn("ignored invalid auth profile entries during store load", {
    source,
...
function resolveLegacyOAuthSidecarCredential(params) {
  if (params.credential.type !== "oauth" || (0, _providerIdCz7K6wgK.r)(params.credential.provider) !== LEGACY_OAUTH_REF_PROVIDER || hasInlineOAuthTokenMaterial(params.credential) || !isRecord(params.raw) || !isLegacyOAuthRef(params.raw.oauthRef)) return params.credential;
...
function preserveLegacyOAuthRefsForDoctorMigration(payload, options) {
  ...
  for (const [profileId, rawProfile] of Object.entries(existingRaw.profiles)) {
    ...
    if (credential?.type !== "oauth" || (0, _providerIdCz7K6wgK.r)(credential.provider) !== LEGACY_OAUTH_REF_PROVIDER) continue;
...
function mergeOAuthFileIntoStore(store) {
  const oauthRaw = (0, _jsonFileDIl8If_.t)((0, _pathsCnwfh6dH._)());
...
function loadLegacyAuthProfileStore(agentDir) {
  return coerceLegacyAuthStore((0, _jsonFileDIl8If_.t)((0, _runtimeSnapshotsBSv4EtU.d)(agentDir)));
}
//#endregion
...
function isInheritedMainOAuthCredential(params) {
  ...
  if ((0, _runtimeSnapshotsBSv4EtU.l)(params.agentDir) === (0, _runtimeSnapshotsBSv4EtU.l)()) return false;
...
function resolveRuntimeAuthProfileStore(agentDir, options) {
  const mainKey = (0, _runtimeSnapshotsBSv4EtU.l)(void 0);
  const requestedKey = (0, _runtimeSnapshotsBSv4EtU.l)(agentDir);
  const mainStore = (0, _runtimeSnapshotsBSv4EtU.n)(void 0);
  const requestedStore = (0, _runtimeSnapshotsBSv4EtU.n)(agentDir);
...
function maybeSyncPersistedExternalCliAuthProfiles(params) {
  ...
  const authPath = (0, _runtimeSnapshotsBSv4EtU.l)(params.agentDir);
  const release = acquireAuthStoreLockSync(authPath);
  if (!release) {
    _storeCacheBCuMtdkq.f.warn("skipped persisted external cli auth sync because auth store is locked", { authPath });
    return {
      store: params.store,
      cacheable: false
    };
  ...
  try {
    ...
    for (const [profileId, credential] of changedProfiles) {
      ...
      if (!(0, _nodeUtil.isDeepStrictEqual)(latest, previous)) {
        _storeCacheBCuMtdkq.f.debug("skipped persisted external cli auth sync for concurrently changed profile", { profileId });
        continue;
      }
...
function buildLocalAuthProfileStoreForSave(params) {
  const localStore = (0, _runtimeSnapshotsBSv4EtU.g)(params.store);
...
async function updateAuthProfileStoreWithLock(params) {
  ...
  try {
    return await (0, _fileLockCCOJxG.o)(authPath, _storeCacheBCuMtdkq.i, async () => {
...
function loadAuthProfileStoreForAgent(agentDir, options) {
  ...
  if (!readOnly) {
    const cached = (0, _storeCacheBCuMtdkq.n)({
      authPath,
      authMtimeMs,
      stateMtimeMs
    });
  ...
  if (asStore) {
    ...
    if (!readOnly && synced.cacheable) (0, _storeCacheBCuMtdkq.r)({
      authPath,
  ...
  if (shouldWrite && legacy !== null) {
    const legacyPath = (0, _runtimeSnapshotsBSv4EtU.d)(agentDir);
    try {
      _nodeFs.default.unlinkSync(legacyPath);
    } catch (err) {
      if (err?.code !== "ENOENT") _storeCacheBCuMtdkq.f.warn("failed to delete legacy auth.json after migration", {
        err,
        legacyPath
      });
    }
...
function findPersistedAuthProfileCredential(params) {
  ...
  if ((0, _runtimeSnapshotsBSv4EtU.l)(params.agentDir) === (0, _runtimeSnapshotsBSv4EtU.l)()) return requestedProfile;
...
function resolvePersistedAuthProfileOwnerAgentDir(params) {
  ...
  if ((0, _runtimeSnapshotsBSv4EtU.l)(params.agentDir) === (0, _runtimeSnapshotsBSv4EtU.l)()) return;
...
function replaceRuntimeAuthProfileStoreSnapshots(entries) {
  (0, _runtimeSnapshotsBSv4EtU.a)(entries);
}
function clearRuntimeAuthProfileStoreSnapshots() {
  (0, _runtimeSnapshotsBSv4EtU.t)();
  (0, _storeCacheBCuMtdkq.t)();
}
function saveAuthProfileStore(store, agentDir, options) {
  ...
  (0, _jsonFileDIl8If_.n)(authPath, buildPersistedAuthProfileSecretsStore(localStore, void 0, {
    existingRaw: (0, _jsonFileDIl8If_.t)(authPath),
    runtimeLegacyOAuthSidecarProfileIds
  }));
  savePersistedAuthProfileState(localStore, agentDir);
  (0, _storeCacheBCuMtdkq.r)({
    authPath,
    authMtimeMs: readAuthStoreMtimeMs(authPath),
    stateMtimeMs: readAuthStoreMtimeMs(statePath),
    store: localStore
  });
  if ((0, _runtimeSnapshotsBSv4EtU.i)(agentDir)) (0, _runtimeSnapshotsBSv4EtU.o)(localStore, agentDir);
}
//#endregion /* v9-8d863cd55ca5e441 */
