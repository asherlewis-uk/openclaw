"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.i = createNpmProjectInstallEnv;exports.n = applyPosixNpmScriptShellEnv;exports.r = createNpmFreshnessBypassArgs;exports.t = applyNpmFreshnessBypassEnv;var _nodeFs = _interopRequireDefault(require("node:fs"));
...
function resolveHomeNpmrc(env) {
  const home = env.HOME?.trim() || env.USERPROFILE?.trim() || _nodeOs.default.homedir();
  return _nodePath.default.join(home, ".npmrc");
}
...
function resolveNpmConfigPath(rawPath, env) {
  ...
  const home = env.HOME?.trim() || env.USERPROFILE?.trim() || _nodeOs.default.homedir();
  return (process.platform === "win32" ? /^~(\/|\\)/u : /^~\//u).test(expanded) && home ? _nodePath.default.resolve(home, expanded.slice(2)) : _nodePath.default.resolve(expanded);
}
...
function readNpmGlobalConfigPath(env, scope) {
  try {
    const raw = (0, _nodeChild_process.execFileSync)("npm", [
    "config",
    "get",
    "globalconfig"],
    {
...
function resolveScopedProjectNpmrc(scope) {
  ...
  return cwd ? _nodePath.default.join(cwd, ".npmrc") : null;
}
function resolveScopedGlobalNpmrc(scope) {
  const prefix = scope.npmConfigPrefix?.trim();
  return prefix ? _nodePath.default.join(prefix, "etc", "npmrc") : null;
}
function resolveNpmConfigFiles(env, scope = {}) {
  ...
  return [...new Set(files.filter((file) => Boolean(file)))];
}
...
function applyNpmFreshnessBypassEnv(env, now = /* @__PURE__ */new Date(), scope = {}) {
  ...
  for (const key of NPM_FRESHNESS_BYPASS_KEYS) env[key] = "";
  if (arg?.startsWith("--before=")) env.npm_config_before = arg.slice(9);else
  if (arg === "--min-release-age=0") env.npm_config_min_release_age = "0";
}
...
function resolvePosixNpmScriptShell(env) {
  ...
  if (_nodeFs.default.existsSync("/bin/sh")) return "/bin/sh";
  const shell = env.SHELL?.trim();
  return shell && _nodePath.default.isAbsolute(shell) && _nodeFs.default.existsSync(shell) ? shell : null;
}
