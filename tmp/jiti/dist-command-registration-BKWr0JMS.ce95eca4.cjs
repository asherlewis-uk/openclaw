"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.A = listRegisteredPluginCommands;exports.C = claimPluginInteractiveCallbackDedupe;exports.D = clearPluginCommandsForPlugin;exports.E = clearPluginCommands;exports.M = restorePluginCommands;exports.N = setPluginCommandRegistryLocked;exports.O = isTrustedReservedCommandOwner;exports.S = restorePluginInteractiveHandlers;exports.T = releasePluginInteractiveCallbackDedupe;exports._ = clearPluginInteractiveHandlers;exports.a = validateCommandName;exports.b = registerPluginInteractiveHandler;exports.h = exports.g = exports.f = exports.d = exports.c = void 0;exports.i = registerPluginCommand;exports.j = void 0;exports.k = listRegisteredPluginAgentPromptGuidance;exports.m = exports.l = void 0;exports.n = listPluginInvocationKeys;exports.o = validatePluginCommandDefinition;exports.p = void 0;exports.r = pluginCommandSupportsChannel;exports.s = void 0;exports.t = isReservedCommandName;exports.u = void 0;exports.v = clearPluginInteractiveHandlersForPlugin;exports.w = commitPluginInteractiveCallbackDedupe;exports.x = resolvePluginInteractiveNamespaceMatch;exports.y = listPluginInteractiveHandlers;var _stringCoerceLndEvhRk = require("./string-coerce-LndEvhRk.js");
var _globalSingletonB2nbp4Tq = require("./global-singleton-B2nbp4Tq.js");
var _utilsCKsuXgDI = require("./utils-CKsuXgDI.js");
var _globalsBJh4IgxP = require("./globals-BJh4IgxP.js");
var _operatorScopesChm67yFb = require("./operator-scopes-Chm67yFb.js");
var _dedupeDz8bskmN = require("./dedupe-Dz8bskmN.js");
//#region src/plugins/command-registry-state.ts
const PLUGIN_COMMAND_STATE_KEY = Symbol.for("openclaw.pluginCommandsState");
const getState$1 = () => (0, _globalSingletonB2nbp4Tq.n)(PLUGIN_COMMAND_STATE_KEY, () => ({
  pluginCommands: /* @__PURE__ */new Map(),
  registryLocked: false
}));
const getPluginCommandMap = () => getState$1().pluginCommands;
const pluginCommands = exports.j = new Proxy(/* @__PURE__ */new Map(), { get(_target, property) {
...
function restorePluginCommands(commands) {
  ...
  for (const command of commands) {
    const name = (0, _stringCoerceLndEvhRk.s)(command.name);
    if (!name) continue;
...
function toPluginInteractiveRegistryKey(channel, namespace) {
  return `${(0, _stringCoerceLndEvhRk.s)(channel) ?? ""}:${namespace.trim()}`;
}
...
function createInteractiveCallbackDedupe() {
  return (0, _dedupeDz8bskmN.n)(PLUGIN_INTERACTIVE_CALLBACK_DEDUPE_KEY, {
    ttlMs: 5 * 6e4,
    maxSize: 4096
  });
}
...
function registerPluginInteractiveHandler(pluginId, registration, opts) {
  ...
  interactiveHandlers.set(key, {
    ...registration,
    namespace,
    channel: (0, _stringCoerceLndEvhRk.s)(registration.channel) ?? "",
    pluginId,
...
const PLUGIN_PROMPT_MUTATION_RESULT_FIELDS = exports.h = [
"systemPrompt",
"prependContext",
"appendContext",
...
exports.g = stripPromptMutationFieldsFromLegacyHookResult;const PLUGIN_HOOK_NAMES = exports.l = [
...
const isPluginHookName = (hookName) => typeof hookName === "string" && pluginHookNameSet.has(hookName);exports.p = isPluginHookName;
const PROMPT_INJECTION_HOOK_NAMES = exports.u = [
"agent_turn_prepare",
"before_prompt_build",
"before_agent_start",
"heartbeat_prompt_contribution"];
...
const isPromptInjectionHookName = (hookName) => promptInjectionHookNameSet.has(hookName);exports.m = isPromptInjectionHookName;
const CONVERSATION_HOOK_NAMES = exports.c = [
"before_model_resolve",
"before_agent_reply",
"llm_input",
"llm_output",
...
const isConversationHookName = (hookName) => conversationHookNameSet.has(hookName);exports.f = isConversationHookName;
const PluginApprovalResolutions = exports.d = {
...
*/
const AGENT_PROMPT_SURFACE_KINDS = exports.s = [
"pi_main",
"codex_app_server",
"cli_backend",
"acp_backend",
...
function isReservedCommandName(name) {
  const trimmed = (0, _stringCoerceLndEvhRk.s)(name) ?? "";
...
function validatePluginCommandDefinition(command, opts) {
  ...
  if (command.ownership === "reserved") {
    ...
    if (!isReservedCommandName(command.name)) return `Reserved command ownership requires a reserved command name: ${(0, _stringCoerceLndEvhRk.s)(command.name) ?? ""}`;
  }
  ...
  if (command.requiredScopes !== void 0) {
    ...
    const unknownScope = command.requiredScopes.find((scope) => !(0, _operatorScopesChm67yFb.s)(scope));
  ...
  if (command.nativeNames !== void 0 && !(0, _utilsCKsuXgDI.c)(command.nativeNames)) return "Command nativeNames must be an object";
  ...
  if (command.nativeProgressMessages !== void 0 && !(0, _utilsCKsuXgDI.c)(command.nativeProgressMessages)) return "Command nativeProgressMessages must be an object";
  ...
  if (command.descriptionLocalizations !== void 0 && !(0, _utilsCKsuXgDI.c)(command.descriptionLocalizations)) return "Command descriptionLocalizations must be an object";
...
function validateAgentPromptGuidance(index, guidance) {
  ...
  if (!(0, _utilsCKsuXgDI.c)(guidance)) return `${label} must be a string or object`;
...
function normalizeAgentPromptGuidance(guidance) {
  ...
  return guidance.map((entry) => {
    ...
    if (entry.surfaces) normalized.surfaces = entry.surfaces.map((surface) => surface.trim());
    return normalized;
  });
}
...
function pluginCommandSupportsChannel(command, channel) {
  ...
  const normalizedChannel = (0, _stringCoerceLndEvhRk.a)(channel);
  return command.channels.some((entry) => (0, _stringCoerceLndEvhRk.a)(entry) === normalizedChannel);
}
function registerPluginCommand(pluginId, command, opts) {
  ...
  const normalizedName = (0, _stringCoerceLndEvhRk.a)(name);
  const description = command.description.trim();
  const normalizedCommand = {
    ...command,
    name,
    description,
    ...(command.channels ? { channels: command.channels.map((channel) => (0, _stringCoerceLndEvhRk.a)(channel)) } : {}),
  ...
  (0, _globalsBJh4IgxP.r)(`Registered plugin command: ${key} (plugin: ${pluginId})`);
  return { ok: true };
}
//#endregion /* v9-2e3593232603b953 */
