...
function formatHookErrorForLog(err) {
  return (replaceLogControlCharacters((0, _errorsVfATXfah.i)(err)).replace(/\s+/g, " ").trim() || "unknown error").slice(0, MAX_HOOK_LOG_MESSAGE_LENGTH);
}
function fireAndForgetHook(task, label, logger = _globalsCouSpJO.r) {
  task.catch((err) => {
...
function fireAndForgetBoundedHook(task, label, logger = _globalsCouSpJO.r, options = {}) {
...
function createHookRunner(registry, options = {}) {
  ...
  const sanitizeHookError = (error) => {
    return (0, _errorsVfATXfah.i)(error).split("\n")[0]?.trim() || "unknown error";
  };
...
const getLog = () => (0, _subsystemDLRoKDlF.t)("plugins");
/**
