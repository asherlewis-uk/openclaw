...
function resolveUserPath(input, env = process.env, homedir = _nodeOs.default.homedir) {
  if (!input) return "";
  return (0, _homeDirBOPFpGo.r)(input, {
    env,
    homedir
  });
}
function resolveConfigDir(env = process.env, homedir = _nodeOs.default.homedir) {
  ...
  const newDir = _nodePath.default.join((0, _homeDirBOPFpGo.o)(env, homedir), ".openclaw");
  try {
...
function resolveHomeDir() {
  return (0, _homeDirBOPFpGo.n)(process.env, _nodeOs.default.homedir);
}
...
async function pathExists(targetPath) {
  return await (0, _fsSafeCzypIej.T)(targetPath);
}
//#endregion /* v9-eae613f6e2c6434d */
