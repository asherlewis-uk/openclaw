"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.n = clearPluginOwnedSessionState;exports.r = runPluginHostCleanup;exports.t = cleanupReplacedPluginHostRegistry;var _stringCoerceLndEvhRk = require("./string-coerce-LndEvhRk.js");
...
function collectStoredSessionEntrySlotKeys(entry, pluginId) {
  ...
  for (const record of records) for (const slotKey of Object.values(record)) {
    const normalized = (0, _storeLoadBzczsLeI.o)(slotKey);
...
async function clearPluginOwnedSessionStores(params) {
  ...
  const storePaths = new Set((0, _targetsB3qI57M.i)(params.cfg).map((target) => target.storePath).filter((storePath) => _nodeFs.default.existsSync(storePath)));
  let cleared = 0;
  for (const storePath of storePaths) cleared += await (0, _storeC86lM9dg.s)(storePath, (store) => {
...
async function runPluginHostCleanup(params) {
  ...
  const sessionEntrySlotKeys = collectSessionEntrySlotKeys(registry ?? (0, _runtimeOTX8N6Lz.a)(), params.pluginId);
  ...
  if (shouldCleanup()) try {
    persistentCleanupCount = params.reason === "restart" ? await clearPromotedSessionEntrySlotStores({
      cfg: params.cfg ?? (0, _ioCiCdMMvQ.i)(),
  ...
  if (registry) {
    for (const registration of registry.sessionExtensions ?? []) {
      ...
      try {
        await (0, _runtimeOTX8N6Lz.M)(hookId, () => cleanup({
    ...
    const schedulerFailures = await (0, _runtimeOTX8N6Lz.w)({
      pluginId: params.pluginId,
      reason: params.reason,
      sessionKey: params.sessionKey,
  ...
  if (params.reason !== "restart" && shouldCleanup()) {
    ...
    })).filter(({ jobId }) => jobId.length > 0).map(({ pluginId, jobId }) => (0, _runtimeOTX8N6Lz.k)(pluginId, jobId)));
    const runtimeSchedulerFailures = await (0, _runtimeOTX8N6Lz.w)({
      pluginId: params.pluginId,
      reason: params.reason,
      sessionKey: params.sessionKey,
      preserveJobIds: params.preserveSchedulerJobIds,
  ...
  if (shouldCleanup() && (params.pluginId || params.runId) && (params.reason !== "restart" || params.runId)) (0, _runtimeOTX8N6Lz.T)({
    pluginId: params.pluginId,
    runId: params.runId
  });
  return {
