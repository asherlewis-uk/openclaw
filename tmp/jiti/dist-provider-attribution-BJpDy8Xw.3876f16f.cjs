"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.i = resolveProviderRequestPolicy;exports.n = resolveProviderEndpoint;exports.r = resolveProviderRequestCapabilities;exports.t = describeProviderRequestRoutingSummary;var _stringCoerceLndEvhRk = require("./string-coerce-LndEvhRk.js");
var _versionB2G3zXnp = require("./version-B2G3zXnp.js");
var _providerIdCz7K6wgK = require("./provider-id-Cz7K6wgK.js");
var _manifestMetadataScanDmQpfwPO = require("./manifest-metadata-scan-DmQpfwPO.js");
...
function tryParseHostname(value) {
  try {
    return (0, _stringCoerceLndEvhRk.s)(new URL(value).hostname);
  } catch {
    return;
  }
}
...
function normalizeComparableBaseUrl(value) {
  ...
  try {
    ...
    url.hash = "";
    url.search = "";
    return (0, _stringCoerceLndEvhRk.s)(url.toString().replace(/\/+$/, ""));
  } catch {
    return;
  }
}
...
function normalizeStringList(value) {
  ...
  return value.map((entry) => (0, _stringCoerceLndEvhRk.c)(entry)).filter((entry) => entry !== void 0);
}
function readManifestProviderEndpoints(manifest) {
  ...
  for (const rawEndpoint of manifest.providerEndpoints) {
    ...
    const endpointClassRaw = (0, _stringCoerceLndEvhRk.c)(rawEndpoint.endpointClass);
    ...
    entries.push({
      ...
      ...((0, _stringCoerceLndEvhRk.c)(rawEndpoint.googleVertexRegion) ? { googleVertexRegion: (0, _stringCoerceLndEvhRk.c)(rawEndpoint.googleVertexRegion) } : {}),
      ...((0, _stringCoerceLndEvhRk.c)(rawEndpoint.googleVertexRegionHostSuffix) ? { googleVertexRegionHostSuffix: (0, _stringCoerceLndEvhRk.c)(rawEndpoint.googleVertexRegionHostSuffix) } : {})
    });
  }
  return entries;
}
function readManifestProviderRequests(manifest) {
  ...
  for (const [providerRaw, requestRaw] of Object.entries(providerRequest.providers)) {
    ...
    const provider = (0, _stringCoerceLndEvhRk.a)(providerRaw);
    if (!provider) continue;
    const compatibilityFamily = (0, _stringCoerceLndEvhRk.c)(requestRaw.compatibilityFamily) === "moonshot" ? "moonshot" : void 0;
    ...
    entries.push([provider, {
      ...((0, _stringCoerceLndEvhRk.c)(requestRaw.family) ? { family: (0, _stringCoerceLndEvhRk.c)(requestRaw.family) } : {}),
...
function collectManifestProviderEndpoints() {
  const entries = [];
  for (const { manifest } of (0, _manifestMetadataScanDmQpfwPO.t)()) entries.push(...readManifestProviderEndpoints(manifest));
  return entries;
}
function collectManifestProviderRequests() {
  const entries = /* @__PURE__ */new Map();
  for (const { manifest } of (0, _manifestMetadataScanDmQpfwPO.t)()) for (const [provider, request] of readManifestProviderRequests(manifest)) entries.set(provider, request);
  return entries;
}
...
function isOpenAIResponsesApi(api) {
  const normalizedApi = (0, _stringCoerceLndEvhRk.s)(api);
...
function resolveProviderAttributionIdentity(env = process.env) {
  return {
    ...
    version: (0, _versionB2G3zXnp.s)(env)
  };
}
...
function resolveProviderRequestPolicy(input, env = process.env) {
  const provider = (0, _providerIdCz7K6wgK.r)(input.provider ?? "");
  const policy = resolveProviderAttributionPolicy(provider, env);
...
function resolveProviderRequestCapabilities(input, env = process.env) {
  ...
  const api = (0, _stringCoerceLndEvhRk.s)(input.api);
...
function describeProviderRequestRoutingSummary(input, env = process.env) {
  const policy = resolveProviderRequestPolicy(input, env);
  const api = (0, _stringCoerceLndEvhRk.s)(input.api) ?? "unknown";
