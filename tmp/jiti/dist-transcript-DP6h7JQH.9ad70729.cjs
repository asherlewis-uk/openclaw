"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.a = resolveSessionTranscriptFile;exports.c = appendSessionTranscriptMessage;exports.i = readTailAssistantTextFromSessionTranscript;exports.l = resolveAndPersistSessionFile;exports.n = appendExactAssistantMessageToSessionTranscript;exports.o = streamSessionTranscriptLines;exports.r = readLatestAssistantTextFromSessionTranscript;exports.s = resolveMirroredTranscriptText;exports.t = appendAssistantMessageToSessionTranscript;exports.u = redactTranscriptMessage;var _redactCxqQQvmK = require("./redact-CxqQQvmK.js");
...
var _transcriptEventsPpVxfGB = require("./transcript-events-Pp-VxfGB.js");
var _chatMessageContentCjC9AtYE = require("./chat-message-content-CjC9AtYE.js");
var _sessionWriteLockCzZgXsp = require("./session-write-lock-CzZgXsp-.js");
...
function resolveTranscriptRedactPatterns(patterns) {
  return patterns && patterns.length > 0 ? [...patterns, ...(0, _redactCxqQQvmK.t)()] : void 0;
}
function redactTranscriptOptions(cfg) {
  const configuredLogging = (0, _redactCxqQQvmK.f)();
...
function redactTranscriptText(value, cfg) {
  ...
  return (0, _redactCxqQQvmK.s)(value, redactTranscriptOptions(cfg));
}
function redactTranscriptStructuredFieldValue(key, value, cfg) {
  if (cfg?.logging?.redactSensitive === "off") return value;
  return (0, _redactCxqQQvmK.i)(key, value, redactTranscriptOptions(cfg));
}
...
async function resolveAndPersistSessionFile(params) {
  ...
  const sessionFile = (0, _pathsRbVDOqFp.i)(sessionId, !shouldReusePersistedSessionFile ? fallbackSessionFile ? {
    ...baseEntry,
  ...
  if (baseEntry.sessionId !== sessionId || baseEntry.sessionFile !== sessionFile) {
    ...
    await (0, _storeC86lM9dg.s)(storePath, (store) => {
      store[sessionKey] = {
        ...store[sessionKey],
        ...persistedEntry
      };
...
async function appendSessionTranscriptMessageLocked(params) {
  const lock = await (0, _sessionWriteLockCzZgXsp.i)({
    sessionFile: params.transcriptPath,
    ...(0, _sessionWriteLockCzZgXsp.d)(params.config),
    allowReentrant: true
  });
  try {
    ...
    const finalMessage = isTranscriptAgentMessage(params.message) ? redactTranscriptMessage(params.message, params.config) : (0, _redactCxqQQvmK.r)(params.message);
    const entry = {
      type: "message",
      id: messageId,
...
function parseAssistantTranscriptText(line, options) {
  ...
  const text = (0, _chatMessageContentCjC9AtYE.n)(message)?.trim();
  if (!text) return;
  return {
...
async function resolveSessionTranscriptFile(params) {
  const sessionPathOpts = (0, _pathsRbVDOqFp.a)({
    agentId: params.agentId,
    storePath: params.storePath
  });
  let sessionFile = (0, _pathsRbVDOqFp.i)(params.sessionId, params.sessionEntry, sessionPathOpts);
  let sessionEntry = params.sessionEntry;
  if (params.sessionStore && params.storePath) {
    const threadIdFromSessionKey = (0, _storeLoadBzczsLeI.S)(params.sessionKey).threadId;
    const fallbackSessionFile = !sessionEntry?.sessionFile ? (0, _pathsRbVDOqFp.o)(params.sessionId, params.agentId, params.threadId ?? threadIdFromSessionKey) : void 0;
...
async function appendExactAssistantMessageToSessionTranscript(params) {
  ...
  const storePath = params.storePath ?? (0, _pathsRbVDOqFp.r)(params.agentId);
  const store = (0, _storeLoadBzczsLeI.t)(storePath, { skipCache: true });
  ...
  try {
    ...
    return {
      ok: false,
      reason: (0, _errorsIxwfrboQ.i)(err)
    };
  }
