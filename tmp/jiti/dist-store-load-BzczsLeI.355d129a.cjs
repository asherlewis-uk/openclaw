"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.A = resolveSessionStoreEntry;exports.C = parseSessionThreadInfoFast;exports.D = isCacheEnabled;exports.E = getFileStatSnapshot;exports.O = resolveCacheTtlMs;exports.S = parseSessionThreadInfo;exports.T = createExpiringMapCache;exports._ = pruneQuotaSuspensions;exports.a = registerSessionMaintenancePreserveKeysProvider;exports.b = shouldPreserveMaintenanceEntry;exports.c = cloneSessionStoreRecord;exports.d = isSessionStoreCacheEnabled;exports.f = setSerializedSessionStore;exports.g = getActiveSessionMaintenanceWarning;exports.h = capEntryCount;exports.i = collectSessionMaintenancePreserveKeys;exports.k = normalizeStoreSessionKey;exports.l = dropSessionStoreObjectCache;exports.m = writeSessionStoreCache;exports.n = normalizeSessionStore;exports.o = normalizeSessionEntrySlotKey;exports.p = takeMutableSessionStoreCache;exports.r = resolveMaintenanceConfig;exports.s = clearSessionStoreCaches;exports.t = loadSessionStore;exports.u = getSerializedSessionStore;exports.v = pruneStaleEntries;exports.w = resolveLoadedSessionThreadInfo;exports.x = shouldRunSessionEntryMaintenance;exports.y = resolveMaintenanceConfigFromInput;var _stringCoerceLndEvhRk = require("./string-coerce-LndEvhRk.js");
...
var _parseFiniteNumberDU39H5m = require("./parse-finite-number-DU39-h5m.js");
...
var _sessionConversationDD7EuDVB = require("./session-conversation-DD7EuDVB.js");
var _registryLoadedReadDORbZUaU = require("./registry-loaded-read-DORbZUaU.js");
var _storeEntryShapeCY4PzDhp = require("./store-entry-shape-CY4PzDhp.js");
var _typesCvORIrpN = require("./types-CvORIrpN.js");
...
function normalizeStoreSessionKey(sessionKey) {
  return (0, _sessionKeyUtilsCJRKuBJA.o)(sessionKey);
}
function resolveSessionStoreEntry(params) {
  const trimmedKey = params.sessionKey.trim();
  const normalizedKey = normalizeStoreSessionKey(trimmedKey);
  const foldedLegacyKey = (0, _stringCoerceLndEvhRk.a)(normalizedKey);
  ...
  for (const [candidateKey, candidateEntry] of Object.entries(params.store)) {
    ...
    if (!(normalizeStoreSessionKey(candidateKey) === normalizedKey || foldedLegacyKey !== normalizedKey && (0, _stringCoerceLndEvhRk.a)(candidateKey) === foldedLegacyKey)) continue;
...
function resolveCacheTtlMs(params) {
  ...
  if (envValue) {
    const parsed = (0, _parseFiniteNumberDU39H5m.r)(envValue);
...
function resolveLoadedSessionConversationThreadInfo(sessionKey) {
  ...
  const resolved = (0, _registryLoadedReadDORbZUaU.t)(raw.channel)?.messaging?.resolveSessionConversation?.({
    kind: raw.kind,
    rawId
  });
  if (!resolved?.id?.trim()) return null;
  const id = resolved.id.trim();
  const threadId = (0, _stringCoerceLndEvhRk.c)(resolved.threadId);
  return {
    baseSessionKey: threadId ? `${raw.prefix}:${id}` : (0, _stringCoerceLndEvhRk.c)(sessionKey),
    threadId
  };
}
function resolveLoadedSessionThreadInfo(sessionKey) {
  return resolveLoadedSessionConversationThreadInfo(sessionKey) ?? (0, _sessionKeyUtilsCJRKuBJA.u)(sessionKey);
}
//#endregion
...
function parseSessionThreadInfo(sessionKey) {
  return (0, _sessionConversationDD7EuDVB.i)(sessionKey);
}
...
const log$1 = (0, _subsystemCH8Q21Y.t)("sessions/store");
...
function resolvePruneAfterMs(maintenance) {
  const normalized = (0, _stringCoerceLndEvhRk.d)(maintenance?.pruneAfter ?? maintenance?.pruneDays);
  if (!normalized) return DEFAULT_SESSION_PRUNE_AFTER_MS;
  try {
    return (0, _parseDurationD8AJG6ba.t)(normalized, { defaultUnit: "d" });
  } catch {
...
function resolveResetArchiveRetentionMs(maintenance, pruneAfterMs) {
  ...
  const normalized = (0, _stringCoerceLndEvhRk.d)(raw);
...
function resolveMaxDiskBytes(maintenance) {
  ...
  try {
    return (0, _zodSchema6ON220xK.r)(normalized, { defaultUnit: "b" });
  } catch {
    return null;
  }
}
function resolveHighWaterBytes(maintenance, maxDiskBytes) {
  ...
  try {
    const parsed = (0, _zodSchema6ON220xK.r)(normalized, { defaultUnit: "b" });
...
function isSyntheticSessionMaintenanceKey(sessionKey) {
  const rest = (0, _stringCoerceLndEvhRk.a)((0, _sessionKeyUtilsCJRKuBJA.c)(sessionKey)?.rest ?? sessionKey);
  return (0, _sessionKeyUtilsCJRKuBJA.a)(sessionKey) || (0, _sessionKeyUtilsCJRKuBJA.n)(sessionKey) || (0, _sessionKeyUtilsCJRKuBJA.i)(sessionKey) || rest.startsWith("hook:") || rest.startsWith("node:") || rest === "heartbeat" || rest.endsWith(":heartbeat") || rest.includes(":heartbeat:");
}
...
function isProtectedSessionMaintenanceEntry(sessionKey, entry) {
  ...
  const chatType = (0, _stringCoerceLndEvhRk.a)(entry?.chatType ?? entry?.origin?.chatType);
...
function resolveMaintenanceConfig() {
  let maintenance;
  try {
    maintenance = (0, _ioCiCdMMvQ.i)().session?.maintenance;
  } catch {}
...
const log = (0, _subsystemCH8Q21Y.t)("sessions/store");
...
function normalizeOptionalDeliveryContext(value) {
  ...
  const normalized = (0, _deliveryContextSharedMTeaAgYN.o)({
...
function normalizePendingFinalDeliveryFields(entry) {
  ...
  const assign = (key, value) => {
    ...
    if (value === void 0) delete next[key];else
    next[key] = value;
  };
...
function normalizePluginExtensions(entry) {
  ...
  for (const [rawPluginId, rawPluginState] of Object.entries(entry.pluginExtensions)) {
    ...
    for (const [rawNamespace, rawValue] of Object.entries(rawPluginState)) {
      ...
      if (!namespace || !(0, _hostHookJsonCgIPvDoo.t)(rawValue)) {
        changed = true;
        continue;
      }
  ...
  const next = { ...entry };
  if (Object.keys(normalizedExtensions).length > 0) next.pluginExtensions = normalizedExtensions;else
  delete next.pluginExtensions;
  return next;
}
function normalizePluginExtensionSlotKeys(entry) {
  ...
  if (!changed) return entry;
  const next = { ...entry };
  if (Object.keys(normalizedSlotKeys).length > 0) next.pluginExtensionSlotKeys = normalizedSlotKeys;else
  delete next.pluginExtensionSlotKeys;
  return next;
}
...
function normalizeSessionEntryDelivery(entry) {
  const entryRoute = (0, _deliveryContextSharedMTeaAgYN.a)(entry.route);
  const normalized = (0, _deliveryContextSharedMTeaAgYN.s)({
    route: entryRoute,
...
function normalizeSessionStore(store) {
  let changed = false;
  for (const [key, entry] of Object.entries(store)) {
    const shaped = (0, _storeEntryShapeCY4PzDhp.t)(entry);
    if (!shaped) {
      delete store[key];
      changed = true;
      continue;
    }
    const normalized = stripPersistedSkillsCache(normalizePluginExtensionSlotKeys(normalizePluginExtensions(normalizePendingFinalDeliveryFields(normalizeSessionEntryDelivery((0, _typesCvORIrpN.a)(shaped))))));
    if (normalized !== entry) {
