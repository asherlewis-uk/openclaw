...
function normalizeSsrFPolicyHostnames(values) {
  ...
  return Array.from(new Set(values.map((value) => (0, _hostnameD0GmZ7Va.t)(value)).filter(Boolean))).toSorted();
}
...
function normalizeHostnameSet(values) {
  ...
  return new Set(values.map((value) => (0, _hostnameD0GmZ7Va.t)(value)).filter(Boolean));
}
function normalizeHostnameAllowlist(values) {
  if (!values || values.length === 0) return [];
  return Array.from(new Set(values.map((value) => (0, _hostnameD0GmZ7Va.t)(value)).filter((value) => value !== "*" && value !== "*." && value.length > 0)));
}
...
function isPrivateIpAddress(address, policy) {
  const normalized = (0, _hostnameD0GmZ7Va.t)(address);
  if (!normalized) return false;
  const blockOptions = resolveIpv4SpecialUseBlockOptions(policy);
  const ipv6BlockOptions = resolveIpv6SpecialUseBlockOptions(policy);
  const strictIp = (0, _ipDb8k2XjS.m)(normalized);
  if (strictIp) {
    if ((0, _ipDb8k2XjS.s)(strictIp)) return (0, _ipDb8k2XjS.n)(strictIp, blockOptions);
    if ((0, _ipDb8k2XjS.r)(strictIp, ipv6BlockOptions)) return true;
    const embeddedIpv4 = (0, _ipDb8k2XjS.t)(strictIp);
    if (embeddedIpv4) return (0, _ipDb8k2XjS.n)(embeddedIpv4, blockOptions);
    return false;
  }
  if (normalized.includes(":") && !(0, _ipDb8k2XjS.h)(normalized)) return true;
  if (!(0, _ipDb8k2XjS.i)(normalized) && (0, _ipDb8k2XjS.l)(normalized)) return true;
  if (looksLikeUnsupportedIpv4Literal(normalized)) return true;
  return false;
}
function isBlockedHostname(hostname) {
  const normalized = (0, _hostnameD0GmZ7Va.t)(hostname);
  if (!normalized) return false;
...
function createPinnedLookup(params) {
  const normalizedHost = (0, _hostnameD0GmZ7Va.t)(params.hostname);
  ...
  return (host, options, callback) => {
    ...
    if (!cb) return;
    const normalized = (0, _hostnameD0GmZ7Va.t)(host);
...
function resolvePinnedDispatcherLookup(pinned, override, policy) {
  ...
  const normalizedOverrideHost = (0, _hostnameD0GmZ7Va.t)(override.hostname);
...
function createPinnedDispatcher(pinned, policy, ssrfPolicy, timeoutMs) {
  ...
  if (!policy || policy.mode === "direct") return (0, _undiciRuntimeDzZbpePE.t)({ connect: withPinnedLookup(lookup, policy?.connect) }, timeoutMs);
  if (policy.mode === "env-proxy") return (0, _undiciRuntimeDzZbpePE.n)({
    connect: withPinnedLookup(lookup, policy.connect),
    ...(policy.proxyTls ? { proxyTls: { ...policy.proxyTls } } : {})
  }, timeoutMs);
  const proxyUrl = policy.proxyUrl.trim();
  const requestTls = withPinnedLookup(lookup, policy.proxyTls);
  if (!requestTls) return (0, _undiciRuntimeDzZbpePE.r)({ uri: proxyUrl }, timeoutMs);
  return (0, _undiciRuntimeDzZbpePE.r)({
    uri: proxyUrl,
    requestTls
  }, timeoutMs);
}
