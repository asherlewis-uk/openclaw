...
var _gatewayProcessArgvDy8AqYte = require("./gateway-process-argv-Dy8AqYte.js");
var _portsLsofCpnIFge = require("./ports-lsof-Cpn-IFge.js");
var _cmdArgvDexenlHS = require("./cmd-argv-DexenlHS.js");
...
function readWindowsProcessArgsResultSync(pid, timeoutMs = DEFAULT_TIMEOUT_MS) {
  ...
  if (!powershell.error && powershell.status === 0) {
    ...
    return {
      ok: true,
      args: command ? (0, _cmdArgvDexenlHS.t)(command) : null
    };
  }
...
const restartLog = (0, _subsystemDLRoKDlF.t)("restart");
function getTimeMs() {
  return Date.now();
}
...
function readUnixProcessArgsSync(pid, spawnTimeoutMs) {
  if (process.platform === "linux") try {
    const args = (0, _gatewayProcessArgvDy8AqYte.n)((0, _nodeFs.readFileSync)(`/proc/${pid}/cmdline`, "utf8"));
...
function filterVerifiedWindowsGatewayPidsResult(rawPids, processArgsResult) {
  ...
  for (const pid of Array.from(new Set(rawPids))) {
    ...
    if (argsResult.args != null && (0, _gatewayProcessArgvDy8AqYte.t)(argsResult.args, { allowGatewayBinary: true })) verified.push(pid);
  }
  return {
    ok: true,
    pids: verified
...
function findGatewayPidsOnPortSync(port, spawnTimeoutMs = SPAWN_TIMEOUT_MS) {
  ...
  const res = (0, _nodeChild_process.spawnSync)((0, _portsLsofCpnIFge.n)(), [
  "-nP",
  `-iTCP:${port}`,
  "-sTCP:LISTEN",
  "-Fpc"],
...
function terminateStaleProcessesWindows(pids) {
  const taskkillPath = _nodePath.default.win32.join((0, _windowsInstallRootsByOWfMp.t)().systemRoot, "System32", "taskkill.exe");
  const killed = [];
