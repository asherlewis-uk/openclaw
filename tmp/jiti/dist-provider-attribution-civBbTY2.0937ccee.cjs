...
function collectManifestProviderEndpoints() {
  const entries = [];
  for (const { manifest } of (0, _manifestMetadataScanCXIMSM.t)()) entries.push(...readManifestProviderEndpoints(manifest));
  return entries;
}
function collectManifestProviderRequests() {
  const entries = /* @__PURE__ */new Map();
  for (const { manifest } of (0, _manifestMetadataScanCXIMSM.t)()) for (const [provider, request] of readManifestProviderRequests(manifest)) entries.set(provider, request);
  return entries;
}
...
function resolveProviderAttributionIdentity(env = process.env) {
  return {
    ...
    version: (0, _versionDmRkhYua.s)(env)
  };
}
