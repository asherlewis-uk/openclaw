"use strict";Object.defineProperty(exports, "__esModule", { value: true });exports.A = resolveSafeBinProfiles;exports.C = resolveExecutable;exports.D = void 0;exports.E = resolveExecWrapperTrustPlan;exports.O = void 0;exports.S = resolvePolicyTargetTrustPath;exports.T = resolveExecutablePath;exports._ = resolveExecutionTargetResolution;exports.a = validateSafeBinArgv;exports.b = resolvePolicyTargetCandidatePath;exports.c = matchAllowlist;exports.d = resolveApprovalAuditCandidatePath;exports.f = resolveApprovalAuditTrustPath;exports.g = resolveExecutionTargetCandidatePath;exports.h = resolveExecutableTrustPath;exports.i = normalizeTrustedSafeBinDirs;exports.k = normalizeSafeBinProfileFixtures;exports.l = parseExecArgvToken;exports.m = resolveCommandResolutionFromArgv;exports.n = isTrustedSafeBinPath;exports.o = listRiskyConfiguredSafeBins;exports.p = resolveCommandResolution;exports.r = listWritableExplicitTrustedSafeBinDirs;exports.s = normalizeSafeBinName;exports.t = getTrustedSafeBinDirs;exports.u = resolveAllowlistCandidatePath;exports.v = resolveExecutionTargetTrustPath;exports.w = resolveExecutableFromPathEnv;exports.x = resolvePolicyTargetResolution;exports.y = resolvePolicyAllowlistCandidatePath;var _stringCoerceLndEvhRk = require("./string-coerce-LndEvhRk.js");
var _homeDirIZwpu = require("./home-dir-iZwpu-40.js");
var _shellWrapperResolutionBOPTchHQ = require("./shell-wrapper-resolution-BOPTchHQ.js");
var _nodeFs = _interopRequireDefault(require("node:fs"));
var _nodePath = _interopRequireDefault(require("node:path"));function _interopRequireDefault(e) {return e && e.__esModule ? e : { default: e };}
//#region src/infra/exec-safe-bin-policy-profiles.ts
const NO_FLAGS$1 = /* @__PURE__ */new Set();
const DEFAULT_SAFE_BINS = exports.D = [
"cut",
"uniq",
"head",
"tail",
...
const SAFE_BIN_PROFILES = exports.O = compileSafeBinProfiles({
  jq: {
    maxPositional: 1,
    allowedValueFlags: [
    "--arg",
...
function normalizeSafeBinProfileName(raw) {
  const name = (0, _stringCoerceLndEvhRk.a)(raw);
...
function normalizeMatchTarget(value) {
  if (process.platform === "win32") return (0, _stringCoerceLndEvhRk.a)(value.replace(/^\\\\[?.]\\/, "").replace(/\\/g, "/"));
...
function normalizeDotPathSegments(value) {
  return normalizeMatchTarget(process.platform === "win32" ? _nodePath.default.win32.normalize(value) : _nodePath.default.posix.normalize(value));
}
...
function matchesExecAllowlistPattern(pattern, target) {
  ...
  const expanded = trimmed.startsWith("~") ? (0, _homeDirIZwpu.t)(trimmed) : trimmed;
...
function finalizeExecWrapperTrustPlan(argv, policyArgv, wrapperChain, policyBlocked, blockedWrapper) {
  ...
  const shellWrapperExecutable = !policyBlocked && rawExecutable.length > 0 && (0, _shellWrapperResolutionBOPTchHQ.c)(rawExecutable);
  const plan = {
    argv,
    policyArgv,
    wrapperChain,
    ...
    shellInlineCommand: shellWrapperExecutable ? (0, _shellWrapperResolutionBOPTchHQ.r)(argv) : null
  };
...
function resolveExecWrapperTrustPlan(argv, maxDepth = 4) {
  ...
  for (let depth = 0; depth < maxDepth; depth += 1) {
    const dispatchPlan = (0, _shellWrapperResolutionBOPTchHQ.b)(current, maxDepth - wrapperChain.length);
    ...
    const shellMultiplexerUnwrap = (0, _shellWrapperResolutionBOPTchHQ.d)(current);
  ...
  if (wrapperChain.length >= maxDepth) {
    const dispatchOverflow = (0, _shellWrapperResolutionBOPTchHQ.S)(current);
    ...
    const shellMultiplexerOverflow = (0, _shellWrapperResolutionBOPTchHQ.d)(current);
...
function resolveExecutablePathCandidate(rawExecutable, options) {
  const expanded = rawExecutable.startsWith("~") ? (0, _homeDirIZwpu.t)(rawExecutable, { env: options?.env }) : rawExecutable;
...
function resolveWindowsExecutableExtensions(executable, env) {
  ...
  if (_nodePath.default.extname(executable).length > 0) return [""];
  return ["", ...(env?.PATHEXT ?? env?.Pathext ?? process.env.PATHEXT ?? process.env.Pathext ?? ".EXE;.CMD;.BAT;.COM").split(";").map((ext) => (0, _stringCoerceLndEvhRk.a)(ext))];
}
function resolveWindowsExecutableExtSet(env) {
  return new Set((env?.PATHEXT ?? env?.Pathext ?? process.env.PATHEXT ?? process.env.Pathext ?? ".EXE;.CMD;.BAT;.COM").split(";").map((ext) => (0, _stringCoerceLndEvhRk.a)(ext)).filter(Boolean));
}
function isExecutableFile(filePath) {
  try {
    if (!_nodeFs.default.statSync(filePath).isFile()) return false;
    if (process.platform === "win32") {
      const ext = (0, _stringCoerceLndEvhRk.a)(_nodePath.default.extname(filePath));
      if (!ext) return true;
...
function resolveExecutableFromPathEnv(executable, pathEnv, env) {
  const delimiter = process.platform === "win32" ? ";" : _nodePath.default.delimiter;
  ...
  for (const entry of entries) for (const ext of extensions) {
    const candidate = _nodePath.default.join(entry, executable + ext);
...
function resolveExecutable(cmd) {
  ...
  if (KNOWN_PATHEXT.has((0, _stringCoerceLndEvhRk.a)(_nodePath.default.extname(cmd)))) return cmd;
  ...
  const matches = [];
  for (const entry of entries) for (const ext of extensions) {
    const candidate = _nodePath.default.join(entry, cmd + ext);
    if (isExecutableFile(candidate)) matches.push(candidate);
  }
  const cmdMatch = matches.find((match) => (0, _stringCoerceLndEvhRk.a)(_nodePath.default.extname(match)) === ".cmd");
  if (cmdMatch) return cmdMatch;
  const exeMatch = matches.find((match) => (0, _stringCoerceLndEvhRk.a)(_nodePath.default.extname(match)) === ".exe");
...
function buildExecutableResolution(rawExecutable, params) {
  ...
  return {
    ...
    executableName: resolvedPath ? _nodePath.default.basename(resolvedPath) : rawExecutable
  };
}
...
function matchArgPattern(argPattern, argv, platform) {
  ...
  try {
    ...
    if ((0, _stringCoerceLndEvhRk.a)(platform ?? process.platform).startsWith("win")) {
...
function matchesExecutableBasenamePattern(pattern, resolution) {
  ...
  if (resolution.resolvedPath) candidates.add(_nodePath.default.basename(resolution.resolvedPath));
...
function normalizeSafeBinName(raw) {
  const trimmed = (0, _stringCoerceLndEvhRk.a)(raw);
  if (!trimmed) return "";
...
function pathCaseInsensitive(value) {
  ...
  for (;;) {
    ...
    if (swapped !== candidate) try {
      const original = _nodeFs.default.statSync(candidate);
      try {
        return sameFsObject(original, _nodeFs.default.statSync(swapped));
      } catch {
        return false;
      }
    } catch {}
    const parent = _nodePath.default.dirname(candidate);
    if (parent === candidate) return process.platform === "win32";
    candidate = parent;
  }
}
function normalizeTrustComparisonPath(value) {
  const resolved = _nodePath.default.resolve(value);
  return pathCaseInsensitive(resolved) ? resolved.toLowerCase() : resolved;
}
function normalizeTrustedDir(value, forComparison = true) {
  const trimmed = value.trim();
  if (!trimmed) return null;
  return forComparison ? normalizeTrustComparisonPath(trimmed) : _nodePath.default.resolve(trimmed);
}
...
function isExecutableSafeBinFile(value) {
  try {
    if (!_nodeFs.default.statSync(value).isFile()) return false;
    if (process.platform === "win32") return true;
    _nodeFs.default.accessSync(value, _nodeFs.default.constants.X_OK);
    return true;
  } catch {
    return false;
  }
...
function resolveTrustedSafeBinTargetDirs(entries, safeBins, forComparison = true) {
  ...
  for (const entry of normalizeTrustedSafeBinDirs(entries)) {
    const dir = _nodePath.default.resolve(entry);
    for (const bin of bins) {
      const candidate = _nodePath.default.join(dir, bin);
      if (!isExecutableSafeBinFile(candidate)) continue;
      try {
        const normalized = normalizeTrustedDir(_nodePath.default.dirname(_nodeFs.default.realpathSync(candidate)), forComparison);
...
function buildTrustedSafeBinCacheKey(entries, safeBins, targetDirs) {
  return `${resolveTrustedSafeBinDirs(normalizeTrustedSafeBinDirs(entries)).join("")}\u0002${Array.from(new Set(safeBins.map((entry) => entry.trim()).filter(Boolean))).toSorted().join("")}\u0002${targetDirs.join("")}`;
}
...
function isTrustedSafeBinPath(params) {
  ...
  const resolvedDir = normalizeTrustComparisonPath(_nodePath.default.dirname(_nodePath.default.resolve(params.resolvedPath)));
...
function listWritableExplicitTrustedSafeBinDirs(entries) {
  ...
  for (const dir of resolved) {
    let stat;
    try {
      stat = _nodeFs.default.statSync(dir);
    } catch {
      continue;
    }
