...
var _installPathsCOOYiHmY = require("./install-paths-COOYiHmY.js");
...
function readJsonObjectFileSync(filePath) {
  const parsed = (0, _jsonFilesCahFuwKs.u)(filePath);
...
function resolveRecoveredManagedNpmPluginId(params) {
  ...
  return (0, _installPathsCOOYiHmY.c)(pluginId) ? void 0 : pluginId;
}
function buildRecoveredManagedNpmInstallRecords(options = {}) {
  const npmRoot = options.stateDir ? _nodePath.default.join(options.stateDir, "npm") : (0, _installPathsCOOYiHmY.a)(options.env);
...
async function readPersistedInstalledPluginIndexInstallRecords(options = {}) {
  return extractPluginInstallRecordsFromPersistedInstalledPluginIndex(await (0, _jsonFilesCahFuwKs.l)(resolveInstalledPluginIndexStorePath(options)));
}
function readPersistedInstalledPluginIndexInstallRecordsSync(options = {}) {
  return extractPluginInstallRecordsFromPersistedInstalledPluginIndex((0, _jsonFilesCahFuwKs.u)(resolveInstalledPluginIndexStorePath(options)));
}
