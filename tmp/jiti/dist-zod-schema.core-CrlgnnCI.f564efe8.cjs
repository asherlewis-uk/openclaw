...
const SecretsExecProviderSchema = _zod.z.object({
  source: _zod.z.literal("exec"),
  command: _zod.z.string().min(1).refine((value) => (0, _execSafetyD79DFxXy.t)(value), "secrets.providers.*.command is unsafe.").refine((value) => isAbsolutePath(value), "secrets.providers.*.command must be an absolute path."),
...
const ModelApiSchema = _zod.z.enum(_typesModelsCgsin5R.t);
const ModelCompatSchema = _zod.z.object({
  ...
  strictMessageKeys: _zod.z.boolean().optional(),
...
const ConfiguredProviderRequestTlsSchema = _zod.z.object({
  ca: SecretInputSchema.optional().register(_zodSchemaSensitiveDZxjbqai.t),
  cert: SecretInputSchema.optional().register(_zodSchemaSensitiveDZxjbqai.t),
  key: SecretInputSchema.optional().register(_zodSchemaSensitiveDZxjbqai.t),
  passphrase: SecretInputSchema.optional().register(_zodSchemaSensitiveDZxjbqai.t),
...
_zod.z.object({
  ...
  token: SecretInputSchema.register(_zodSchemaSensitiveDZxjbqai.t)
}).strict(),
_zod.z.object({
  mode: _zod.z.literal("header"),
  headerName: _zod.z.string().min(1),
  value: SecretInputSchema.register(_zodSchemaSensitiveDZxjbqai.t),
...
const ConfiguredProviderRequestFields = {
  headers: _zod.z.record(_zod.z.string(), SecretInputSchema.register(_zodSchemaSensitiveDZxjbqai.t)).optional(),
...
const ModelAgentRuntimePolicySchema = _zod.z.object({ id: _zod.z.string().optional() }).strict().optional();
...
const ModelProviderLocalServiceSchema = _zod.z.object({
  command: _zod.z.string().min(1),
  args: _zod.z.array(_zod.z.string()).optional(),
  cwd: _zod.z.string().min(1).optional(),
  env: _zod.z.record(_zod.z.string(), _zod.z.string().register(_zodSchemaSensitiveDZxjbqai.t)).optional(),
  healthUrl: _zod.z.string().min(1).optional(),
  readyTimeoutMs: _zod.z.number().int().positive().optional(),
  idleStopMs: _zod.z.number().int().nonnegative().optional()
}).strict().optional();
const ModelProviderSchema = _zod.z.object({
  baseUrl: _zod.z.string().min(1),
  apiKey: SecretInputSchema.optional().register(_zodSchemaSensitiveDZxjbqai.t),
  auth: _zod.z.union([
...
const TtsProviderConfigSchema = _zod.z.object({ apiKey: SecretInputSchema.optional().register(_zodSchemaSensitiveDZxjbqai.t) }).catchall(_zod.z.union([
_zod.z.string(),
_zod.z.number(),
_zod.z.boolean(),
_zod.z.null(),
...
const CliBackendSchema = exports.r = _zod.z.object({
  ...
  reseedFromRawTranscriptWhenUncompacted: _zod.z.boolean().optional(),
...
const normalizeAllowFrom = (values) => (0, _stringNormalizationDEwYgSEp.s)(values);
...
const TranscribeAudioSchema = exports.k = _zod.z.object({
  command: _zod.z.array(_zod.z.string()).superRefine((value, ctx) => {
    ...
    if (!(0, _execSafetyD79DFxXy.t)(executable)) ctx.addIssue({
      code: _zod.z.ZodIssueCode.custom,
      path: [0],
...
const ExecutableTokenSchema = exports.s = _zod.z.string().refine(_execSafetyD79DFxXy.t, "expected safe executable name or path");
