...
var _identityDX9yGpYy = require("./identity-DX9yGpYy.js");
...
const resolveAuthAgentDir = (agentDir, config) => agentDir ?? (0, _agentScopeConfig26EcJVc.s)(config ?? {});
function buildEnvSecretRef(id) {
  return {
    source: "env",
    provider: _typesSecretsBxqheYvy.t,
    id
  };
}
...
function resolveProviderDefaultEnvSecretRef(provider, config) {
  const envVar = (0, _providerEnvVarsBdegz5z.t)(provider, {
    ...(config ? { config } : {}),
...
function resolveApiKeySecretInput(provider, input, options) {
  if (options?.secretInputMode === "plaintext") return (0, _normalizeSecretInputCrCOUFln.n)(input);
  const coercedRef = (0, _typesSecretsBxqheYvy.o)(input);
  if (coercedRef) return coercedRef;
  const normalized = (0, _normalizeSecretInputCrCOUFln.n)(input);
...
function upsertApiKeyProfile(params) {
  const profileId = params.profileId ?? (0, _identityDX9yGpYy.t)({ providerId: params.provider });
...
function applyAuthProfileConfig(cfg, params) {
  const normalizedProvider = (0, _providerAuthAliases3NFJcokO.r)(params.provider, { config: cfg });
  const profiles = {
    ...cfg.auth?.profiles,
    [params.profileId]: {
  ...
  const configuredProviderProfiles = Object.entries(cfg.auth?.profiles ?? {}).filter(([, profile]) => (0, _providerAuthAliases3NFJcokO.r)(profile.provider, { config: cfg }) === normalizedProvider).map(([profileId, profile]) => ({
    profileId,
    mode: profile.mode
  }));
  const matchingProviderOrderEntries = Object.entries(cfg.auth?.order ?? {}).filter(([providerId]) => (0, _providerAuthAliases3NFJcokO.r)(providerId, { config: cfg }) === normalizedProvider);
  ...
  const baseOrder = matchingProviderOrderEntries.length > 0 ? Object.fromEntries(Object.entries(cfg.auth?.order ?? {}).filter(([providerId]) => (0, _providerAuthAliases3NFJcokO.r)(providerId, { config: cfg }) !== normalizedProvider)) : cfg.auth?.order;
...
async function writeOAuthCredentials(provider, creds, agentDir, options) {
  ...
  const profileId = (0, _identityDX9yGpYy.t)({
    providerId: provider,
