...
var _archiveBIRRxQjC = require("./archive-BIRRxQjC.js");
require("./local-file-access-Bd-6sdA_.js");
var _mediaSourceUrlHaUxP11l = require("./media-source-url-HaUxP11l.js");
...
function hostPathLooksAbsolute(expanded) {
  return _nodePath.default.isAbsolute(expanded) || (0, _archiveBIRRxQjC.h)(expanded);
}
function resolveToCwd(filePath, cwd) {
  const expanded = expandPath(filePath);
  if ((0, _archiveBIRRxQjC.h)(expanded)) return _nodePath.default.win32.normalize(expanded);
...
function resolveSandboxPath(params) {
  ...
  if (relative === ".." || relative.startsWith("../") || relative.startsWith("..\\") || _nodePath.default.isAbsolute(relative) || (0, _archiveBIRRxQjC.h)(relative)) throw new Error(`Path escapes sandbox root (${shortPath(rootResolved)}): ${params.filePath}`);
  return {
    resolved,
    relative
  };
...
async function assertSandboxPath(params) {
  ...
  await (0, _secureTempDirGC3bO7Qi.l)({
...
function isManagedMediaPathUnderRoot(candidate) {
  ...
  const mediaRoot = _nodePath.default.join((0, _utilsCKsuXgDI.d)(), "media");
  const resolvedMediaRoot = _nodePath.default.resolve(mediaRoot);
  const resolvedExpanded = _nodePath.default.resolve(expanded);
  if (resolvedExpanded === resolvedMediaRoot || !(0, _pathB5B_oAT.i)(resolvedMediaRoot, resolvedExpanded)) return false;
  const firstSegment = _nodePath.default.relative(resolvedMediaRoot, resolvedExpanded).split(_nodePath.default.sep)[0] ?? "";
...
async function resolveAllowedManagedMediaPath(candidate) {
  ...
  await assertNoManagedMediaAliasEscape({
    filePath: resolved,
    managedMediaRoot: _nodePath.default.resolve((0, _utilsCKsuXgDI.d)(), "media")
  });
  return resolved;
}
async function resolveSandboxedMediaSource(params) {
  ...
  if (!raw) return raw;
  if ((0, _mediaSourceUrlHaUxP11l.t)(raw)) return raw;
  let candidate = raw;
  if (/^file:\/\//i.test(candidate)) {
    ...
    try {
      candidate = (0, _fsSafeDpJlqO1z.C)(candidate);
    } catch (err) {
  ...
  (0, _fsSafeDpJlqO1z.y)(candidate, "Sandbox media path");
...
function mapContainerWorkspaceFileUrl(params) {
  ...
  if ((0, _fsSafeDpJlqO1z.x)(parsed.pathname)) return;
...
async function resolveAllowedTmpMediaPath(params) {
  ...
  const openClawTmpDir = _nodePath.default.resolve((0, _tmpOpenclawDirDJsd5QL.n)());
  if (!(0, _pathB5B_oAT.i)(openClawTmpDir, resolved)) return;
