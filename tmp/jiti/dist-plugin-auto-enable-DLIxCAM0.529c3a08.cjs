...
var _modelRefProfileBAEY2i_C = require("./model-ref-profile-BAEY2i_C.js");
var _bundledQ4WXUmmu = require("./bundled-Q4WXUmmu.js");
var _pluginScopeD0hUY2Gw = require("./plugin-scope-D0hUY2Gw.js");
var _harnessRuntimesBz4u5UtH = require("./harness-runtimes-Bz4u5UtH.js");
var _configPresenceZxkqSlIB = require("./config-presence-zxkqSlIB.js");
var _channelConfiguredMsxgGeYA = require("./channel-configured-MsxgGeYA.js");
var _setupRegistryC32GYnQD = require("./setup-registry-C32GYnQD.js");
var _pluginsAllowlistVNS6kKFK = require("./plugins-allowlist-VNS6kKFK.js");
...
function loadProviderRegistrySnapshot(params) {
  ...
  return (0, _pluginRegistryCdr1zbpf.p)({
...
function loadScopedProviderRegistry(params) {
  return {
    ...
    onlyPluginIdSet: (0, _pluginScopeD0hUY2Gw.t)(params.onlyPluginIds)
  };
}
...
function resolveProviderOwnerPluginIds(params) {
  ...
  const normalizedConfig = (0, _pluginRegistryCdr1zbpf.r)(params.config?.plugins, registry);
...
function toManifestOwnerRecord(plugin) {
  return {
    ...
    enabledByDefault: (0, _installedPluginIndexStoreDetkjvO.g)(plugin)
  };
}
function withBundledProviderVitestCompat(params) {
  return (0, _bundledCompatDfs9mbab.r)(params);
}
function resolveBundledProviderCompatPluginIds(params) {
  if (params.manifestRegistry) {
    const onlyPluginIdSet = (0, _pluginScopeD0hUY2Gw.t)(params.onlyPluginIds);
...
function stripModelProfileSuffix(value) {
  return (0, _modelRefProfileBAEY2i_C.t)(value).model;
}
...
function resolveOwningPluginIdsForProvider(params) {
  ...
  const deduped = dedupeSortedPluginIds([...(0, _pluginRegistryCdr1zbpf.c)({
    config: params.config,
    workspaceDir: params.workspaceDir,
    env,
    providerId: normalizedProvider,
    includeDisabled: true
  }), ...(0, _pluginRegistryCdr1zbpf.s)({
    config: params.config,
    workspaceDir: params.workspaceDir,
    env,
...
function splitEnvPaths(value) {
  ...
  if (!trimmed) return [];
  return (0, _stringNormalizationDEwYgSEp.s)(trimmed.split(/[;,]/g).flatMap((chunk) => chunk.split(_nodePath.default.delimiter)));
}
...
function resolveBuiltInChannelPreferOver(channelId) {
  const builtInChannelId = (0, _idsDFu3Ho6n.r)(channelId);
  if (!builtInChannelId) return [];
  return (0, _chatMetaDTmQp8Tt.t)(builtInChannelId)?.preferOver ?? [];
}
...
function canReuseUnscopedCurrentPluginMetadataSnapshot(config) {
  return (0, _configStateBgyjpLHd.s)(config.plugins).loadPaths.length === 0;
}
...
function hasConfiguredEmbeddedHarnessRuntime(cfg, env) {
  return (0, _harnessRuntimesBz4u5UtH.t)(cfg, env, { includeEnvRuntime: false }).length > 0;
}
...
function isProviderConfigured(cfg, providerId) {
  ...
  for (const { value: ref } of (0, _modelRefsDXRPZZl.n)(cfg, { includeChannelModelOverrides: false })) {
...
function normalizeManifestChannelId(channelId) {
  return (0, _idsDFu3Ho6n.r)(channelId) ?? channelId;
}
...
function collectPluginIdsForConfiguredChannel(channelId, registry) {
  ...
  const builtInId = (0, _idsDFu3Ho6n.r)(normalizedChannelId);
  const claims = [];
...
function collectConfiguredChannelIds(cfg, env) {
  const configuredStateChannelIds = new Set((0, _channelConfiguredMsxgGeYA.i)());
  return (0, _configPresenceZxkqSlIB.a)(cfg, env, { includePersistedAuthState: false }).map((signal) => ({
    source: signal.source,
    channelId: (0, _idsDFu3Ho6n.r)(signal.channelId) ?? signal.channelId
...
function isAutoEnableConfiguredChannelSignal(params) {
  if (params.source === "env" && params.configuredStateChannelIds.has(params.channelId) && !(0, _channelConfiguredMsxgGeYA.r)({
    channelId: params.channelId,
    cfg: params.cfg,
    env: params.env
  })) return false;
  return (0, _channelConfiguredMsxgGeYA.t)(params.cfg, params.channelId, params.env);
}
...
function hasConfiguredProviderModelOrHarness(cfg, env) {
  ...
  if ((0, _modelRefsDXRPZZl.n)(cfg, { includeChannelModelOverrides: false }).length > 0) return true;
...
function resolvePluginAutoEnableReadiness(cfg, env) {
  ...
  return {
    mayNeedAutoEnable: (0, _setupRegistryC32GYnQD.n)({
      config: cfg,
      env,
...
function resolveConfiguredPluginAutoEnableCandidates(params) {
  ...
  for (const { value: modelRef } of (0, _modelRefsDXRPZZl.n)(params.config, { includeChannelModelOverrides: false })) {
  ...
  for (const runtime of (0, _harnessRuntimesBz4u5UtH.t)(params.config, params.env, { includeEnvRuntime: false })) {
  ...
  if (hasSetupAutoEnableRelevantConfig(params.config)) {
    ...
    for (const entry of (0, _setupRegistryC32GYnQD.n)({
      config: params.config,
      env: params.env,
...
function isPluginExplicitlyDisabled(cfg, pluginId) {
  const builtInChannelId = (0, _idsDFu3Ho6n.r)(pluginId);
  if (builtInChannelId) {
...
function disableImplicitPreferredOverPlugin(params) {
  ...
  if (!(0, _idsDFu3Ho6n.r)(params.pluginId) && !isKnownPluginId(params.pluginId, params.manifestRegistry)) return params.config;
...
function resolveAutoEnableChannelId(params) {
  const builtInChannelId = (0, _idsDFu3Ho6n.r)(params.entry.pluginId);
...
function isKnownPluginId(pluginId, manifestRegistry) {
  if ((0, _idsDFu3Ho6n.r)(pluginId)) return true;
...
function materializeConfiguredPluginEntryAllowlist(params) {
  ...
  for (const pluginId of Object.keys(entries).toSorted((left, right) => left.localeCompare(right))) {
    ...
    next = (0, _pluginsAllowlistVNS6kKFK.t)(next, pluginId);
...
function resolveChannelAutoEnableDisplayLabel(entry, manifestRegistry) {
  const builtInChannelId = (0, _idsDFu3Ho6n.r)(entry.channelId);
  const plugin = manifestRegistry.plugins.find((record) => record.id === entry.pluginId);
  return (builtInChannelId ? (0, _chatMetaDTmQp8Tt.t)(builtInChannelId)?.label : void 0) ?? plugin?.channelConfigs?.[entry.channelId]?.label ?? plugin?.channelCatalogMeta?.label;
}
...
function resolvePluginAutoEnableManifestRegistry(params) {
  ...
  return ((0, _currentPluginMetadataSnapshotDHmOxzz.n)({
    config: params.config,
    env: params.env,
    allowWorkspaceScopedSnapshot: true
  }) ?? (() => {
    if (!canReuseUnscopedCurrentPluginMetadataSnapshot(params.config)) return;
    const snapshot = (0, _currentPluginMetadataSnapshotDHmOxzz.n)({
      env: params.env,
      allowWorkspaceScopedSnapshot: true,
      requireDefaultDiscoveryContext: true
    });
    return snapshot?.policyHash === (0, _installedPluginIndexStoreDetkjvO._)(params.config) ? snapshot : void 0;
  })())?.manifestRegistry ?? (0, _pluginMetadataSnapshotDlaHO4z.r)({
    config: params.config,
    env: params.env
  }).manifestRegistry;
}
function materializePluginAutoEnableCandidatesInternal(params) {
  ...
  for (const entry of params.candidates) {
    ...
    next = (0, _pluginsAllowlistVNS6kKFK.t)(next, entry.pluginId);
  ...
  for (const [pluginId, reasons] of autoEnabledReasons) if (!(0, _prototypeKeysDna4GplE.t)(pluginId)) autoEnabledReasonRecord[pluginId] = [...reasons];
  return {
    config: next,
    changes,
