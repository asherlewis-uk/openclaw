...
async function writeTextAtomic$1(filePath, content, options) {
  ...
  await (0, _replaceFileC7_Inj8B.r)({
    filePath,
    content: payload,
...
function tryReadJsonSync(pathname) {
  try {
    const raw = (0, _regularFileDaVeNX.i)({ filePath: pathname }).buffer.toString("utf8");
...
function writeJsonSync(pathname, data) {
  ...
  const payload = `${(0, _jsonStringifyDYDqVIo.t)(data, null, 2)}\n`;
...
async function tryReadJson(filePath) {
  try {
    const raw = (await (0, _regularFileDaVeNX.r)({ filePath })).buffer.toString("utf8");
    return JSON.parse(raw);
  } catch {
    return null;
  }
...
async function readJson(filePath) {
  let raw;
  try {
    raw = (await (0, _regularFileDaVeNX.r)({ filePath })).buffer.toString("utf8");
  } catch (err) {
...
function readJsonSync(filePath) {
  let raw;
  try {
    raw = (0, _regularFileDaVeNX.i)({ filePath }).buffer.toString("utf8");
  } catch (err) {
    throw new JsonFileReadError(filePath, "read", err);
  }
  try {
...
async function writeJson(filePath, value, options) {
  await writeTextAtomic$1(filePath, (0, _jsonStringifyDYDqVIo.t)(value, null, 2), {
    mode: options?.mode,
