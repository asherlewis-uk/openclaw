...
function normalizeNonNegativeInt(value, fallback) {
  const normalized = (0, _stringCoerceLndEvhRk.d)(value);
...
function normalizeStringArray(value, allowed, fallback) {
  ...
  for (const entry of value) {
    const normalizedEntry = (0, _stringCoerceLndEvhRk.s)(entry);
...
function resolveExecutionConfig(value, fallback) {
  const record = (0, _recordCoerceCtdL1LKF.n)(value);
...
function normalizePathForComparison(input) {
  ...
  return process.platform === "win32" ? (0, _stringCoerceLndEvhRk.r)(normalized) : normalized;
}
...
function resolveMemoryDreamingPluginId(cfg) {
  const configuredSlot = normalizeTrimmedString((0, _recordCoerceCtdL1LKF.n)((0, _recordCoerceCtdL1LKF.n)((0, _recordCoerceCtdL1LKF.n)(cfg)?.plugins)?.slots)?.memory);
  if (configuredSlot && (0, _stringCoerceLndEvhRk.a)(configuredSlot) !== "none") return configuredSlot;
  return DEFAULT_MEMORY_DREAMING_PLUGIN_ID;
}
function resolveMemoryDreamingPluginConfig(cfg) {
  const entries = (0, _recordCoerceCtdL1LKF.n)((0, _recordCoerceCtdL1LKF.n)((0, _recordCoerceCtdL1LKF.n)(cfg)?.plugins)?.entries);
  const pluginId = resolveMemoryDreamingPluginId(cfg);
  return (0, _recordCoerceCtdL1LKF.n)((0, _recordCoerceCtdL1LKF.n)(entries?.[pluginId])?.config) ?? void 0;
}
...
function resolveMemoryDreamingConfig(params) {
  const dreaming = (0, _recordCoerceCtdL1LKF.n)(params.pluginConfig?.dreaming);
  ...
  const storage = (0, _recordCoerceCtdL1LKF.n)(dreaming?.storage);
  const execution = (0, _recordCoerceCtdL1LKF.n)(dreaming?.execution);
  const phases = (0, _recordCoerceCtdL1LKF.n)(dreaming?.phases);
  ...
  const light = (0, _recordCoerceCtdL1LKF.n)(phases?.light);
  const deep = (0, _recordCoerceCtdL1LKF.n)(phases?.deep);
  const rem = (0, _recordCoerceCtdL1LKF.n)(phases?.rem);
  const deepRecovery = (0, _recordCoerceCtdL1LKF.n)(deep?.recovery);
...
function resolveMemoryDreamingWorkspaces(cfg, options = {}) {
  ...
  for (const entry of configured) {
    ...
    const id = (0, _stringCoerceLndEvhRk.s)(entry.id);
    if (!id || seenAgents.has(id)) continue;
    seenAgents.add(id);
    agentIds.push(id);
  }
  if (agentIds.length === 0) agentIds.push((0, _agentScopeConfigDCRwWQZy.c)(cfg));
  const byWorkspace = /* @__PURE__ */new Map();
  const addWorkspace = (workspaceDirRaw, agentIdRaw) => {
    const workspaceDir = workspaceDirRaw?.trim();
    if (!workspaceDir) return;
    const agentId = (0, _stringCoerceLndEvhRk.s)(agentIdRaw) || (0, _agentScopeConfigDCRwWQZy.c)(cfg);
  ...
  for (const agentId of agentIds) addWorkspace((0, _agentScopeConfigDCRwWQZy.o)(cfg, agentId), agentId);
  addWorkspace(options.primaryWorkspaceDir ?? void 0, options.primaryAgentId ?? (0, _agentScopeConfigDCRwWQZy.c)(cfg));
  return [...byWorkspace.values()];
}
//#endregion /* v9-3b95d5270c163a33 */
